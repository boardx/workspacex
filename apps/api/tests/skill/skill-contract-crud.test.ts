...
/**
 * design-delta `platform-owned-skills`（2026-08-27）—— `org-platform` 名下的 skill 对
 * **每一个** org 都可见（RLS platform-read 策略 + 查询层 OR 子句），包括这份测试用的
 * `ORG`。这不是测试隔离缺陷：这份夹具从未自己种过它们，只有当同一次
 * `with-test-isolation` 共享的数据库里**另一个测试文件**（`standard-platform-packs` /
 * `ensure-platform-skill-catalog` / `office-full-packages`）先种过，它们才会出现——
 * `org-platform` 是全局共享事实（同生产），`resetOrgs(ORG, OTHER_ORG)` 不会、也不应该
 * 清掉它。下面几条断言原本假设"这个 org 的列表 = 只有它自己种的草稿"，现在要**排除掉
 * 平台自有的 skill 再数**，不然断言在"平台 skill 是否已被种进这次共享测试库"这件与
 * 本文件无关的事实上随机摇摆。
 *
 * ⚠ **不要退回按 id 名字过滤。** 这里曾经写死 `OFFICIAL_SKILLS` 那四个 id；
 * `ensureStandardSkillPacksSeeded()` 往 `org-platform` 又种了 16 个 `skill-<uuid>`
 * 之后，四条名字一个都盖不住，本文件三条用例随之在 e2e-full 里变红。判据必须是**归属**
 * （`org_id = 'org-platform'`），不是名字——见 `tests/support/platform-owned-skills.ts`。
 */
...
describe("F192 · POST /skills 新写入路径已关闭，存量数据仍可读", () => {
  ...
  it("试图写 source（曾经 ⇒ SOURCE_TAG_IMMUTABLE）——现在写入口本身已冻结，同样是 410 且不入库", async () => {
    ...
    // org 自己的 skill"——见文件顶部 `withoutPlatformSkills` 的注释。
...
describe("F192 · 存量数据（经应用层直调种下的草稿）仍可读", () => {
  it("列表能看见种下的草稿，出参过契约 strict 校验", async () => {
    ...
    // 东西——过滤掉再断言，见文件顶部 `withoutPlatformSkills` 的注释。
    expect(listed.total).toBe(listed.items.length); // total 与实际返回条数自洽
    const ownItems = await withoutPlatformSkills(listed.items);
  ...
  it("空组织没有自己的 org-owned skill（真实空态），**不生成示例 skill**", async () => {
    ...
    // `org-platform` 名下的 skill 对所有 org 可见是这个 delta 的**本意**，不是要在
    // 这里被断言掉的缺陷。这条测试原本要守的是"没有为了填满界面伪造一个这个 org
    // 自己的 skill"，该守的性质换成"过滤掉平台自有的 skill 之后为空"依然成立。
    const response = await get(`${C.operations.listSkills.path}?orgId=${ORG}&entry=library`);
    const body = (await response.json()) as { items: readonly { skillId: string }[]; total: number };
    const ownItems = await withoutPlatformSkills(body.items);
