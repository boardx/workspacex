...
describe("V1 static: no built-in capability list exists in product code", () => {
  ...
  it("the prototype's mock data is counted as DEBT, not silently excluded", () => {
    ...
    const DECLARED_MOCK_DEBT = [
      ...
      "apps/web/lib/mock/chat-diagram-fabric.ts", // phase-01 chat 域 VZ-fabric 图保存 UI 先行遗留（#1149，与本轮 skill 修复无关，见 issue #1161）
      ...
      "apps/web/lib/mock/chat-diagram-fabric.ts", // chat 域 VZ-fabric（#1112）fabric.js 渲染+最大化/编辑/保存签核增量的原型预览数据（2026-08-13）
    ];
    const r = runGate();
