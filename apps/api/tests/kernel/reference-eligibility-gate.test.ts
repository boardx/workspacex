...
describe("contract gaps found while implementing F07", () => {
  ...
  // ⚠ F07 原本在这里登记了一条缺口：「本束三十来个 out schema 还不是 strict」，
  // 并注明「如果这个数归零，就该把它升级成 lint」。合并到主线时它确实归零了——
  // `packages/contracts/tests/out-schemas-strict.test.ts` 已经对**全部束**强制 strict。
  //
  // 于是缺口断言反过来把自己判红了。**这正是把缺口写成会变红的东西的意义**：
  // 缺口补上时它主动报警，而不是永远绿着、无人知道它已经过时。
  //
  // 断言不在此处重写，原因有二：
  //   ① 全仓门控已经覆盖，在这里再写一份就是「同一事实声明在两处」——本仓的头号失败模式；
  //   ② F07 那条（以及我按它修法改的第一版）都只看 `op.out._def.unknownKeys`，
  //      对 `listBackflow.out = z.array(BackflowEntry)` 会**误报**——
  //      数组本身没有 unknownKeys 这个概念，而元素 BackflowEntry 是 strict 的。
  //      全仓那道门控走的是树遍历（含嵌套 object 与数组元素，`.strict()` 不传播），
  //      比这里的浅层检查严格得多。
});
