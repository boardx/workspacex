...
describe("F09: 拒绝与不存在对外无法区分（I-3）", () => {
  ...
  it("组织成员但不在项目里：三个读接口都与「不存在」同一个出口（码与 404 响应体一致）", async () => {
    const ctl = new KnowledgeGraphController(deps.repo, deps.ids, deps.chat, deps.knowledge, unusedActions, {} as never, {} as never, {} as never, {} as never);
  ...
  it("判定依赖读不到（成员关系查询失败）⇒ 503，不是 404 / 500，也不放行", async () => {
    ...
    const ctl = new KnowledgeGraphController(failingRepo, deps.ids, deps.chat, deps.knowledge, unusedActions, {} as never, {} as never, {} as never, {} as never);
    ...
    const ctl2 = new KnowledgeGraphController(deps.repo, deps.ids, failingChat, deps.knowledge, unusedActions, {} as never, {} as never, {} as never, {} as never);
