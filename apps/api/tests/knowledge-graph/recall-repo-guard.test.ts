...
const TENANT = ["claims", "claim_message_evidence", "chat_messages", "ontology_objects"];
...
describe("F08 会话记忆召回读取的豁免前提", () => {
  it("(a) 只出现四张租户表（外加只回 id 的 kg_graph_neighbors）；不用逗号连接、不从子查询取行", () => {
  ...
  it("(c) 结论与实体都限定本会话作用域，绑定的是 [orgId, threadId]；读之前设 app.current_user_id = 发起人", () => {
    expect(code).toMatch(/FROM claims c\s+WHERE c\.org_id = \$1 AND c\.scope_kind = 'chat_session' AND c\.scope_id = \$2[^`]*`,\s*\[orgId, threadId\]/);
    ...
    expect(code.match(/FROM claims\b/g)).toHaveLength(1);
    expect(code.match(/FROM ontology_objects\b/g)).toHaveLength(1);
  ...
  it("(e) 类成员只有端口要求的两个方法——不能悄悄多出一个读全组织的方法（不论 async / 修饰符 / 箭头属性）", () => {
  ...
  it("(e2) 模块顶层只有 stripKind 和这个类——不能在类外另挂一个读正文的函数", () => {
    const topLevel = code.split("\n").filter((l) => /^\S/.test(l) && !/^(?:import\b|\}|\)|export\s+type\b|type\b)/.test(l));
    expect(topLevel.map((l) => /^(?:export\s+)?(?:const|class)\s+(\w+)/.exec(l)?.[1] ?? l)).toEqual(["stripKind", "PgKnowledgeRecall"]);
  });
});
