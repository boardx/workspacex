...
export async function seedCredential(opts: {
  ...
  /**
   * `credentials.display_name` is NOT NULL (F19 merged it in -- there is no separate
   * `users` table, so the name lives 1:1 with the credential). Defaulted here rather than
   * made required so F20/F21's tests state only what they are about.
   */
  displayName?: string;
  ...
  await asOwner(async (c) => {
    await c.query(
      `INSERT INTO credentials (user_id, email, password_hash, email_verified_at, display_name)
       VALUES ($1, $2, $3, $4, $5)
      ...
      [
        ...
        email,
        hash,
        (opts.emailVerified ?? true) ? new Date() : null,
