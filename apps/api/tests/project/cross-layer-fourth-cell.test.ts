 ...
 * 持管理权、不持内容读取权（Q-4② 裁）。
 *
 * ⚠ 2026-08-06（#614）：`authorizeManageMembers` 路径②的组织角色判据从「仅 `lead`」
 *   扩到「`lead` 或 `admin`」，第①条断言因此现在对 `admin` 同样成立——本文件**不**
 *   跟着复制一份 admin 版本（三条断言用同一个 actor 才证明的是「第 ④ 格」，
 *   见下方「反证方式」；admin 的对称版本在
 *   `tests/project/manage-members-admin-org-role.test.ts` 单独钉住，
 *   那份不重复本文件「同一个人三处」的组装论证，只钉 #614 本身）。
 *
 * ## 为什么这是「组装」而不是一处新判断
 *
