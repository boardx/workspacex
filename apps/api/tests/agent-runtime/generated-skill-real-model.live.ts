...
const generatedScenarios=[{id:'S015-generated',skill:'count-sum',filename:'counts.csv',source:'count\n1\n2\n',prompt:'请按适合的技能汇总附件CSV的count列。把真实计算结果和使用的脚本路径写入report.md并交付。',required:[/3/,/sum_counts\.py/],forbidden:[/已经启用|已发布技能/]}];
...
it.each(generatedScenarios)('$id real configured model selects skill and delivers source-grounded artifact',async(scenario)=>{
 ...
 try{
  for(const tool of ['wx_project_list','wx_project_read','wx_knowledge_search','wx_knowledge_read','write_todos','read_file','write_file','edit_file','ls','glob','execute','wx_artifact_publish','wx_audio_transcribe'])await grants.grantForRun(org,parent,tool);
  const transcript=scenario.source;
  const uploaded=await uploadAttachment({repo:identity,ids:{next:()=>randomUUID()},chat,attachments:new PgChatAttachmentRepository(db),store:objects,attachmentIds:{next:()=>randomUUID()},clock:{now:()=>new Date().toISOString()}},{orgId:org,userId:'actor',threadId:`thread-${org}`,filename:scenario.filename,mime:'text/csv',bytes:Buffer.from(transcript)});
  ...
  const generated=JSON.parse(await readFile(join(workspace,'docs/design/standard-capabilities/evidence/G-SKILL-methods/authoring-final/S015/count-sum-skill-draft.json'),'utf8'));const pack=verifySkillStarterPack(generated,{packId:generated.packId,packVersion:generated.packVersion});
  ...
  const ctx={orgId:org,parentRunId:parent,attemptId:parent+':0',leaseEpoch:1};const ref=await owner.provision(ctx,skills,{execute:false,wx_artifact_publish:false,wx_audio_transcribe:false,wx_project_list:false,wx_project_read:false,wx_knowledge_search:false,wx_knowledge_read:false});
  class TestModule{};Module({controllers:[StandardContextToolsController,NativeSessionController,NativeOutputStagingController,RunInterjectionController,StandardAudioController],providers:[{provide:STANDARD_CONTEXT_SERVICE,useValue:realMethodContext(db,identity,chat,objects)},{provide:IDENTITY_REPOSITORY,useValue:identity},{provide:DECISION_ID_FACTORY,useValue:{next:()=>randomUUID()}},{provide:CHAT_REPOSITORY,useValue:chat},{provide:NATIVE_SESSION_OWNER,useValue:owner},{provide:NATIVE_OUTPUT_STAGING,useValue:staging},{provide:TOOL_EXECUTION_AUTHORITY,useValue:authority},{provide:AGENT_RUN_STORE,useValue:repo},{provide:INTERJECTION_STORE,useValue:new PgInterjectionStore(db)},{provide:TOOL_PERMISSION_GRANT_STORE,useValue:grants},{provide:STANDARD_AUDIO_SERVICE,useValue:null}]})(TestModule);
  ...
  const execution=report.calls.find((call:{id:string;name:string;args:{command?:string}})=>call.name==='execute'&&call.args.command?.includes('scripts/sum_counts.py')&&report.results.some((result:{toolCallId:string;content:string;status:string})=>result.toolCallId===call.id&&result.status==='success'&&/\"total\"\s*:\s*3/.test(result.content)));expect(execution).toBeDefined();
  const files=await staging.listFiles(org,parent);expect(files.length).toBeGreaterThan(0);const minutes=files.find(f=>/(?:^|[-_])report\.md$/.test(f.name));expect(minutes).toBeDefined();
