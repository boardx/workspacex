/**
 * #662 直接后续 —— `backfillImageGenAgent`（`scripts/backfill-image-gen-agent.ts`）。
 * 同 `deep-research-agent-backfill.test.ts` 的形状：直接在数据库里造一个"这次改动
 * 落地之前就存在、从未走过 bootstrap/register"的组织，验证脚本补得对、补得幂等，
 * 且不动没有 admin 的组织。
 *
...
const DATABASE = `wsx_image_gen_backfill_${process.pid}_${Date.now()}`;

const ORG_WITH_ADMIN = "org-image-gen-backfill-with-admin";
const ADMIN_USER = "u-image-gen-backfill-admin";
const ORG_NO_ADMIN = "org-image-gen-backfill-no-admin";
...
describe("#662 backfillImageGenAgent：补种既有组织", () => {
  it("有 admin 的老组织补出一个真实已发布的图片生成 agent；没有 admin 的老组织被如实跳过", async () => {
    const { backfillImageGenAgent } = await import("../../scripts/backfill-image-gen-agent");

    const first = await backfillImageGenAgent();
    ...
    try {
      const agentRow = await owner.query<{ id: string; published_version_id: string | null }>(
        "SELECT id, published_version_id FROM agents WHERE org_id = $1 AND stable_name = 'image-gen-agent'",
        [ORG_WITH_ADMIN],
      );
      ...
      const versionRow = await owner.query<{ model_provider: string }>(
        `SELECT av.model_provider FROM agent_versions av
          ...
          WHERE a.org_id = $1 AND a.stable_name = 'image-gen-agent'`,
        [ORG_WITH_ADMIN],
      );
      expect(versionRow.rows[0]!.model_provider).toBe("bailian-image");
    ...
    const second = await backfillImageGenAgent();
