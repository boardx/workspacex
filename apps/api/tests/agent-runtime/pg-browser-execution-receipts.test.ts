  ...
  const session: TenantSession = {
    async query<R>(sql: string, params: readonly unknown[] = []): Promise<QueryResult<R>> {
      ...
      if (sql.includes('kernel_finish_browser_execution')) {
        ...
        row.status = 'succeeded'; row.result = JSON.parse(String(params[5]));
...
describe('Pg browser execution receipts', () => {
  it('replays only a matching durable success across store instances', async () => {
    ...
    expect(await first.claim(context('call-ok'), invocation, 'digest-a')).toEqual({ kind: 'claimed' });
    await first.succeed(context('call-ok'), invocation, 'digest-a', result);
    const afterRestart = new PgBrowserExecutionReceipts(db);
    expect(await afterRestart.claim(context('call-ok'), invocation, 'digest-a')).toEqual({ kind: 'succeeded', result });
    await expect(afterRestart.claim(context('call-ok'), invocation, 'digest-b')).rejects.toThrow('receipt conflict');
  });

  it('keeps pending or unconfirmed outcomes non-replayable', async () => {
    const { db } = database();
    const receipts = new PgBrowserExecutionReceipts(db);
    expect(await receipts.claim(context('call-unknown'), invocation, 'digest-a')).toEqual({ kind: 'claimed' });
    expect(await new PgBrowserExecutionReceipts(db).claim(context('call-unknown'), invocation, 'digest-a')).toEqual({ kind: 'unconfirmed' });
    await receipts.markUnconfirmed(context('call-unknown'), invocation, 'digest-a');
    expect(await receipts.claim(context('call-unknown'), invocation, 'digest-a')).toEqual({ kind: 'unconfirmed' });
  });

  it('locks the running leased run and exposes only narrow browser receipt functions', () => {
    const sql = readFileSync(new URL('../../migrations/20260909123000_w10_browser_execution_receipts.sql', import.meta.url), 'utf8');
