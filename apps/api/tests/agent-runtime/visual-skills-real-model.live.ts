...
import {visualScenarios as methodScenarios} from './visual-skills-real-model.cases';
...
it.each(methodScenarios)('$id real configured model selects skill and delivers source-grounded artifact',async(scenario)=>{
 ...
 try{
  ...
  if(scenario.id==='S017')expect(text.split(/[。；\n]/).some(clause=>/像素/.test(clause)&&/排除豆腐|确认文字已正确|确认非乱码/.test(clause)&&!/不能|不代表|无法|不足|未能|不可/.test(clause))).toBe(false);
  if(scenario.id==='S020'){const script=files.find(f=>f.name==='chart.py');expect(script).toBeDefined();const scriptBytes=await objects.get(script!.objectKey);expect(scriptBytes?.length).toBeGreaterThan(100);await writeFile(join(evidence,'chart.py'),Buffer.from(scriptBytes!));}
  const graphic=files.find(f=>f.name===scenario.image.name);expect(graphic).toBeDefined();
  const graphicBytes=Buffer.from((await objects.get(graphic!.objectKey))!);const graphicPath=join(evidence,scenario.image.name);await writeFile(graphicPath,graphicBytes);
  const decoded=await processRun('/usr/bin/sips',['-g','pixelWidth','-g','pixelHeight',graphicPath]);
  expect(decoded).toContain(`pixelWidth: ${scenario.image.width}`);expect(decoded).toContain(`pixelHeight: ${scenario.image.height}`);
