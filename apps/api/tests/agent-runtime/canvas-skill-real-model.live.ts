...
import {canvasScenarios as methodScenarios} from './canvas-skill-real-model.cases';
import {StandardCanvasService,STANDARD_CANVAS_SERVICE} from '../../src/application/agent-run/standard-canvas-tools';
import {StandardCanvasToolsController} from '../../src/interface/controllers/standard-canvas-tools.controller';
import {PgCanvasInstanceRepository} from '../../src/infrastructure/canvas/pg-canvas-instance-repository';
...
async function seed(scope: typeof org, id: string) {
  ...
  const seeded=await seedOrg({ orgId: scope, projectId: project });
  await addOrgMember(scope,"actor","consultant",null);
  await addProjectMember(scope,project,'actor','member',seeded.groups.g1!);
  await seedAgendaSegment(scope,project,`segment-${scope}`);
  await asApp(scope,c=>c.query("INSERT INTO canvas_templates(org_id,key,version,display_name,status,builtin,visibility,underlying_type,sections) VALUES($1,'swot',1,'SWOT','published',false,'org-wide','canvas','[]')",[scope]));
  const markdown='# Synthetic workflow\n\n```mermaid\ngraph TD\n A[Start]-->B[Review]\n```\n';
  await new PgCanvasInstanceRepository(db).createInstances({orgId:scope,agendaSegmentId:`segment-${scope}`,workshopId:project,idempotencyKey:'fixture',createdBy:'actor',instances:[{instanceId:`canvas-${scope}`,groupId:seeded.groups.g1!,templateKey:'swot',templateVersion:1,initialVersionId:`cv-${scope}`,initialMarkdown:markdown,contentHash:createHash('sha256').update(markdown).digest('hex')}]});
...
it.each(methodScenarios)('$id real configured model selects skill and delivers source-grounded artifact',async(scenario)=>{
 ...
 try{
  for(const tool of ['wx_canvas_read','wx_canvas_update','wx_project_list','wx_project_read','wx_knowledge_search','wx_knowledge_read','write_todos','read_file','write_file','edit_file','ls','glob','execute','wx_artifact_publish','wx_audio_transcribe'])await grants.grantForRun(org,parent,tool);
  const transcript=scenario.source.replace('__CANVAS__',`canvas-${org}`);
  ...
  const ctx={orgId:org,parentRunId:parent,attemptId:parent+':0',leaseEpoch:1};const ref=await owner.provision(ctx,skills,{wx_canvas_read:false,wx_canvas_update:false,execute:false,wx_artifact_publish:false,wx_audio_transcribe:false,wx_project_list:false,wx_project_read:false,wx_knowledge_search:false,wx_knowledge_read:false});
  class TestModule{};Module({controllers:[StandardCanvasToolsController,StandardContextToolsController,NativeSessionController,NativeOutputStagingController,RunInterjectionController,StandardAudioController],providers:[{provide:STANDARD_CANVAS_SERVICE,useValue:new StandardCanvasService({instances:new PgCanvasInstanceRepository(db),auth:{repo:identity,ids:{next:()=>randomUUID()}}})},{provide:STANDARD_CONTEXT_SERVICE,useValue:realMethodContext(db,identity,chat,objects)},{provide:IDENTITY_REPOSITORY,useValue:identity},{provide:DECISION_ID_FACTORY,useValue:{next:()=>randomUUID()}},{provide:CHAT_REPOSITORY,useValue:chat},{provide:NATIVE_SESSION_OWNER,useValue:owner},{provide:NATIVE_OUTPUT_STAGING,useValue:staging},{provide:TOOL_EXECUTION_AUTHORITY,useValue:authority},{provide:AGENT_RUN_STORE,useValue:repo},{provide:INTERJECTION_STORE,useValue:new PgInterjectionStore(db)},{provide:TOOL_PERMISSION_GRANT_STORE,useValue:grants},{provide:STANDARD_AUDIO_SERVICE,useValue:null}]})(TestModule);
  ...
  expect(report.calls.filter((c:{name:string})=>c.name==='wx_canvas_read').length).toBeGreaterThanOrEqual(2);expect(report.calls.some((c:{name:string})=>c.name==='wx_canvas_update')).toBe(true);
  const canvasService=new StandardCanvasService({instances:new PgCanvasInstanceRepository(db),auth:{repo:identity,ids:{next:()=>randomUUID()}}});
  const updated=await canvasService.read({orgId:org,userId:'actor'},{canvasId:`canvas-${org}`});expect(updated.revision).toBe(2);expect(updated.source).toContain('A[Start]');expect(updated.source).toContain('B[审核完成]');expect(updated.source).toMatch(/A(?:\[Start\])?\s*-->\s*B/);
  await expect(canvasService.update({orgId:org,userId:'actor'},{canvasId:`canvas-${org}`,expectedRevision:1,changes:{kind:'replace-source',markdown:'conflicting overwrite'},idempotencyKey:'stale-fixture'})).rejects.toThrow();
  await expect(canvasService.read({orgId:org,userId:'intruder'},{canvasId:`canvas-${org}`})).rejects.toThrow();
  await writeFile(join(evidence,'actual-canvas.json'),JSON.stringify(updated));
