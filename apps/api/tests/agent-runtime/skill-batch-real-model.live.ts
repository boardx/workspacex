...
it('real configured model executes the selected synthetic skill scenario',async()=>{
 ...
 try{
  ...
  await writeFile(join(evidence,'result.json'),JSON.stringify({model:report.model,caseId,pins:originalPins,selectedPin,outputSha256,artifactDownloadSha256,artifacts:files.length,actualWriteback:true,assertions},null,2));
 }finally{
