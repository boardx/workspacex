...
it('real configured model executes the selected synthetic skill scenario',async()=>{
 ...
 try{
  ...
  await writeFile(join(evidence,'result.json'),JSON.stringify({model:report.model,modelSnapshot:{provider:'dashscope',modelId:process.env.DASHSCOPE_MODEL},caseId,packId:scenario.packId,packVersion,packDigest:pack?.packDigest??null,packages:skills.map(s=>({name:s.stableName,version:s.package.versionId})),pins:originalPins,selectedPin,outputSha256,artifactDownloadSha256,artifacts:files.length,actualWriteback:true,assertions},null,2));
 }finally{
