...
import {authoringScenarios as methodScenarios} from './authoring-skill-real-model.cases';
...
it.each(methodScenarios)('$id real configured model selects skill and delivers source-grounded artifact',async(scenario)=>{
 ...
 try{
  for(const tool of ['wx_skill_create_draft','wx_project_list','wx_project_read','wx_knowledge_search','wx_knowledge_read','write_todos','read_file','write_file','edit_file','ls','glob','execute','wx_artifact_publish','wx_audio_transcribe'])await grants.grantForRun(org,parent,tool);
  ...
  const ctx={orgId:org,parentRunId:parent,attemptId:parent+':0',leaseEpoch:1};const ref=await owner.provision(ctx,skills,{wx_skill_create_draft:false,execute:false,wx_artifact_publish:false,wx_audio_transcribe:false,wx_project_list:false,wx_project_read:false,wx_knowledge_search:false,wx_knowledge_read:false});
  class TestModule{};Module({controllers:[SkillDraftController,StandardContextToolsController,NativeSessionController,NativeOutputStagingController,RunInterjectionController,StandardAudioController],providers:[{provide:SKILL_DRAFT_SERVICE,useValue:new DefaultSkillDraftService(owner,b=>createNativeDraftSession({socketPath:socket,...b}),authority,objects,staging)},{provide:STANDARD_CONTEXT_SERVICE,useValue:realMethodContext(db,identity,chat,objects)},{provide:IDENTITY_REPOSITORY,useValue:identity},{provide:DECISION_ID_FACTORY,useValue:{next:()=>randomUUID()}},{provide:CHAT_REPOSITORY,useValue:chat},{provide:NATIVE_SESSION_OWNER,useValue:owner},{provide:NATIVE_OUTPUT_STAGING,useValue:staging},{provide:TOOL_EXECUTION_AUTHORITY,useValue:authority},{provide:AGENT_RUN_STORE,useValue:repo},{provide:INTERJECTION_STORE,useValue:new PgInterjectionStore(db)},{provide:TOOL_PERMISSION_GRANT_STORE,useValue:grants},{provide:STANDARD_AUDIO_SERVICE,useValue:null}]})(TestModule);
  ...
  const draft=files.find(f=>f.name.endsWith('.json'));expect(draft).toBeDefined();const draftBytes=Buffer.from((await objects.get(draft!.objectKey))!);await writeFile(join(evidence,draft!.name),draftBytes);
  const parsedDraft=JSON.parse(draftBytes.toString('utf8'));const verifiedDraft=verifySkillStarterPack(parsedDraft,{packId:parsedDraft.packId,packVersion:parsedDraft.packVersion});
  const reportFile=verifiedDraft.skills[0]!.files.find(f=>/(?:^|\/)(?:validation-)?report\.md$/.test(f.path));const standaloneReport=files.find(f=>/(?:^|[-_])(?:verification[-_])?report\.md$/.test(f.name));expect(Boolean(reportFile||standaloneReport)).toBe(true);const bytes=reportFile?Buffer.from(reportFile.contentBase64,'base64'):Buffer.from((await objects.get(standaloneReport!.objectKey))!);const text=new TextDecoder('utf-8',{fatal:true}).decode(bytes);await writeFile(join(evidence,'report.md'),bytes);await writeFile(join(evidence,'source.txt'),transcript);
  ...
  expect(report.calls.some((c:{name:string})=>c.name==='wx_skill_create_draft')).toBe(true);
  const script=verifiedDraft.skills[0]!.files.find(f=>f.path==='scripts/sum_counts.py');expect(script).toBeDefined();expect(verifiedDraft.skills[0]!.stableName).toBe('count-sum');
  const bound=await owner.resolve(ref.bindingId,ctx),session=createNativeDraftSession({socketPath:socket,...bound});
  await session.write({path:'/workspace/acceptance-sum.py',contentBase64:script!.contentBase64});
  for(const [name,content]of [['positive','count\n1\n2\n'],['negative','count\n-1\n']])await session.write({path:`/workspace/acceptance-${name}.csv`,contentBase64:Buffer.from(content!).toString('base64')});
  const runFixture=async(name:string)=>{const output=JSON.parse(await processRun('docker',['exec','-i',container,'node','-e',relayCode],JSON.stringify({method:'POST',path:`/sessions/${bound.sessionId}/executions`,headers:{'content-type':'application/json',authorization:`Bearer ${bound.token}`},body:JSON.stringify({executionId:randomUUID(),command:`python3 /workspace/acceptance-sum.py /workspace/acceptance-${name}.csv`,timeoutMs:10000})})));expect(output.status).toBe(200);return JSON.parse(output.body);};
  const positive=await runFixture('positive'),negative=await runFixture('negative');expect(positive.exitCode).toBe(0);expect(JSON.parse(positive.output).total).toBe(3);expect(negative.exitCode).not.toBe(0);await writeFile(join(evidence,'independent-script-fixtures.json'),JSON.stringify({positive,negative}));
