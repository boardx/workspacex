...
import {methodScenarios} from './method-skills-real-model.cases';
...
it.each(methodScenarios)('$id real configured model selects skill and delivers source-grounded artifact',async(scenario)=>{
 ...
 try{
  ...
  if(scenario.id==='S014'){expect(report.calls.some((c:{name:string})=>c.name==='wx_project_list')).toBe(true);expect(report.calls.some((c:{name:string})=>c.name==='wx_project_read')).toBe(true);}
  ...
  if(scenario.id==='S019'){
   expect(text).toMatch(/未完成|退出者|放弃者|中途放弃/);expect(text).toMatch(/(?:可|可以|有权|允许).{0,8}跳过/);
   const questions=text.split('\n').filter(line=>/[？?]/.test(line));expect(questions.length).toBeGreaterThan(3);for(const question of questions)expect(question).toMatch(/O1|O2/);
  }
