...
it.each(methodScenarios)('$id real configured model selects skill and delivers source-grounded artifact',async(scenario)=>{
 ...
 // The finally-block readback below probes six workspace paths, four of which are legitimately
 // absent when the run failed early. Every ENOENT comes back as 404 SESSION_FILE_NOT_FOUND, and
 // recording those would overwrite transport-failure.json with a red herring that hides the real
 // failure — exactly what cost a full diagnosis cycle in #2930.
 let recordTransportFailures=true;
 const relay=createServer(async(req,res)=>{try{req.setEncoding('utf8');let body='';for await(const c of req)body+=c;const output=JSON.parse(await processRun('docker',['exec','-i',container,'node','-e',relayCode],JSON.stringify({method:req.method,path:req.url,headers:req.headers,body})));if(recordTransportFailures&&output.status>=400){const detail=JSON.parse(output.body);const code=typeof detail.error==='string'&&/^[A-Za-z0-9_ -]{1,100}$/.test(detail.error)?detail.error:'unclassified';const dir=join(process.env.WX_AUDIO_REAL_EVIDENCE!,scenario.id);await mkdir(dir,{recursive:true});await writeFile(join(dir,'transport-failure.json'),JSON.stringify({status:output.status,code}));}res.writeHead(output.status,{'content-type':'application/json'});res.end(output.body);}catch{res.writeHead(503);res.end('{}');}});await new Promise<void>(r=>relay.listen(socket,r));
 ...
 try{
  ...
  const bound=await owner.resolve(ref.bindingId,ctx),scriptSession=createNativeDocumentSession({socketPath:socket,...bound});
  const scriptApparatus=await scriptSession.execute({executionId:randomUUID(),command:"node -e \"process.stdout.write('S013_EXEC_APPARATUS_OK')\"",timeoutMs:5000});
  expect(scriptApparatus).toMatchObject({exitCode:0,timedOut:false,cancelled:false});expect(scriptApparatus.output).toBe('S013_EXEC_APPARATUS_OK');
  const scriptNetwork=await scriptSession.execute({executionId:randomUUID(),command:"node -e \"fetch('http://169.254.169.254/latest/meta-data').then(()=>{console.error('S013_NETWORK_UNEXPECTED_REACHABLE');process.exit(42)},()=>{console.error('S013_NETWORK_DENIED');process.exit(23)})\"",timeoutMs:5000});
  expect(scriptNetwork).toMatchObject({exitCode:23,timedOut:false,cancelled:false});expect(scriptNetwork.output).toContain('S013_NETWORK_DENIED');expect(scriptNetwork.output).not.toContain('S013_NETWORK_UNEXPECTED_REACHABLE');
  await writeFile(join(evidence,'network-counterproof.json'),JSON.stringify({browser:{urlClass:'link_local_metadata',error:'browser_network_denied',receiptDelta:receiptsAfter-receiptsBefore,forwarded:false},script:{apparatus:{exitCode:scriptApparatus.exitCode,signal:'S013_EXEC_APPARATUS_OK'},exitCode:scriptNetwork.exitCode,timedOut:scriptNetwork.timedOut,cancelled:scriptNetwork.cancelled,stderrClass:'S013_NETWORK_DENIED',providerDetail:'redacted'}}));
