...
it.each(["succeeded", "failed", "cancelled"] as const)("%s suppresses historical pause and pending approval controls", async status => {
  const run: PlanRunSnapshot = { runId: "run", status, pausedAt: "2026-09-07T00:00:00Z", pauseRequestedAt: "2026-09-06T23:59:00Z", pendingToolName: "call_skill", pendingArgsSummary: null, pendingPermissionRequestId: null, createdAt: "2026-09-06T23:00:00Z", agentId: "agent", remoteRunId: null, errorCode: status === "failed" ? "MODEL_CALL_FAILED" : null };
