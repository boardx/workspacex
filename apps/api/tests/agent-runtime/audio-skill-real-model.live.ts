...
it('real configured model selects the minutes skill and writes a source-grounded artifact',async()=>{
 ...
 try{
  ...
  const pack=verifySkillStarterPack(await new FileSkillStarterPackSource(join(workspace,'skills/starter-packs')).load('standard-audio','1.1.2'),{packId:'standard-audio',packVersion:'1.1.2'});
