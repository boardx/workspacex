...
describe("2026-08-30 回归：分区的条数上限（layout.max）要传给模型", () => {
  /**
   * ⚠ 2026-09-10 人类实测：产出的围栏里写着 `## 阶段2行为（最多4条）`，那块渲染成空白。
   *
   * 条数上限此前被拼进**分区名本身**（`名字(最多N条)`），而同一份指引又说「分区名必须
   * 与列出的分区名逐字一致」——模型两条都照做了，把括号一起抄进了 `## ` 那一行。
   * `lookupSectionItems` 按真名匹配，对不上，整块要点被静默丢弃。
   *
   * 修法：〔〕里只放真名，上限另起一段单独说。名字从此只有一个写法，模型没有第二种
   * 可复制的形态。
   */
  it("〔〕里只出现真名，条数上限另起一段——名字里不带括号", () => {
    const out = buildCanvasTemplateGuidance([{
      key: "persona",
      displayName: "用户画像",
      sections: [
    ...
    const line = out.split("\n").find((l) => l.startsWith("- persona"))!;
    // 分区名清单里是干净的名字——改动前这里是 `用户描述(最多6条)`。
    expect(line).toContain("〔用户描述/目标和需求〕");
    expect(line).not.toContain("用户描述(最多6条)");
    // 上限仍然给到模型，只是换了个位置。
    expect(line).toContain("条数上限〔用户描述=6条/目标和需求=6条〕");
    expect(out).toContain("是这块画布实际能放下的条数，按 N 尽量写满");
    // 并且逐字告诉模型：这个数不进 `## ` 那一行。
    expect(out).toContain("`## ` 后面**只写分区名本身**");
    expect(out).toContain("阶段2行为（最多4条）");
    /*
     * 2026-09-10 人类实测（journey-map 整张 20 格全空）：模型把交叉命名的分区
     * `行为 · 阶段1` 重排成了 `阶段1行为`，还把阶段号写成中文数字 `阶段一`。
     * 引擎那侧已经加了兜底（`lookupSectionItems` 第 ⑤ 级 + 中文数字归一），
     * 但兜底是安全网不是许可证——提示词这侧必须先把三种写法逐条点名说清楚，
     * 否则「写错了也能渲染出来」会让下一次的走样越走越远。
     */
    expect(out).toContain("`/` 本身不是名字的一部分");
    expect(out).toContain("不要重排成 `阶段1行为`");
    expect(out).toContain("`阶段1` 不要写成 `阶段一`");
  });
