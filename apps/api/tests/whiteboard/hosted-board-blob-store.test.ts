...
describe.each(['aliyun-oss', 's3-compatible'] as const)('%s BoardBlobStore contract', provider => {
  ...
  it('enumerates only the requested tenant and board with a bounded advancing cursor', async () => {
    ...
    client.list = async ({ keyPrefix, limit }) => ({
      objects: Array.from({ length: limit + 1 }, (_, index) => ({
        key: `${keyPrefix}update/sha256/${String(index).padStart(64, '0')}`,
        sizeBytes: 1,
...
describe('Hosted Board bucket policy', () => {
  it.each([
    ...
    { access: 'private', versioning: 'unsupported', objectLock: 'unsupported' },
