...
describe('AliyunOssBoardBlobClient', () => {
  it('uses forbid-overwrite, immutable metadata, prefixing, and policy inspection', async () => {
    ...
    const protocol: AliyunOssBoardProtocol = {
      ...
      async list() { return { objects: [{ name: 'board-content/tenants/t/boards/b/update/sha256/d', size: 3, lastModified: '2026-01-01T00:00:00.000Z' }], nextCursor: 'next' }; },
      async delete() {},
    };
    ...
    await expect(client.list({ keyPrefix: 'tenants/t/boards/b/', limit: 10 })).resolves.toEqual({
      objects: [{ key: 'tenants/t/boards/b/update/sha256/d', sizeBytes: 3, createdAt: new Date('2026-01-01T00:00:00.000Z') }],
      nextCursor: 'next',
    });
  });
  ...
  it('normalizes conflicts and missing objects and forwards version-fenced deletion', async () => {
    ...
    const protocol = {
      ...
      async get() { throw Object.assign(new Error('secret'), { code: 'NoSuchKey' }); }, async head() { throw Object.assign(new Error('secret'), { code: 'NoSuchKey' }); }, async list() { return { objects: [] }; },delete:remove,
    ...
    await expect(client.get('key', 1)).resolves.toBeNull(); await expect(client.head('key')).resolves.toBeNull();
...
describe('S3CompatibleBoardBlobClient', () => {
  it('uses If-None-Match for S3/R2/MinIO and carries verified metadata', async () => {
    ...
    const protocol: S3CompatibleBoardProtocol = {
      ...
      async listObjects() { return { objects: [{ key: 'board-content/tenants/t/boards/b/update/sha256/d', sizeBytes: 3, lastModified: new Date('2026-01-01T00:00:00.000Z') }], nextCursor: 'next' }; },
      async deleteObject() {},
    };
  ...
  it('normalizes conditional conflicts', async () => {
    const protocol = {
      ...
      async getObject() { return null; }, async headObject() { return null; }, async listObjects() { return { objects: [] }; },
      async deleteObject() {},
  ...
  it.each(['../escape', 'tenant/../escape', '/absolute', 'tenant//key'])('rejects unsafe object keys before calling the provider: %s', async key => {
    ...
    const protocol = {
      ...
      async getObject() { return null; }, async headObject() { return null; }, async listObjects() { return { objects: [] }; }, async deleteObject() {},
    } satisfies S3CompatibleBoardProtocol;
    const client = new S3CompatibleBoardBlobClient(protocol, 'bucket', 'prefix');
