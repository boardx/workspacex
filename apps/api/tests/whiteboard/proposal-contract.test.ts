...
describe('AI proposal trust boundary',()=>{
  ...
  it('requires bounded valid commands and explicit epoch/sequence preconditions',()=>{
    for(const bad of [{baseEpoch:0},{baseSeq:-1},{baseSeq:Number.MAX_SAFE_INTEGER+1},{commands:[]},{commands:Array.from({length:201},()=>({type:'delete',id:'note'}))},{commands:[{type:'execute-tool',tool:'admin'}]}])expect(C.CreateProposal.safeParse({...input,...bad}).success).toBe(false);
  });
