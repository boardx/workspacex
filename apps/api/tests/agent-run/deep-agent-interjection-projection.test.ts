 ...
 * · 缺席 ⇒ 两条分支的请求体都**没有**这个键（resume 连 `config` 键都没有）——T2 锁：
 *   `deep-agent-produces-files.test.ts` 已经锁死"没挂 skill 时 configurable 逐字不变"，
 *   这里只补 resume 分支的"逐字不变"对照。
 *
...
describe("#2755 DeepAgentModelProvider：ModelCallInput.interjection → config.configurable.interjection", () => {
  it("resume 分支：带插话 ⇒ command.resume 之外多出 config.configurable.interjection（形状逐字 = 输入）", async () => {
    ...
    expect(body.config).toEqual({ configurable: { [KERNEL_INTERJECTION_CONFIGURABLE_KEY]: INTERJECTION } });
    expect(KERNEL_INTERJECTION_CONFIGURABLE_KEY).toBe("interjection");
  });

  it("resume 分支：不带插话 ⇒ 请求体连 config 键都没有（与本 feature 之前逐字节相同）", async () => {
    const body = await lastBodyAfter({ ...BASE, resume: { decision: "reject" } });
    expect(Object.keys(body).sort()).toEqual(["assistant_id", "command"]);
  });
