/**
 * 投后两个 ad-hoc Agent 的**数据清除**脚本（issue #4012）：
 *   - team2「投后财务项目评级 Agent」
 *   - team4「投后管理报告 AI 生成单元」及其平台内置 Skill「投后管理报告」
 *
 ...
 * ## 定位方式（差异点）
 *
 * - team2 由已删除的 `backfill-team2-agent.ts` 用固定 stable_name 种下；更早的
 *   `publish-team2-agent.ts` 则走 API 建过同名 agent（stable_name 是自动生成的）。
 * - team4 只走过前端 `ensure-agent.ts` 的 API 建法，**没有**固定 stable_name。
 * 所以 agents 按「stable_name 命中 **或** 展示名命中」定位；`agents_name_casefold_uniq`
 * 保证同一组织里同名只有一行。按名字定位有误伤用户自建同名 agent 的理论可能，
 * 这正是默认 dry-run 的原因：先看清楚命中的 id / org 再 --apply。
 *
 * - 平台内置 Skill 由已删除的 `ensurePostInvestmentSkillSeeded()` 在 `main.ts` 启动时
 *   自愈种下：`skills` / `skill_versions` / `skill_version_files` / `capability_listings`
 *   各一行（id 固定），线程挂载写在 `thread_skill_mounts`。按 skill id 与 stable_name
 *   定位，凡是有 `skill_id` 列的表同样按 information_schema 横扫。
 *
 * ## 默认不删：用户的对话内容
 *
 ...
 * ## 用法
 *
 *   pnpm --filter api exec tsx scripts/purge-postinvest-agents.ts              # 只报告
 *   pnpm --filter api exec tsx scripts/purge-postinvest-agents.ts --apply      # 真的删
 *   pnpm --filter api exec tsx scripts/purge-postinvest-agents.ts --apply --purge-threads
 *
...
/** 与已删除的 `backfill-team2-agent.ts` 用的是同一个字面量。 */
export const TEAM2_AGENT_STABLE_NAME = "team2-postinvest-rating";
/** 已删除的 `lib/postinvest-rating/agent-directory.ts` 的 `RATING_AGENT.name`。 */
export const TEAM2_AGENT_DISPLAY_NAME = "投后财务项目评级 Agent";
/** 已删除的 `lib/post-investment/ensure-agent.ts` 的 `AGENT_NAME`（无固定 stable_name）。 */
export const TEAM4_AGENT_DISPLAY_NAME = "投后管理报告 AI 生成单元";
/** 已删除的 `lib/post-investment/skill-identity.ts` 的 `POST_INVESTMENT_SKILL_ID`。 */
...
export const POST_INVESTMENT_SKILL_STABLE_NAME = "post-investment-report";
export const POST_INVESTMENT_SKILL_DISPLAY_NAME = "投后管理报告";

const AGENT_NAMES = [TEAM2_AGENT_DISPLAY_NAME, TEAM4_AGENT_DISPLAY_NAME] as const;
const TAG = "[purge-postinvest-agents]";
...
export async function purgePostinvestAgents(
  ...
  try {
    ...
    const { rows: agentRows } = await client.query<{ id: string; org_id: string; name: string }>(
      "SELECT id, org_id, name FROM agents WHERE stable_name = $1 OR name = ANY($2::text[])",
      [TEAM2_AGENT_STABLE_NAME, [...AGENT_NAMES]],
    );
    ...
    const { rows: skillRows } = await client.query<{ id: string; org_id: string }>(
      ...
      [POST_INVESTMENT_SKILL_ID, POST_INVESTMENT_SKILL_STABLE_NAME],
    );
    const skillIds = skillRows.map((r) => r.id);

    if (agentIds.length === 0 && skillIds.length === 0) {
      await client.query("ROLLBACK");
      console.log(`${TAG} 库里没有投后两个 agent 或其内置 Skill 的行——无需清除（幂等）`);
    ...
    const listingSql =
      "(kind = 'agent' AND name = ANY($1::text[])) OR (kind = 'skill' AND (id = ANY($2::text[]) OR name = $3))";
    const listingArgs = [[...AGENT_NAMES], skillIds.length > 0 ? skillIds : [POST_INVESTMENT_SKILL_ID], POST_INVESTMENT_SKILL_DISPLAY_NAME];
    ...
    console.log(
      `${TAG} 入编过这两个 agent 的 chat 线程 ${threadIds.length} 条` +
...
if (isCliEntry(import.meta.url)) {
  ...
  await purgePostinvestAgents({ apply, purgeThreads });
}
