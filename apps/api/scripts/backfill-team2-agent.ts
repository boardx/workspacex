 ...
 * "本机开发库手工验证"的逃生口。
 *
 * （这个形状最早由 team3 的补种脚本示范，那条线已于 2026-09-22 随 team3 整体下线删除；
 *  同形状的现存参照是 `backfill-deep-research-agent.ts`。）
 *
 * ## 为什么是脚本，不是迁移；为什么不 hook 进注册控制器
 *
 * 同 `backfill-deep-research-agent.ts` 的两段理由，逐字适用，此处不复述（ADR-020：同一事实
...
export const TEAM2_AGENT_TEMPLATE: SystemAgentTemplate = {
  ...
  // 0x7ea3 曾由 team3 占用（已随其下线删除）；本条沿用一个不与之相撞的常量。
  lockKey: 0x7ea4,
