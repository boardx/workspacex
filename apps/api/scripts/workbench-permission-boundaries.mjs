...
export const workbenchBoundaries = new Map([
  ...
  ['src/infrastructure/whiteboard/pg-board-content-rollout.ts', {
    ...
    reason:'Internal CLI-only fleet control plane over the already admitted Board content relocation state machine. It has no requester-facing disclosure surface: reads return only Board UUIDs, durable control state, bounded aggregate counters and sanitized error codes. Tenant rows remain RLS scoped. Cross-tenant concurrency, rate and fairness state is inaccessible to app_rw and is reached only through tenant-bound security-definer primitives with a fixed search path and metadata-only return. Stable snapshot cycles, token/epoch fenced claims, persistent pause/cancel and append-only audit are mechanically checked. Content bytes, object keys, exception messages and user content never enter this repository.',
    checks:[[null,/this\.db\.withTenant\(toOrgId\(tenantId\)/],
      [null,/snapshot_upper_board_id[\s\S]*?ORDER BY b\.id LIMIT \$4/],
      [null,/lease_owner=\$14 AND lease_token=\$15 AND lease_epoch=\$16/],
      [null,/claim_whiteboard_content_rollout_admission/],
      [null,/release_whiteboard_content_rollout_admission/],
      [null,/invalidate_whiteboard_content_rollout_leases/],
