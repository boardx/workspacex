...
export const whiteboardPermissionBoundaries = new Map([
  ['src/infrastructure/whiteboard/pg-board-content-copy-store.ts', {
    ...
    checks: [
      ...
      /await this\.assertVisible\(session,p,sourceBoardId\)[\s\S]*await this\.lockSourceTags\(session,p,sourceBoardId\)[\s\S]*await this\.capture\(session,p,sourceBoardId\)[\s\S]*await this\.assertTagsUnchanged\(session,p,sourceBoardId,sourceTagIds\)/,
      /FROM whiteboard_tag_bindings bt[\s\S]*JOIN whiteboard_tags t[\s\S]*t\.deleted_at IS NULL[\s\S]*FOR SHARE OF t/,
      /\(b\.owner_id=\$2 OR m\.user_id IS NOT NULL\) FOR SHARE OF b/,
      ...
      /INSERT INTO whiteboards\(id,org_id,owner_id,request_id,name,tags_revision\)[\s\S]*\[targetBoardId,p\.orgId,p\.userId,input\.targetName\]/,
