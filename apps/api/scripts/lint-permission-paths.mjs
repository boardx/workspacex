...
const ALLOWLIST = new Map([
  ...
  [
    "src/infrastructure/agent/pg-default-agent-repository.ts",
    "#662 ensureDefaultAgent: a system-provisioned WRITE path with no admin gate to make ONE LAYER UP, because there is no caller to gate against -- the only call site is `auth-registration.controller.ts` immediately after `bootstrapFirstUser` COMMITS, for the org that same request JUST CREATED, acting as the SAME user who is that org's sole member and its only admin (the authorization already happened: becoming the first user of a brand-new org IS the grant). The one SELECT (`agents` by `stable_name`) exists only to decide idempotency -- did this org already get its default agent -- and returns nothing but an id, never a row's content; the three INSERTs (`agents`/`agent_versions`/`capability_listings`) and the one UPDATE (`agents.published_version_id`) write rows this same call is minting from a fixed, hardcoded system template (`DEFAULT_AGENT_INSTRUCTIONS`), not content sourced from -- or disclosed to -- any other tenant or any other member of this org. No `Guarded<T>` is possible here because nothing is ever read back and shown to anyone: the repository's only return value is the id it just wrote. ⚠ The exemption is valid ONLY while (a) this file names no tenant table other than `agents`/`agent_versions`/`capability_listings`, (b) it never calls `withoutTenant`, and (c) its sole call site remains the post-bootstrap step in `auth-registration.controller.ts` -- not a route reachable with a caller-supplied `orgId` that differs from the org the request itself just created.",
  ],
  [
