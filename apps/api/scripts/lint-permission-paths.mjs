...
const ALLOWLIST = new Map([
  ...
  [
    ...
    "#595 URL import: a WRITE path plus its own idempotency replay, with the organization-admin decision already made ONE LAYER UP and -- critically -- BEFORE the fetch. `application/skill-import/import-skill-from-url.ts` calls `identities.findOrgMembership(actorId, orgId)` and refuses anything but `orgRole === 'admin'` as its FIRST action, the same gate `import-skill-starter-pack.ts` applies to the same four tables; both import paths therefore lead to `skills`/`skill_versions`/`skill_version_files` through one height of door, not two. Ordering is part of the justification, not decoration: an authorization placed AFTER the fetch would still let a non-admin make the server issue an outbound request on their behalf -- an SSRF prober -- even though nothing is ever persisted. Same shape as the F119 / F124 entries above (permission decided one layer up, repository second). `disclose()` also has no question to ask here: three of this file's statement groups are INSERTs, and every SELECT is either the caller's own `skill_url_imports` row keyed by `(org_id, idempotency_key)` -- the actor's own prior write echoed back for replay -- or the `path` column of the version that replay just identified, which is a file NAME the same caller supplied on the import that created it. No Artifact, no Segment, no other tenant's row. ⚠ The exemption is valid ONLY while (a) this file names no tenant table other than `skills`/`skill_versions`/`skill_version_files`/`skill_url_imports`, (b) it never calls `withoutTenant`, and (c) the admin check in `import-skill-from-url.ts` exists AND still precedes the `deps.fetch` call -- so it is not left as a claim: tests/skill/url-import-repo-guard.test.ts parses both files and fails on a fifth table, on `withoutTenant`, on the admin check disappearing, and on that check moving after the fetch. If that test is ever deleted, this entry must go with it.",
  ],
  [
