 ...
 * fields, because a shared row type is how a body ends up somewhere that only wanted a
 * status -- and the personal-layer boundary is exactly a rule about what must not travel.
 *
 * `findItemBody` returns `Guarded<string>`, not a string. F02's guarded read path and F03's
 * content read path were built in parallel and each grew its own way of keeping tenant
 * content behind a decision; the merge is where that showed up as two doorways for one
 * rule. Converged here: the body is only reachable through `permission-filter`, whichever
 * decision function produced the verdict.
 */
...
export class PgContentRepository implements ContentRepository {
  ...
  async findItemBody(orgId: OrgId, itemId: string): Promise<Guarded<string> | null> {
    return this.db.withTenant(orgId, async (s) => {
      ...
      return body === undefined ? null : guard({ kind: "artifact", id: itemId }, body);
    });
  }
