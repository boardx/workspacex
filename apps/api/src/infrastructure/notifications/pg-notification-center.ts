...
import type { NotificationCenter, NotificationViewer, PublishNotificationInput } from "../../application/notifications/notification-center";
interface Row { id: string; kind: Notification["kind"]; title: string; body: string; thread_id: string | null; created_at: Date; read_at: Date | null }
const project = (row: Row): Notification => Notification.parse({
  id: row.id, kind: row.kind, title: row.title, body: row.body, threadId: row.thread_id,
...
export class PgNotificationCenter implements NotificationCenter {
  ...
  async publish(input: PublishNotificationInput): Promise<void> {
    const run = async (s: { query: (sql: string, params: unknown[]) => Promise<unknown> }) => {
      await s.query(
        `INSERT INTO user_notifications(org_id,user_id,kind,title,body,thread_id,source_key) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
        [input.orgId, input.userId, input.kind, input.title.slice(0, 500), input.body.slice(0, 4000), input.threadId ?? null, input.sourceKey ?? null],
      );
    };
  ...
  async list(viewer: NotificationViewer): Promise<z.infer<typeof NotificationListOutput>> {
    return this.db.withTenant(viewer.orgId, async (s) => {
      const rows = (await s.query<Row>(
        `SELECT id,kind,title,body,thread_id,created_at,read_at FROM user_notifications WHERE user_id=$1 AND (org_id IS NULL OR org_id=$2) ORDER BY (read_at IS NULL) DESC, created_at DESC LIMIT $3`,
