...
import {disclose,type Guarded,type Disclosure} from '../../application/security/permission-filter';
...
export interface AuthorizedIndexedSegment {readonly row:CandidateRow;readonly locator:LocatorRow;readonly permissionDecisionId:string;}
...
export class PgOrganizationKnowledgeIndex {
 ...
 private async authorized(actor:TrustedContextActor,item:Guarded<CandidateRow>,projectId?:string,path:"retrieval"|"embedding-similarity"="retrieval"):Promise<AuthorizedIndexedSegment|null>{
  ...
  const result=await disclose(this.identity,{orgId:actor.orgId,userId:actor.userId,projectId:locator.project_id??undefined,action:'read.allHands',path,items:[item]});
  ...
  return {row,locator,permissionDecisionId:result.visible[0]!.permissionDecisionId};
 }
 async discloseChannel(actor:TrustedContextActor,items:readonly Guarded<CandidateRow>[],projectId:string|undefined,path:"retrieval"|"embedding-similarity"):Promise<Disclosure<CandidateRow>>{
  const visible=[];
  for(const item of items){const value=await this.authorized(actor,item,projectId,path);if(value)visible.push({ref:item.ref,payload:value.row,permissionDecisionId:value.permissionDecisionId});}
  // Out-of-profile/hidden source identities never become organization-tool diagnostics.
