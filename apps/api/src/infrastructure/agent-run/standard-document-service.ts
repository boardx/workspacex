...
export class DefaultStandardDocumentService implements StandardDocumentService {
 ...
 async parse(context:DocumentContext,raw:z.infer<typeof DocumentParseInput>){
  ...
  const nativeStructure=input.outputMode==='chunks'&&!input.ocr;
  const structuredFormats=['application/pdf','application/vnd.openxmlformats-officedocument.wordprocessingml.document','application/vnd.openxmlformats-officedocument.presentationml.presentation','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'];
  if(nativeStructure&&!structuredFormats.includes(source.mediaType))throw new Error('document_structure_format_unsupported');
  ...
  const structurePath=`${directory}/structure.json`;
  const nativeStructureCommand=['python3','/usr/local/lib/workspacex/structure-document.py','--source',source.path,'--media-type',source.mediaType,'--output',structurePath].map(quote).join(' ');
  const command=(input.ocr?'':`mkdir -- ${quote(directory)} && `)+convert.map(quote).join(' ')+(nativeStructure?` && ${nativeStructureCommand}`:'');
  ...
  if(input.ocr||nativeStructure){
   const file=schemas.file.parse(await session.read(structurePath)),data=Buffer.from(file.contentBase64,'base64');
   const maxOutput=input.ocr?DOCUMENT_OCR_LIMITS.maxOutputBytes:DOCUMENT_STRUCTURE_LIMITS.maxOutputBytes;
   if(file.path!==structurePath||data.length!==file.sizeBytes||data.toString('base64')!==file.contentBase64||data.length+content.length>maxOutput)throw new Error('document_structure_invalid');
   const structure=DocumentStructure.parse(JSON.parse(new TextDecoder('utf-8',{fatal:true}).decode(data)));
   ...
   }else if(structure.sourceFormat==='pdf'){
    for(const table of structure.tables??[]){
     const cells=structure.chunks.filter(chunk=>chunk.type==='pdf_table_cell'&&chunk.locator.tableId===table.tableId&&chunk.locator.fragmentIndex===table.fragmentIndex);
     if(!cells.length||cells.some(cell=>cell.type!=='pdf_table_cell'||cell.locator.pageNumber!==table.pageNumber||cell.locator.pageTableIndex!==table.pageTableIndex))throw new Error('document_structure_invalid');
    }
   }
  ...
  const warnings=input.ocr?['ocr_may_misrecognize_text','tables_may_lose_layout'] as const
   :input.outputMode!=='chunks'?['markdown_only_no_page_coordinates','ocr_not_performed','tables_may_lose_layout'] as const
   :source.mediaType==='application/pdf'?['ocr_not_performed','cross_page_table_grouping_heuristic'] as const
   :source.mediaType==='application/vnd.openxmlformats-officedocument.wordprocessingml.document'?['ocr_not_performed','docx_page_numbers_unavailable','office_native_locations_no_rendered_coordinates'] as const
   :source.mediaType==='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'?['ocr_not_performed','office_native_locations_no_rendered_coordinates','formulas_not_calculated'] as const
   :['ocr_not_performed','office_native_locations_no_rendered_coordinates'] as const;
  return DocumentParseOutput.parse({...structureResult,textPath,sourceHash:source.digest,textHash:hash(content),source:{attachmentId:source.attachmentId,path:source.path,mediaType:source.mediaType,sizeBytes:source.sizeBytes},warnings});
 }
}
