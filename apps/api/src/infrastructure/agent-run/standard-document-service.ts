...
import {DocumentParseInput,DocumentParseOutput,DOCUMENT_PARSE_LIMITS,DOCUMENT_PARSE_TOOL} from '@repo/contracts/standard-document-tools';
...
export class DefaultStandardDocumentService implements StandardDocumentService {
 ...
 async parse(context:DocumentContext,raw:z.infer<typeof DocumentParseInput>){
  ...
  if(plan.kind!=='convert'&&plan.kind!=='passthrough')throw new Error('document_parse_format_unsupported');
  const directory=`/workspace/parsed-${randomUUID()}`,textPath=`${directory}/document.md`;
  const convert=plan.kind==='convert'
  ...
  const command=`mkdir -- ${quote(directory)} && ${convert.map(quote).join(' ')}`;
  ...
  return DocumentParseOutput.parse({textPath,sourceHash:source.digest,textHash:hash(content),source:{attachmentId:source.attachmentId,path:source.path,mediaType:source.mediaType,sizeBytes:source.sizeBytes},warnings:['markdown_only_no_page_coordinates','ocr_not_performed','tables_may_lose_layout']});
 }
}
