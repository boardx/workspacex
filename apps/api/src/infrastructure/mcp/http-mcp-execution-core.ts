...
export function createHttpMcpExecutionCore(options:{seams?:GuardedFetchSeams;extraTrustedCa?:string|Buffer;timeoutMs?:number}={}):McpExecutionCall{
 return async(frozen,args)=>{
  ...
  const transport=new StreamableHTTPClientTransport(endpoint,{fetch:bounded,reconnectionOptions:{maxRetries:0,initialReconnectionDelay:0,maxReconnectionDelay:0,reconnectionDelayGrowFactor:1}});
