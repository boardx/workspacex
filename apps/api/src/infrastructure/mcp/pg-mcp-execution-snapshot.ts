...
const digest=(value:unknown)=>createHash('sha256').update(JSON.stringify(value,(_k,v)=>v&&typeof v==='object'&&!Array.isArray(v)?Object.fromEntries(Object.entries(v).sort(([a],[b])=>a<b?-1:a>b?1:0)):v)).digest('hex');
...
interface ServerFacts {server_id:string;endpoint:string;auth_scope:AuthScope;review_status:ReviewStatus;connection_status:ConnectionStatus;quarantine_until:Date|null;current_review_id:string|null;involves_customer_data:boolean;credential_configured:boolean;}
...
export class PgMcpExecutionSnapshot implements McpExecutionSnapshot {
 constructor(private db:DatabasePort,private reader:ToolAuthorityReader,private authority:ToolExecutionAuthority,private visibility:ResolveVisibilityDeps,private execute:McpExecutionCall){}
 ...
 private async servers(org:OrgId):Promise<ServerFacts[]>{return this.db.withTenant(org,async s=>(await s.query<ServerFacts>(`SELECT m.server_id,m.endpoint,m.auth_scope,m.review_status,m.connection_status,m.quarantine_until,m.current_review_id,m.involves_customer_data,
 ...
 private async eligible(org:OrgId,facts:RunFacts):Promise<z.infer<typeof McpFrozenTool>[]>{
  const organization=await this.visibility.repo.findOrganization(org);
  ...
  for(const server of await this.servers(org)){
   if(!server.current_review_id||!callability({reviewStatus:server.review_status,connectionStatus:server.connection_status}).callable||server.credential_configured||server.involves_customer_data||server.quarantine_until&&server.quarantine_until.getTime()>Date.now()||!await this.allowedScope(org,facts,server.auth_scope))continue;
   const review=await this.db.withTenant(org,async s=>(await s.query<{endpoint:string;record:unknown;tools:unknown}>('SELECT endpoint,record,tools FROM mcp_review_snapshots WHERE org_id=$1 AND review_id=$2 AND server_id=$3',[org,server.current_review_id,server.server_id])).rows[0]);
   if(!review||review.endpoint!==server.endpoint)continue;
   ...
   for(const tool of await createPgMcpToolStore(this.db,org).current(server.server_id)){
    ...
    tools.push(McpFrozenTool.parse({reviewId:server.current_review_id,endpoint:server.endpoint,tool,whitelistEntry:entry,runtime:runtimeDescriptor(tool)}));
   }
  }
 ...
 private async recheck(context:ToolExecutionCheck,frozen:z.infer<typeof McpFrozenTool>){
  ...
  const current=(await this.eligible(context.orgId,facts)).find(t=>t.runtime.name===frozen.runtime.name);
  if(!current||current.reviewId!==frozen.reviewId||current.endpoint!==frozen.endpoint||current.tool.schemaFingerprint!==frozen.tool.schemaFingerprint)throw new Error('mcp_capability_revoked');
 }
 async invoke(runId:string,raw:z.infer<typeof McpInvokeInput>){
  ...
  if(!frozen||!current||frozen.reviewId!==current.reviewId||frozen.endpoint!==current.endpoint||frozen.tool.schemaFingerprint!==current.tool.schemaFingerprint)throw new Error('mcp_capability_revoked');
  ...
  const cached=await this.admitted(context,async()=>{
   ...
   await s.query("INSERT INTO mcp_tool_executions(org_id,run_id,tool_call_id,tool_name,args_digest,status) VALUES($1,$2,$3,$4,$5,'pending')",[org,runId,input.toolCallId,input.toolName,hash]);return null;
   });
  });
  if(cached)return cached;
  try{const result=await this.execute(frozen,input.toolArgs);return await this.admitted(context,async()=>{
   await this.recheck(context,frozen);
   const saved=await this.db.withTenant(org,s=>s.query("UPDATE mcp_tool_executions SET status='succeeded',result=$4::jsonb WHERE org_id=$1 AND run_id=$2 AND tool_call_id=$3 AND status='pending' RETURNING tool_call_id",[org,runId,input.toolCallId,JSON.stringify(result)]));if(saved.rows.length!==1)throw new Error('mcp_execution_unconfirmed');return result;
  });}
  catch{await this.db.withTenant(org,s=>s.query("UPDATE mcp_tool_executions SET status='unconfirmed' WHERE org_id=$1 AND run_id=$2 AND tool_call_id=$3 AND status='pending'",[org,runId,input.toolCallId]));throw new Error('mcp_execution_unconfirmed');}
 }
}
