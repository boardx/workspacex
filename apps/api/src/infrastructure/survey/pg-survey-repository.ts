import { claimSurveyAttachments } from "./pg-survey-attachment-repository";
import type {
  SurveyLibraryTemplate,
  SurveyTemplateInput,
} from "@repo/contracts/survey-template-library";
import {
  assertTemplateUpdate,
  type SurveyTemplateRepository,
} from "../../application/survey/survey-template-service";
...
export class PgSurveyRepository
  implements SurveyRepository, SurveyTemplateRepository
{
    ...
    return this.db.withTenant(orgId, async (s) => {
      ...
      const value = await work(record, {
        claimAttachments: (input) =>
          claimSurveyAttachments(s, { ...input, orgId, surveyId: id }),
      });
  ...
  async delete(orgId: OrgId, id: string, ownerId: string, version: number) {
    await this.db.withTenant(orgId, async (s) => {
      ...
      // Keep object metadata until the purge worker confirms physical deletion.
      // The workspace lock serializes this with upload finalization and answer claim.
      await s.query(
        "UPDATE survey_attachments SET status='deleted',expires_at=now() WHERE org_id=$1 AND survey_id=$2",
        [orgId, id],
      );
      await s.query(
        "UPDATE survey_upload_sessions SET expires_at=now() WHERE org_id=$1 AND survey_id=$2",
        [orgId, id],
      );
  ...
  async createTemplate(
    orgId: OrgId,
    ownerId: string,
    model: SurveyLibraryTemplate,
  ) {
    await this.db.withTenant(orgId, (session) =>
      session.query(
  ...
  getTemplate(orgId: OrgId, ownerId: string, id: string) {
    return this.db.withTenant(orgId, async (session) => {
      const result = await session.query<{ document: SurveyLibraryTemplate }>(
        "SELECT document FROM survey_library_templates WHERE org_id=$1 AND owner_id=$2 AND id=$3",
        [orgId, ownerId, id],
      );
  ...
  updateTemplate(
    ...
    return this.db.withTenant(orgId, async (session) => {
      const result = await session.query<{ document: SurveyLibraryTemplate }>(
        "SELECT document FROM survey_library_templates WHERE org_id=$1 AND owner_id=$2 AND id=$3 FOR UPDATE",
        [orgId, ownerId, id],
      );
      ...
      const model: SurveyLibraryTemplate = {
        kind: input.kind,
        title: input.title,
      ...
      await session.query(
        "UPDATE survey_library_templates SET document=$4::jsonb,updated_at=now() WHERE org_id=$1 AND owner_id=$2 AND id=$3",
        [orgId, ownerId, id, JSON.stringify(model)],
      );
      return model;
    });
  }
  async deleteTemplate(
    ...
    await this.db.withTenant(orgId, async (session) => {
      ...
      const deleted = await session.query(
        "DELETE FROM survey_library_templates WHERE org_id=$1 AND owner_id=$2 AND id=$3 RETURNING id",
        [orgId, ownerId, id],
      );
