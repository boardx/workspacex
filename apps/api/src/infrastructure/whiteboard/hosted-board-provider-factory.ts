...
const DATA_CONNECT_TIMEOUT_MS = 5_000;
const DATA_READ_TIMEOUT_MS = 10_000;
const DATA_REQUEST_TIMEOUT_MS = 15_000;
type StreamTimeouts = { wholeRequestMs: number; idleReadMs: number };

type DestroyableAsyncStream = AsyncIterable<unknown> & {
  destroy?(error?: Error): void;
};

async function readTimedStream(
  body: DestroyableAsyncStream,
  input: { expectedSizeBytes: number; maxBytes: number; declaredSize?: number },
  timeouts: StreamTimeouts,
): Promise<Uint8Array> {
  const deadlineError = new Error('object stream deadline exceeded');
  let deadlineTimer: ReturnType<typeof setTimeout> | undefined;
  const deadline = new Promise<never>((_resolve, reject) => {
    deadlineTimer = setTimeout(() => {
      body.destroy?.(deadlineError);
      reject(deadlineError);
    }, timeouts.wholeRequestMs);
    deadlineTimer.unref?.();
  });
  const iterator = body[Symbol.asyncIterator]();
  const read = async (): Promise<Uint8Array> => {
    const limit = Math.min(input.expectedSizeBytes, input.maxBytes);
    if (!Number.isSafeInteger(limit) || limit < 1 || input.declaredSize !== undefined && input.declaredSize !== input.expectedSizeBytes) {
      throw new Error('invalid object size');
    }
    const chunks: Buffer[] = [];
    let size = 0;
    while (true) {
      const idleError = new Error('object stream idle timeout');
      let idleTimer: ReturnType<typeof setTimeout> | undefined;
      const idle = new Promise<never>((_resolve, reject) => {
        idleTimer = setTimeout(() => {
          body.destroy?.(idleError);
          reject(idleError);
        }, timeouts.idleReadMs);
        idleTimer.unref?.();
      });
      let item: IteratorResult<unknown>;
      try { item = await Promise.race([iterator.next(), idle]); }
      finally { if (idleTimer) clearTimeout(idleTimer); }
      if (item.done) break;
      const value = item.value;
      const chunk = Buffer.isBuffer(value) ? value : value instanceof Uint8Array ? Buffer.from(value) : typeof value === 'string' ? Buffer.from(value) : null;
      if (!chunk) throw new Error('invalid object stream');
      size += chunk.byteLength;
      if (size > limit) throw new Error('object stream too large');
      chunks.push(chunk);
    }
    if (size !== input.expectedSizeBytes) throw new Error('invalid object size');
...
type AliSdk = OSS & {
  ...
  getStream(name: string): Promise<{ stream: DestroyableAsyncStream; res: { headers: object } }>;
};

class AliyunOssSdkBoardProtocol implements AliyunOssBoardProtocol {
  constructor(private readonly sdk: AliSdk, private readonly streamTimeouts: StreamTimeouts) {}
  ...
  async get(key: string, expectedSizeBytes: number): Promise<{ content: Buffer; headers: Record<string, string> }> {
    const startedAt = Date.now();
    const deadlineError = new Error('object request deadline exceeded');
    let expired = false;
    let openTimer: ReturnType<typeof setTimeout> | undefined;
    const open = (this.sdk.getStream(key) as Promise<{ stream: DestroyableAsyncStream; res: { headers: object } }>).then(result => {
      if (!result.stream) throw new Error('invalid OSS response');
      if (expired) {
        result.stream.destroy?.(deadlineError);
        throw deadlineError;
      }
      return result;
    });
    const timeout = new Promise<never>((_resolve, reject) => {
      openTimer = setTimeout(() => { expired = true; reject(deadlineError); }, this.streamTimeouts.wholeRequestMs);
      openTimer.unref?.();
    });
    let result: Awaited<ReturnType<AliSdk['getStream']>>;
    try { result = await Promise.race([open, timeout]); }
    finally { if (openTimer) clearTimeout(openTimer); }
    const headers = headersOf(result.res.headers);
    const declared = headers['content-length'];
    const remainingRequestMs = this.streamTimeouts.wholeRequestMs - (Date.now() - startedAt);
    if (remainingRequestMs < 1) {
      result.stream.destroy?.(deadlineError);
      throw deadlineError;
    }
    try {
      const content = await readTimedStream(result.stream, {
        expectedSizeBytes,
        maxBytes: 64 * 1024 * 1024,
        ...(declared === undefined ? {} : { declaredSize: Number(declared) }),
      }, { ...this.streamTimeouts, wholeRequestMs: remainingRequestMs });
      return { content: Buffer.from(content), headers };
    } catch (error) {
      result.stream.destroy?.(error instanceof Error ? error : undefined);
      throw error;
    }
  }
...
class LazyAliyunOssSdkBoardProtocol implements AliyunOssBoardProtocol {
  ...
  constructor(private readonly createSdk: () => Promise<AliSdk>, private readonly streamTimeouts: StreamTimeouts) {}
  private configured(): Promise<AliyunOssSdkBoardProtocol> {
    return this.protocol ??= this.createSdk().then(sdk => new AliyunOssSdkBoardProtocol(sdk, this.streamTimeouts));
  }
  ...
  async get(key: string, expectedSizeBytes: number) { return (await this.configured()).get(key, expectedSizeBytes); }
...
class OfficialS3BoardProtocol implements S3CompatibleBoardProtocol {
  ...
  async getObject(input: { bucket: string; key: string; expectedSizeBytes: number; maxBytes: number }): Promise<{ body: Uint8Array; metadata?: Record<string, string>; contentLength?: number; contentType?: string; lastModified?: Date } | null> {
    try {
      const result = await this.sdk.send(
        new GetObjectCommand({ Bucket: input.bucket, Key: input.key }),
        { abortSignal: AbortSignal.timeout(DATA_REQUEST_TIMEOUT_MS) },
      );
      if (!result.Body) throw new Error('invalid S3 response');
      const body = await boundedBytes(result.Body as AsyncIterable<unknown>, {
        expectedSizeBytes: input.expectedSizeBytes,
        maxBytes: input.maxBytes,
        ...(result.ContentLength === undefined ? {} : { declaredSize: result.ContentLength }),
      });
      return { body, metadata: result.Metadata, contentLength: result.ContentLength, contentType: result.ContentType, lastModified: result.LastModified };
    } catch (error) {
...
export class EnvHostedBoardClientFactory implements HostedBoardClientFactory {
  ...
  async create(input: { provider: 'aliyun-oss' | 's3-compatible'; bucket: string; prefix: string }) {
    if (input.provider === 'aliyun-oss') {
      ...
      const streamTimeouts = this.env.NODE_ENV === 'test'
        ? { wholeRequestMs: 500, idleReadMs: 100 }
        : { wholeRequestMs: DATA_REQUEST_TIMEOUT_MS, idleReadMs: DATA_READ_TIMEOUT_MS };
    ...
    const timeoutScale = this.env.NODE_ENV === 'test' ? 0.025 : 1;
    const sdk = new S3Client({
      endpoint: url.toString(), region, forcePathStyle: true, credentials,
      requestHandler: new NodeHttpHandler({
        connectionTimeout: DATA_CONNECT_TIMEOUT_MS * timeoutScale,
        socketTimeout: DATA_READ_TIMEOUT_MS * timeoutScale,
        requestTimeout: DATA_REQUEST_TIMEOUT_MS * timeoutScale,
      }),
    });
