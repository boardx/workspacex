...
/* ------------------------------------------------------------------------ *
 * 「上会审阅」平台内置 Skill（team1 = 上会材料智能审阅助手，ad-hoc 临时 Agent）
 *
 * ⚠ 本来想单独成文件（临时 Agent 的东西不该和永久基础设施混在一起，删起来干净），
 *   实测不行：`lint-permission-paths.mjs` 的 ALLOWLIST 有一条 ratchet
 *   （`tests/kernel/permission-propagation-six-paths.test.ts` 断言豁免条目总数
 *   ≤ 90），新文件 = 第 91 条，CI 当场判红。那条 ratchet 是对的——豁免清单里
 *   每一条都必须是「过滤器根本用不上」而不是「另起一个文件更顺手」，而这段代码
 *   的豁免理由与本文件其余部分**逐字相同**（无参数、写死 PLATFORM_ORG_ID），
 *   本来就该共用同一条豁免，不该占第二条。
 *
 *   代价是删除这个临时 Agent 时要回到这个文件删掉这一段（连同 main.ts 的一次
 *   调用与 `scripts/ic-review-skill-content.ts`）——`docs/agents/
 *   team1-ic-review-mvp.md` 的「怎么删」一节记了这件事。
 *
 * ## 为什么这是「代码层面」、不受运行时双人审核约束
 *
 * `createSkillDraft` → `submitSkillForReview` → `reviewSkillVersion` 那条运行时
 * 路径审的是「一个用户在界面上临时提交的 skill 该不该被授予能力」
 * （`SELF_REVIEW_FORBIDDEN`/`NO_SECOND_REVIEWER` 防的是这个人自己批准自己）。
 * 这里走的是四个官方 skill 那条路：内容在代码里、随 git PR 走人类 code review、
 * 用 `migrationConfig()`（owner/迁移身份）直接写库、`main.ts` 进程启动时自愈——
 * 审核已经在「这段代码该不该合入仓库」这一步完成了，两道门审的是不同的东西。
 *
 * ## id/版本号单源
 *
