 ...
 */
/**
 * ## 为什么 `已归档` 只能从 `已修复` / `不做` 进入
 *
 * `已归档`（issue #2681）是"把已完成的收起来，避免收件箱越集越长"——它表达的是
 * "这条已经有结论了，不用再占列表位置"，而 `待处理` / `已进入迭代` 都还没有结论。
 * 允许它们直接归档，等于给"还没处理的反馈"开了一条绕过分诊直接消失的路。
 *
 * `已归档` 能回 `待处理`：理由同其它终态的回边——归错了、或者三个月后同一个问题
 * 又被人提起，需要把它重新拉回收件箱。
 */
