...
export function buildRecommendationPrompt(t: TemplateRecommendation): string {
  // ⚠ 这句曾写成「没有依据的分区留空，不要编造」，与 system prompt 里的画布指引
  //   （`canvas-template-guidance.ts`：没聊到的分区**推理补全并标注来源**，不留空白块）
  //   正好相反——同一件事两处相反，小模型听了用户消息这句，遇到新会话就反问
  //   「是否允许推理填充」而不出围栏（Mac 本地版实测 2026-09-17，人类裁决：默认推理填充）。
  //   这里只说「用哪个模板、基于本次对话、没聊到的按指引推理补全」，不复述标记字样。
  return `请基于我们目前这段对话的内容，用「${t.displayName}」（模板 key：${t.key}）`
    + `这个工作坊协作画布模板，产出一份 canvas 围栏。对话里聊过的照实写；`
    + `没聊到的分区不要留空，按画布指引基于背景与常识推理补全并标注来源，`
    + `不要把推测冒充成对话里说过的话。直接产出，不必先向我确认。`;
}
