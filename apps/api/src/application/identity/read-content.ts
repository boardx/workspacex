...
export async function readContent(
  ...
  const guarded = await content.findItemBody(orgId, itemId);
  if (guarded === null) throw new ContentNotFoundError();

  // The body is not a string until a decision says so. `decideContentRead` already ruled
  // above -- this is where that ruling is CONSUMED, through the same doorway every other
  // read path uses (F02's permission-filter). Two mechanisms for one rule is how the one
  // nobody is watching becomes the one that leaks.
  const disclosed = discloseDecided(guarded, { ...baseDecision, allowed: true });
  // Unreachable while allowed:true is passed above -- kept because `discloseDecided` is a
  // shared doorway and a future caller may hand it a denying decision. Narrowing here is
  // what makes the payload access below type-safe rather than asserted.
  if (!isDisclosed(disclosed)) throw new ContentDeniedError(disclosed.reasonCode ?? "NO_PROJECT_ROLE");

  return {
    itemId,
    layer: facts.layer,
    status: facts.status,
    body: disclosed.payload,
    provenanceEventId,
  };
}
