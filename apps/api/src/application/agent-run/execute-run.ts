 ...
 *
 * ## 2026-09-12（issue #3462）补一条「一句话要几张图」
 *
 * 缺陷现场：「画一张流程图，再画一张时间轴」这类**一条消息里明确要多张不同图**的请求，
 * 实测偶发只产出一张。前端 `extractMermaidBlocks`（`packages/fabric-markdown`）本来就按
 * 「文档里有几个 ```mermaid 围栏就切几段」处理，`markdown-message.tsx` 用 `cvs-${i}`/`mmd-${i}`
 * 给每一段配独立 key——**多围栏早已是受支持的既有路径**，不是这次要新开的能力；6 次真实
 * dashscope 采样里有 4 次已经能一次性给两个独立围栏（`## 一、流程图` / `## 二、时间轴`
 * 这类小标题分段），说明模型本身具备这个能力，只是不总是稳定触发。
 *
 * 可疑点在旧版原文这一句的**单数框架**——「输出一个 ```mermaid 围栏代码块」（`一个`）+
 * 「先给一个能渲染的简洁图」（另一个`一个`）——两处都在暗示「这一轮只画一张」，与
 * 「一条消息要两张不同的图」这类请求天然冲突。这不是能确定性复现的 code 层 bug（前端
 * 抽取/渲染早就按多段处理，后端落地/存储也不对围栏数设上限），补一条**显式**的多图规则、
 * 去掉两处「一个」的单数措辞，是针对**已知会诱导单张的措辞**做的加固，而不是宣称把一个
 * 概率性的模型行为改成了确定性修复——如果模型仍然偶发漏画一张，请把新的真实采样贴回
 * issue #3462，不要把这条注释当成「已证明彻底修好」。
 */
export const VISUALIZATION_GUIDANCE = [
  ...
  "**一条消息里可以要求多张不同的图**（例如「画一张流程图，再画一张时间轴」「分别画出 A 和 B 的状态图」）："
    + "这种情况下要对应画出同样数量的图——每张图各自用一个独立的 ```mermaid 围栏代码块，"
    + "前端会把每个围栏都渲染成一张独立、可单独打开的图；不要把多张图硬合并成一张，也不要只画其中一张就结束、"
    + "把另一张放进文字描述里敷衍过去。",
...
async function executeClaimed(
  ...
  const closeOpenToolCalls = (outcome: RunTerminationOutcome) => openToolCalls.closeAll(
    ...
    (toolCallId) => deps.log("open tool call terminal event pending", { runId: run.runId, toolCallId }),
  );
  try {
    ...
    // issue #3403 ④：`classifyModelCallFailureReason` 六条规则全按 `detail` 的**措辞**匹配，
    // 工具卡住那一幕的 detail 是远端超时原话 ⇒ `provider_timeout`，仍然指向模型侧。这里改用
    // **结构事实**：这轮终止时还有已开未闭的工具调用。理由与判据见该枚举值的头注。
    // **不猜**——没有未闭调用时照旧走原分类器，一个字不改。
    const unresolvedTool = openToolCalls.unresolvedToolName;
