 ...
 *
 * ## 刻意不做的两件事（原三件；2026-08-16 人类裁决推翻了第 1 件，见下方新增一节）
 *
 * 1. **不写应用层的「组织已停用」码**。那是 PG RESTRICTIVE 策略拒写（I-P28），
 *    断言在数据库层；在这里补一个 `if` 就是同一条规则的第二处声明。
 * 2. **不发明每段议程环节的时长**。见下方「BP-08」一节。
 *
 * ## Q-4② 已被推翻（2026-08-16 人类裁决）—— 创建者自动获得最高权限
 *
 * 旧判据（本节以下保留原文存档，不是仍然生效）：「不给创建者授予任何项目角色」，
 * 理由是「若创建即授角色，『lead 对自建未加入的项目持管理权、不持内容读取权』那条边
 * 的两端就不存在了」。人类直接裁决：**创建者自动获得该项目最高权限的角色**
 * （`facilitator` + `is_host=true`）——这正是 `ADMIN_NOT_SUPERUSER`（D-18 那条边）
 * 在真实产品里造成的可用性问题（建了项目却进不去自己的项目）的根治：不是绕开
 * D-18，是让创建者从一开始就不落在 D-18 判定的那个"无项目角色的组织层人"格子里。
 *
 * 落地在 `PgProjectRepository.create()`（`infrastructure/project/pg-project-repository.ts`）：
 * 与 `projects`/子类型行**同一个事务**多写一行 `project_memberships`，不给它开
 * 独立端口（`ports.ts` 头注同步更新）。`member-authorization.ts` 里 Q-4② 那条
 * 组织层管理权 override（#614）**不需要跟着删**——它覆盖的是"lead/admin 管理
 * 任意项目成员"这条更宽的边（不限于自建项目），本次推翻只窄化了"创建者对自己
 * 建的项目"这一格的起始状态，不影响那条 override 对其他项目仍然成立。
 *
...
export async function createProject(
  ...
  //   （覆盖 U-4 裁 A 的那一半）。2026-08-16 人类裁决进一步推翻了 Q-4②——下面的
  //   `repo.create` 现在**会**写一行 `project_memberships`（`facilitator`），
  //   admin 建完之后与 lead 建完之后行为仍然一致（都拿到同一种角色），只是
  //   那一致的结果从「都没有角色」变成「都是 facilitator」。
