 ...
 *   3. **建项目会不会自动拥有该项目的项目角色**（Q-4②）——⚠ **2026-08-16 人类裁决推翻**：
 *      现在**会**，由 `PgProjectRepository.create()` 同一事务写入（facilitator），
 *      不经过 `addProjectMember`、不经过本函数。本文件写下时 Q-4② 还是「永远不会」，
 *      下面②这条组织层 override 当时**专为**「创建者自建未加入」这个情形而设——
 *      推翻之后，创建者不再落在这个情形里（他现在有真实的 facilitator 行，走路径①）。
 *      **override 本身不需要删**：它覆盖的是「lead/admin 管理**任意**项目成员」这条
 *      更宽的边（无论是不是自己建的），仍然是别人未加入某个项目时的合法路径——
 *      只是它原本举的例子（创建者自己）不再是它唯一或典型的触发场景了。
 *
