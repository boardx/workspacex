...
export const DESIGN_PRINCIPLES =
  ...
  "⑬一组并列选项（网格/多张卡片）要真的能互相区分：image(kind:illustration) 在这套系统里对每一项画出来都是" +
...
export const DESIGN_OUTLINE_SYSTEM_PROMPT =
  ...
  "比「简洁现代」有用得多）。" +
  // ⚠ accent 是闭集（PrototypeAccent 八档：neutral/blue/violet/teal/green/amber/rose/slate）——
  // 这句里出现的每个颜色词都必须能映回其中一档。写一个不存在的颜色（比如"橙""黄"）比不给建议更糟：
  // 模型答非闭集值会被契约拒，那一轮 accent 直接丢空，比什么都不建议还差。
  "accent 按这个产品**该有的**气质挑，不是按你喜欢什么：记账/银行偏靛蓝或石板灰，健康/环保偏绿，" +
  "美食/零售偏琥珀或玫红，效率工具偏紫或青；心理健康/情绪类偏柔和的青或紫、忌高饱和暖色，" +
  "社交/社区偏活力的玫红或青，教育/儿童偏明快的琥珀或靛蓝；实在拿不准就给 neutral——一个不搭的主色比没有主色更糟。";
