...
import {StandardRunCancelInvocation} from '@repo/contracts/standard-run-cancel';
...
@Controller() export class StandardRunCancelController {
 constructor(@Inject(STANDARD_RUN_CANCEL) private readonly service:StandardRunCancelService,@Inject(STANDARD_ENTRY_SCOPE) private readonly scope:StandardEntryScope){}
 @Public() @Post('/internal/agent-runs/:parentRunId/standard-run-cancel/invoke') @HttpCode(202)
 async invoke(@Headers('x-deep-agent-internal-key') key:string|undefined,@Param('parentRunId') parentRunId:string,@Body() body:unknown){
  ...
  const parsed=StandardRunCancelInvocation.safeParse(body);if(!parsed.success)throw new BadRequestException('standard_entry_invalid');
