/**
 * `POST /auth/register-open` -- protocol adaptation only (open-self-serve-registration
 * delta, issue #1929; supersedes F19's `POST /auth/register`).
 *
 ...
 * means opening one has to be an explicit, greppable act -- and this is one of the two routes
 * in the kernel that need it (the other is `/auth/bootstrap`, untouched by this delta).
 *
 * ⚠ Public here really does mean unauthenticated-and-unrestricted for WHO may call it --
 * that is the entire point of this delta (decision ④: the invite-code gate is removed, only
 * open self-serve registration remains). What is NOT removed is the anti-abuse control that
 * survives past this endpoint: the new account starts unverified and `login.ts` refuses it
 * until the email is confirmed (decision ②/③, `EMAIL_NOT_VERIFIED`).
 */
import {
  Body,
  ConflictException,
...
import { registerNewAccount } from "../../application/auth/register-new-account";
...
export const REGISTER_SCHEMA = C.operations.registerNewAccount.in;
...
type BootstrapBody = RegisterBody;
...
export class AuthRegistrationController {
   ...
   *
   * ## The one contracted failure
   *
   *   409 + `EMAIL_TAKEN`  the address already has an account. Deliberately informative
   *                        (not anti-enumeration): the user is told to sign in instead of
   *                        registering again. Email enumeration is defended at `Login` and
   *                        `RequestPasswordReset`, not here -- there is no invite-code
   *                        search space left to protect (decision ④, issue #1929).
   *
