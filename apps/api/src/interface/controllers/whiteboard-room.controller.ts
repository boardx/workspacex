...
export class WhiteboardRoomController {
  constructor(@Inject(WHITEBOARD_ROOM_REPOSITORY) private readonly rooms: WhiteboardRoomRepository) {}
  ...
  create(@CurrentPrincipal() p: Principal, @Param('boardId',new ParseUUIDPipe()) boardId:string, @Body(new ZodBodyPipe(C.CreatePairing)) input: unknown) {
    assertPrincipal(p); return this.hide(()=>this.rooms.createPairing(p,boardId,C.CreatePairing.parse(input)));
  }
  @Post('/whiteboards/:boardId/room-pairings/:pairingId/status') @HttpCode(200)
  status(@CurrentPrincipal() p:Principal,@Param('boardId',new ParseUUIDPipe()) boardId:string,@Param('pairingId',new ParseUUIDPipe()) pairingId:string){assertPrincipal(p);return this.hide(()=>this.rooms.pairingStatus(p,boardId,pairingId));}
  ...
  join(@Req() req: Request, @Body(new ZodBodyPipe(C.JoinRoom)) input: unknown) {
    ...
    return this.hide(()=>this.rooms.join(C.JoinRoom.parse(input),source));
  }
  ...
  read(@Param('sessionId',new ParseUUIDPipe()) sessionId:string,@Body(new ZodBodyPipe(C.RoomCredential)) input:unknown){
    return this.hide(()=>this.rooms.read(sessionId,C.RoomCredential.parse(input)));
  }
  ...
  publish(@CurrentPrincipal() p:Principal,@Param('boardId',new ParseUUIDPipe()) boardId:string,@Param('sessionId',new ParseUUIDPipe()) sessionId:string,@Body(new ZodBodyPipe(C.PublishViewport)) input:unknown){
    assertPrincipal(p); return this.hide(()=>this.rooms.publishViewport(p,boardId,sessionId,C.PublishViewport.parse(input)));
  }
  ...
  async revoke(@CurrentPrincipal() p:Principal,@Param('boardId',new ParseUUIDPipe()) boardId:string,@Param('sessionId',new ParseUUIDPipe()) sessionId:string){
    assertPrincipal(p); if(!await this.rooms.revoke(p,boardId,sessionId))throw new NotFoundException(); return {ok:true as const};
  }
}
