...
// F11 曾在这里加过一个 `teamOccupancyOf` 结构化异常字段（`TEAM_IN_USE` 的 `blocked` 占用项），
// 服务 `mutateTeam` 的删除阻断响应。issue #2615（组织去掉团队概念）把 `mutateTeam`
// 连同 `TEAM_IN_USE` 一起移除后，这个字段已没有任何路径能产生——同一并删除，
// 不留一段永远返回 `{}` 的死代码。
