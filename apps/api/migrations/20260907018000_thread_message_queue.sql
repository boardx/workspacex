...
CREATE INDEX IF NOT EXISTS thread_message_queue_pending ON thread_message_queue(org_id,thread_id,sequence) WHERE status='pending';
ALTER TABLE thread_message_queue ENABLE ROW LEVEL SECURITY;
ALTER TABLE thread_message_queue FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS thread_message_queue_tenant ON thread_message_queue;
...
CREATE OR REPLACE FUNCTION kernel_message_queue_orgs() RETURNS TABLE(org_id text)
