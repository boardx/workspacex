...
CREATE TABLE IF NOT EXISTS whiteboard_discussion_comments (
 id uuid PRIMARY KEY, org_id text NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS whiteboard_comment_mentions (
 ...
 comment_id uuid NOT NULL REFERENCES whiteboard_discussion_comments(id) ON DELETE CASCADE, user_id text NOT NULL,
...
CREATE TABLE IF NOT EXISTS whiteboard_comment_tasks (
 ...
 source_comment_id uuid NOT NULL REFERENCES whiteboard_discussion_comments(id) ON DELETE RESTRICT,
...
CREATE INDEX IF NOT EXISTS whiteboard_discussion_comments_thread ON whiteboard_discussion_comments(org_id,thread_id,created_at,id);
...
ALTER TABLE whiteboard_discussion_comments ENABLE ROW LEVEL SECURITY; ALTER TABLE whiteboard_discussion_comments FORCE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS whiteboard_discussion_comments_tenant ON whiteboard_discussion_comments; CREATE POLICY whiteboard_discussion_comments_tenant ON whiteboard_discussion_comments USING(org_id=current_setting('app.current_org',true)) WITH CHECK(org_id=current_setting('app.current_org',true));
...
GRANT SELECT,INSERT,UPDATE ON whiteboard_threads,whiteboard_discussion_comments,whiteboard_comment_mentions,whiteboard_comment_tasks TO app_rw;
