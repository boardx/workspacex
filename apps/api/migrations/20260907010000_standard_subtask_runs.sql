...
CREATE UNIQUE INDEX subtask_runs_idempotency_idx ON subtask_runs(org_id,parent_run_id,idempotency_key) WHERE idempotency_key IS NOT NULL;
CREATE INDEX subtask_runs_pending_idx ON subtask_runs(org_id, created_at, id) WHERE status='pending';
CREATE INDEX subtask_runs_parent_idx ON subtask_runs(org_id, parent_run_id, created_at, id);
