...
async function profile({ cpu, net, width, height }) {
...
const desktop = await profile({ width: 1440, height: 900 });
r.note(`desktop  — ${(desktop.bytes / 1024).toFixed(1)} KB, LCP ${desktop.lcp}ms, CLS ${desktop.cls}, frames ${desktop.frames.median}ms median / ${desktop.frames.longPct}% over 33ms`);
r.check(desktop.bytes / 1024 <= BUDGET.transferKb, `transfer ${(desktop.bytes / 1024).toFixed(1)} KB over budget ${BUDGET.transferKb} KB`);
r.check(desktop.lcp <= BUDGET.lcpDesktopMs, `desktop LCP ${desktop.lcp}ms over budget ${BUDGET.lcpDesktopMs}ms`);
r.check(desktop.cls <= BUDGET.cls, `desktop CLS ${desktop.cls} over budget ${BUDGET.cls}`);
r.check(desktop.frames.median <= BUDGET.frameMedianMs, `frame median ${desktop.frames.median}ms over budget ${BUDGET.frameMedianMs}ms`);
r.check(desktop.frames.longPct <= BUDGET.longFramePct, `${desktop.frames.longPct}% of frames over 33ms, budget ${BUDGET.longFramePct}%`);

const slow = await profile({
  cpu: 4, width: 390, height: 844,
  net: { latency: 400, downloadThroughput: 400 * 1024 / 8, uploadThroughput: 400 * 1024 / 8 },
});
r.note(`slow 3G  — LCP ${slow.lcp}ms, CLS ${slow.cls}, frames ${slow.frames.median}ms median`);
r.check(slow.lcp <= BUDGET.lcpSlow3gMs, `slow-3G LCP ${slow.lcp}ms over budget ${BUDGET.lcpSlow3gMs}ms`);
r.check(slow.cls <= BUDGET.cls, `slow-3G CLS ${slow.cls} over budget ${BUDGET.cls}`);
