 ...
 *
 * ## 曾经挂 `test.fixme`：产品缺口 #3028，不是断言写错（**2026-09-08 已解除**）
 *
 * 本条要证的事需要两件同时成立：① 在**种好 30 条填充轮次的那条线程**里跑；
 * ② 这一轮走 `CHAT_READ_E2E.agentId`（loopback-echo）——只有它会回显 L2 的那两个哨兵。
 * 而当时 v2 上这两件事互斥（#3028）：深链进那条线程用的是服务端默认 agent（deep-agent，
 ...
 * `key={selectedAgentId}` 已经去掉，换 agent 发生在同一条线程里、历史不清空。本条
 * 因此从 `test.fixme` 改回 `test`，正文一个字未动（下面那行
 * `selectWorkbenchAgent(...)` 本来就已经写好在这里等它）。
 *
 * ## 边界诚实
 *
...
test("@path:A3 长会话压缩：被挤出 L1 的早期事实真的活着穿过摘要层", async ({ page }) => {
  ...
  // issue #3028（已补上）—— 这一行既留在种好的线程里、又切到回显 agent。
