...
export function analyzeSoak(input: Pick<SoakReport, 'runId'|'exactSha' | 'environment' | 'startedAt' | 'finishedAt' | 'collaborationStartedAt' | 'collaborationFinishedAt' | 'collaborationDurationMs' | 'initialClients' | 'operations' | 'clients' | 'freshClient' | 'server' | 'reconnects' | 'serverLedger' | 'config'>): SoakAnalysis {
  ...
  if(!ledger)evidenceFailures.push('server-signed soak ledger is missing');
    ...
    if(ledger.requiredDurationMs!==input.config.durationMs||ledger.requiredOfflineMs!==input.config.offlineMs||ledger.expectedClients!==input.config.clients||ledger.expectedWriters!==input.config.writers||ledger.expectedReconnects!==Math.min(5,input.config.writers))evidenceFailures.push('server ledger acceptance thresholds do not match report');
    ...
    if(signedInitialConnections.length!==input.config.clients||new Set(signedInitialConnections.map(item=>item.clientNonce)).size!==input.config.clients||signedInitialConnections.some(item=>item.connectedAtMs>ledger.startedAtMs||item.disconnectedAtMs!==null&&item.disconnectedAtMs<=ledger.startedAtMs)||Number.isFinite(collaborationStartedMs)&&(ledger.startedAtMs>collaborationStartedMs||signedInitialConnections.some(item=>item.connectedAtMs>collaborationStartedMs)))evidenceFailures.push('initial connections were not simultaneously live before collaboration started');
    ...
    const reconnectClients=new Set(input.reconnects.map(item=>item.client));
    for(const reconnect of input.reconnects){const oldConnection=ledgerConnections.get(reconnect.previousConnectionId),newConnection=ledgerConnections.get(reconnect.connectionId),signedDisconnectedAt=oldConnection?.disconnectedAtMs;if(!signedDisconnectedAt||signedDisconnectedAt<reconnect.offlineAtMs||signedDisconnectedAt>reconnect.onlineAtMs)evidenceFailures.push(`${reconnect.client} signed disconnect is outside its outage window`);if(!newConnection||newConnection.connectedAtMs<reconnect.onlineAtMs||newConnection.connectedAtMs>(reconnect.recoveredAtMs??Number.NEGATIVE_INFINITY)||signedDisconnectedAt===null||signedDisconnectedAt===undefined||newConnection.connectedAtMs-signedDisconnectedAt<input.config.offlineMs||newConnection.disconnectedAtMs!==null&&newConnection.disconnectedAtMs<ledger.finishedAtMs)evidenceFailures.push(`${reconnect.client} signed reconnect is outside its recovery window`);}
    for(const connection of signedInitialConnections)if(!reconnectClients.has(connection.clientNonce)&&connection.disconnectedAtMs!==null&&connection.disconnectedAtMs<ledger.finishedAtMs)evidenceFailures.push(`${connection.clientNonce} has an unexpected signed mid-run disconnect`);
