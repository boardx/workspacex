...
test.describe.serial("端到端自检：蓝本设计器 16 项全部结构化，真读真写真落库", () => {
  ...
  test("② 挑三项做真实编辑并刷新：值回到原来那一格（真落库），完成度随服务端响应联动", async ({ page }) => {
    ...
    //
    // ⚠ 这里刻意挑「改议程 × 组员」，不挑「改议程 × 组长」——后者只在 F203（把占位
    //   锁定数据换成原型真实 permRows[].lockedFor）之后才是锁的，而 F203 走的是另一条
    //   分支（PR #1529），与本分支血统上还没汇合。「改议程 × 组员」在**两边都是锁的**
    //   （占位版的 LOCKED_CELLS 里有这个 key；F203 版 改议程 的 lockedFor 是
    //   [组长,组员,观察者]），所以这条断言不依赖两个 PR 谁先合。
    //   —— 这不是为了让测试变绿而挑软柿子：第一版就是写的「组长」，在真栈里真红了，
    //      红的原因是血统而不是行为，据实改成两边都成立的那一格并写明缘由。
