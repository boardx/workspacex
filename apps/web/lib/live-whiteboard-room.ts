...
export async function readRoom(sessionId:string,input:z.infer<typeof C.RoomCredential>,signal?:AbortSignal){const op=C.operations.readRoom;return C.RoomState.parse(await apiRequest(path(op.path,{sessionId}),{method:op.method,sessionToken:null,body:C.RoomCredential.parse(input),signal}));}
