 ...
 *     （F965：屏上已删掉这个只弹本地对话框、不产生真实副作用的按钮，见 `tab-results.tsx`）。
 *   · 候选决策（来自转写，待签署）实体 —— 成果沉淀屏有，契约未建模（F965 同上处理）。
 *   · 项目结论（结论文本 + 签字人）—— 成果沉淀屏有，契约未建模（F965 同上）。
 *   · 假设状态（已验证/待验证/已推翻计数）—— 成果沉淀屏有，契约未建模（F965 同上）。
...
/*
 * ─────────────────────── 成果沉淀（原型 isWsAfter） ───────────────────────
 * F965（2026-08-20）：`RESULTS`/`CANDIDATE_DECISIONS`/`AUDIT_TRAIL` 三个 mock 常量已删除
