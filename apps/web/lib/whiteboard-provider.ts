...
export class WhiteboardProvider {
  ...
  private onUpdate = (update: Uint8Array, origin: unknown) => {
    ...
    this.pending.push(message); this.publish({}); this.drain();
  };
  private send(message: WhiteboardClientMessage): boolean {
  ...
  private drain() {
    ...
    for (const message of this.pending) {
      ...
      this.inFlight.add(message.updateId);
    }
  }
  private schedulePresence() {
    ...
    this.presenceTimer = setTimeout(() => {
      ...
      // Awareness has no ACK. Never add it behind durable writes whose server-side
      // processing time is unknown; the newest presence is sent after the queue drains.
  ...
  private connect() {
    ...
    this.ready = false; this.inFlight.clear(); this.publish({ phase: this.epoch ? 'offline' : 'connecting' });
    ...
    socket.onmessage = event => {
      ...
      try {
        ...
        if (message.type === 'sync') {
          ...
          // Only a frame sent on this live socket may be acknowledged. Duplicate or
          // unsolicited ACKs must never remove a queued update that has not been sent.
          if (!this.inFlight.has(message.updateId)) return;
          this.pending = this.pending.filter(item => item.updateId !== message.updateId);
          this.inFlight.delete(message.updateId); this.publish({}); this.drain();
          if (!this.pending.length) this.schedulePresence();
  ...
  close() { if (this.presenceTimer) clearTimeout(this.presenceTimer); this.stopped = true; this.ready = false; this.inFlight.clear(); if (this.timer) clearTimeout(this.timer); if (this.handshake) clearTimeout(this.handshake); clearInterval(this.sessionTimer); this.doc.off('update', this.onUpdate); this.socket?.close(); }
}
