...
export async function createBoardCheckpoint(boardId: string, input: Omit<z.infer<typeof H.CreateCheckpoint>,'requestId'>) { const op=H.operations.createCheckpoint,body=H.CreateCheckpoint.parse({...input,requestId:crypto.randomUUID()}); return op.out.parse(await apiRequest(historyPath(op.path,boardId),{method:op.method,body})); }
...
export async function restoreBoardCheckpoint(boardId: string, checkpointId: string, input: Omit<z.infer<typeof H.RestoreCheckpoint>,'requestId'>) { const op=H.operations.restoreCheckpoint,body=H.RestoreCheckpoint.parse({...input,requestId:crypto.randomUUID()}); return op.out.parse(await apiRequest(historyPath(op.path,boardId,checkpointId),{method:op.method,body})); }
