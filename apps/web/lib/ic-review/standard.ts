 ...
 */
/**
 * 项目类型——同一份清单要同时服务并购与融资两类项目，有些条目只对其中一类成立
 * （issue #3710：融资类不要尽调报告；并购类暂不校验退出机制）。类型「只在这里
 * 声明一次」：判定规则见 PROJECT_TYPE_RULE，条目适用范围见每条的 appliesTo，
 * 正文（review-prompt.ts）只渲染，不另写一套。
 */
export type ProjectType = "并购" | "融资";

export const PROJECT_TYPES: readonly ProjectType[] = ["并购", "融资"];

/**
 * 「本项目是哪一类」的唯一判定规则。写成数据而不是散在正文里，是因为它同时决定
 * 「哪些条目要判」和「判不出来时怎么办」——两处各写一套必然漂移。
 */
export const PROJECT_TYPE_RULE = `本清单同时服务两类项目，先判类型再逐条判定，类型决定哪些条目适用：
- 「并购」：以取得标的控制权为目的（收购股权/资产、控股、吸收合并），材料里常见 SPA/SHA、交割、整合、商誉等表述。
- 「融资」：以投入资金取得少数股权为目的（参股、增资、领投/跟投轮次），材料里常见轮次（A/B/C 轮）、投前投后估值、领投方、老股转让等表述。
判定要求（不许省略这一步）：
- 在任务一的开头用一行写明「项目类型：并购／融资；依据：<材料中可定位的原文>」。
- 「判不出来就明说判不出来」：材料不足以支持任一类型时，第一行写
  「项目类型：无法判定——请确认本项目属于并购还是融资，两类适用的标准条目不同」，
  并「停下来等用户确认」，不要默认按并购或按融资往下判。
  此时可以先做与类型无关的部分，但所有带适用范围标记的条目一律先留空，不要猜。`;
...
export interface StandardItem {
  ...
  /**
   * 该条目对哪些项目类型成立。省略 = 两类都要判。
   * 只在这里声明，正文按它渲染适用范围标记，不在方法论里重复写一遍条件。
   */
  readonly appliesTo?: readonly ProjectType[];
  /** 条目级补充判据（比标题更细的口径约定），会原样渲染进清单。 */
...
export const IC_STANDARD: readonly StandardItem[] = [
  ...
  { id: "IC-2-3", category: "标的价值：为什么选择它", title: "财务表现：收入毛利、净利润、营运资金、债务、现金流 五项分析", blocking: true, note: "五项逐项都要有分析才算满足（n=5）；本条「不考核会计政策相关问题」（收入确认政策、资本化政策、折旧年限等口径问题不在本条判定范围内，也不因此把本条判为缺失）——但它们仍属于任务三的交叉验证范围，发现口径矛盾照常在那里报。" },
  ...
  { id: "IC-3-1", category: "交易合理性：是否值得买", title: "业务尽调：市场调研与上下游头部客户访谈，验证行业天花板与真实市占率", blocking: true, appliesTo: ["并购"] },
  { id: "IC-3-2", category: "交易合理性：是否值得买", title: "财税尽调：盈利质量、资产真实性、表外负债、关联交易，逐项给出解决方法与底稿支撑", blocking: true, appliesTo: ["并购"] },
  { id: "IC-3-3", category: "交易合理性：是否值得买", title: "法务尽调：合规、知识产权权属、核心专利/商标纠纷、重大合同及应对策略", blocking: true, appliesTo: ["并购"] },
  { id: "IC-3-4", category: "交易合理性：是否值得买", title: "人力尽调：核心管理层评分、组织架构与薪酬激励合理性、文化相容性、人才流失风险", blocking: true, appliesTo: ["并购"] },
  { id: "IC-3-5", category: "交易合理性：是否值得买", title: "其他尽调：IT（数据安全/出海合规）、环境（潜在修复责任）等", blocking: false, appliesTo: ["并购"] },
  ...
  { id: "IC-3-9", category: "交易合理性：是否值得买", title: "交易条款：SPA/SHA/战略协同合同/技术转让授权协议等核心条款摘要与预案", blocking: true, note: "涉及对赌／业绩承诺条款时，「投资对赌期一般按三年」判断：材料未写明期限的，按三年提示其应当约定；写明的期限短于或长于三年，标注偏离并写进需核实清单，不因偏离直接判不满足。" },
  { id: "IC-3-10", category: "交易合理性：是否值得买", title: "投资回报：IRR、ROIC、投资回收周期", blocking: true },
  { id: "IC-3-11", category: "交易合理性：是否值得买", title: "退出机制：极端情况下的退出方案", blocking: true, appliesTo: ["融资"] },
...
/** 条目是否适用于某类项目——判定只有这一处实现。 */
export function appliesToProjectType(item: StandardItem, type: ProjectType): boolean {
  return item.appliesTo === undefined || item.appliesTo.includes(type);
}

/** 适用范围标记（两类都适用时无标记，避免给每条都挂一个没有信息量的后缀）。 */
export function projectScopeLabel(item: StandardItem): string {
  return item.appliesTo === undefined ? "" : `（仅${item.appliesTo.join("／")}类适用）`;
}

/** 各项目类型的适用条数 / 阻断条数——总数一律算出来，不手写第二份。 */
export function countByProjectType(type: ProjectType): { total: number; blocking: number } {
  const applicable = IC_STANDARD.filter((i) => appliesToProjectType(i, type));
  return { total: applicable.length, blocking: applicable.filter((i) => i.blocking).length };
}
