...
export default {
   ...
   *
   * Project and personal deep links now share the workbench. Query scope is
   * resolved by CopilotKitV2ShellRoute; legacy URLs remain compatible.
   */
  async redirects() {
    return [
      { source: "/chat/legacy", destination: "/chat", permanent: false },
      {
  ...
  async rewrites() {
    // Keep personal bookmarks canonical; project query links are handled by
    // the same workbench route without losing their project/thread scope.
    const chatV2BranchRewrites = [{
      source: "/chat",
