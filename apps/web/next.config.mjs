...
export default {
  ...
  async rewrites() {
    ...
    const afterFiles = [
      ...
      // UC-17.8 B4.3（PM 设计工作台）：`design-workbench.ts` 挂了裸 `/design-projects`
      // 与 `/design-projects/:projectId` 等——同一个坑的复现，理由同上面 `/inbox` 那条。
      { source: `${prefix}/design-projects`, destination: `${apiOrigin}/design-projects` },
      { source: `${prefix}/design-projects/:path*`, destination: `${apiOrigin}/design-projects/:path*` },
