...
export default defineConfig({
   ...
   *
   * issue #2038（默认 agent 服务端动态解析 + env 配错容错）—— 新增
   * `copilotkit-v2-default-agent.spec.ts` 同样由本 config 接住：同一条真登录、
   * 同一个 Next rewrite（`/copilotkit/:path*`）、同两个已发布 agent 种子。
   * `COPILOTKIT_V2_AGENT_ID` 保持配成有效的 `deepAgentId` 不变——env **有效**时
  ...
  fullyParallel: false,
  /*
   * issue #3047 —— **本 config 的全部 project 都串行跑，一个 worker**。
   *
   * `fullyParallel: false` 只保证**文件内**串行；文件之间仍然按 worker 数并行，而
   * `workers` 此前没设 ⇒ Playwright 按 CPU 推算，CI 上实测是 **2 workers**。两个
   * worker 同时打的是**同一套**真栈：同一个 Postgres 库、同一个 API 进程、同一个
   * 确定性 deep-agent 替身、同一个种子账号。这不是"两条独立的测试"，是两条竞争同
   * 一份可变状态的测试。
   *
   * 反证（不是推断，是别人已经跑过的对照实验）：
   *   · PR #3035（077da8486）的提交信息里记着一次控制变量实验——同样这批文件、同一
   *     份代码，只把并行度改成 `--workers=1`：**19 passed / 0 failed**，此前每轮必红
   *     的 5 条（F65 11.6s、图片附件 12.5s、png 预览 9.2s、G2 画像 29.5s、键盘发消息
   *     5.3s）全部通过，**耗时只有并行那轮的 1/4**。作者当时判定「这是车道自身的并发
   *     缺陷」，按纪律没有在那个 PR 里顺手改。
   *   · 并发缺陷的指纹在 CI 上也对得上：两跑失败集合几乎不重叠，且规模巨大——
   *     run 34155551896（main@938d16bf3）45 passed / **46 failed**；
   *     run 34185270399（main@ef9feca52）60 passed / **24 failed**。
   *     同一个车道两跑差 22 条，方向还相反：这是抽样噪声的形状，不是"某个提交引入
   *     回归"的形状。#3047 最初把它归因到 ef9feca52，而那个提交改的 `chat-path-*`
   *     文件属于另一个 project，在 `--project=chat-read` 这一跑里一行都没被加载。
   *
   * ⚠ 串行不是"为了掩盖红而放宽"：一条断言都没有改，判据逐字不动。串行掉的是
   *   **测试之间互相污染**这个变量。而且按上面的实测，串行大概率**更快**——并行那轮
   *   的时间大头是失败用例各自等满 30s–2.5m 的超时。
   *
   * 谁要改回并行：先把"两个 worker 共享同一份真栈状态"这件事真正解决（每 worker 一
   * 套隔离作用域），再改这一行；只把数字调大等于把 #3047 重新买一遍。
   */
  workers: 1,
  retries: 0,
  /*
