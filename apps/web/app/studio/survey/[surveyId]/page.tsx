...
export default function SurveyWorkflowPage({ params, searchParams }: {
  ...
  searchParams: { step?: string; state?: string; readonly?: string; module?: string };
}) {
  ...
  const state = (["loading", "empty", "error"] as const).includes(searchParams.state as "loading" | "empty" | "error")
    ...
    : "default";
  return <SurveyWorkflowShell surveyId={params.surveyId} initialStep={parsedStep.success ? parsedStep.data : "design"} uiState={state} readonly={searchParams.readonly === "1"} moduleId={searchParams.module} />;
}
