...
it("stores pressure-aware drawing and eraser strokes as vector compositing objects", async () => {
  ...
  expect(readObjects(doc)[0]?.extensionData?.contentObject).toMatchObject({ type: "drawing", strokes: [{ tool: "pen" }, { tool: "eraser" }] });
  doc.destroy();
});
...
it("validates HTTPS image MIME, size, and magic bytes before storing a durable URL reference", async () => {
  vi.stubGlobal("fetch", vi.fn(async () => new Response(Uint8Array.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a]), { status: 206, headers: { "content-type": "image/png", "content-range": "bytes 0-5/2048" } })));
  const doc = await setup();
  ...
  expect(readObjects(doc)[0]?.extensionData?.contentObject).toMatchObject({ type: "image", status: "ready", sourceUrl: "https://assets.example.com/photo.png", mimeType: "image/png" });
  ...
  expect(fetch).toHaveBeenCalledWith("https://assets.example.com/photo.png", expect.objectContaining({ headers: { Range: "bytes=0-511" } }));
  doc.destroy();
});
...
it("instantiates a template as one stable four-object command set", async () => {
  const doc = await setup();
  fireEvent.click(screen.getByTestId("board-add-more"));
  fireEvent.click(screen.getByTestId("board-content-template"));
  const objects = readObjects(doc);
  expect(objects).toHaveLength(4);
  const template = objects.find((object) => (object.extensionData?.contentObject as { type?: string } | undefined)?.type === "template")!;
  expect(objects.filter((object) => object.kind === "sticky").map((object) => object.id).sort()).toEqual([`${template.id}-1`, `${template.id}-2`, `${template.id}-3`]);
  doc.destroy();
});
