 ...
 * 卡片也可以切换为列表」）。
 *
 * #1381 起本屏的列表读真实 `GET /models`（`lib/live-model.ts` 的 `listModels()`），不再是
 * `lib/mock/admin.ts` 的静态清单——本文件因此 mock 那条真实调用，断言的仍然是**渲染行为**
 * 本身：
 ...
 *   · 两种视图下同一条模型的关键字段（供应商/标签/上下文/单价/凭据状态）都在场，
 *     不因为换布局丢字段。
...
/** 已启用、非机密的闭源模型，字段齐全，适合做断言锚点——形状取自真实 `listModelPool.out`。 */
...
describe("admin-model-view-toggle · 卡片/列表切换", () => {
  beforeEach(() => {
    ...
    listModels.mockResolvedValue([SAMPLE_ROW]);
  });

  it("默认是卡片视图：卡片网格在场，列表容器不在场", async () => {
    render(<ModelScreen state="default" />);
    await screen.findByTestId(`admin-model-card-${SAMPLE_MODEL_ID}`);
  ...
  it("点击列表切换按钮 ⇒ 卡片消失、列表出现（真实切换，不是文本空转）", async () => {
    render(<ModelScreen state="default" />);
    await screen.findByTestId(`admin-model-card-${SAMPLE_MODEL_ID}`);
  ...
  it("切回卡片视图 ⇒ 列表消失、卡片重新出现（双向切换都真实生效）", async () => {
    render(<ModelScreen state="default" />);
    await screen.findByTestId(`admin-model-card-${SAMPLE_MODEL_ID}`);
    fireEvent.click(screen.getByTestId("admin-model-view-toggle-list"));
    expect(screen.getByTestId("admin-model-list-hosted")).toBeInTheDocument();
  ...
  it("卡片视图不丢字段：供应商/能力标签/上下文/单价/凭据状态都在场", async () => {
    render(<ModelScreen state="default" />);
    const card = await screen.findByTestId(`admin-model-card-${SAMPLE_MODEL_ID}`);
    ...
    expect(within(card).getByTestId(`admin-model-key-status-${SAMPLE_MODEL_ID}`)).toHaveTextContent("已配置");
  });

  it("卡片视图的启用开关是真实可交互控件：点击后状态徽标从「已启用」变「未启用」", async () => {
    render(<ModelScreen state="default" />);
    const card = await screen.findByTestId(`admin-model-card-${SAMPLE_MODEL_ID}`);
