...
vi.mock('@/lib/live-whiteboard-proposals',()=>({listWhiteboardProposals:vi.fn(),decideWhiteboardProposal:vi.fn()}));
const proposal:Proposal={id:'123e4567-e89b-42d3-a456-426614174000',title:'Reorganize notes',status:'pending',baseEpoch:1,baseSeq:5,commands:[{type:'delete',id:'old_note'}],provenance:{submittedBy:'authorized-person',generator:{label:'claimed assistant',verified:false}},decidedBy:null,createdAt:'2026-09-24T00:00:00.000Z',decidedAt:null,committedEpoch:null,committedSeq:null};
...
describe('AI proposal explicit review panel',()=>{
  it('shows real fetched command summaries and explicitly unverified generator attribution',async()=>{
    render(<ProposalPanel boardId="board" role="owner" online/>);await open();
    expect(screen.getByText('删除对象 old_note')).toBeVisible();expect(screen.getByText(/调用方声明，身份未验证/)).toBeVisible();expect(screen.getByText(/尚未接入自动模型生成/)).toBeVisible();
