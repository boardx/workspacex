...
it("exports real OOXML with Chinese text, tables, superscript citations and source links", async () => {
  ...
  try {
    ...
    expect(xml.indexOf("子项一")).toBeLessThan(xml.indexOf("子项之后")); expect(xml).toContain("政策：需要补充适用范围证据"); expect(xml).toContain("证据代码"); expect(xml).toContain("子项一"); expect(xml).toContain('w:ilvl w:val="1"'); expect(xml).not.toContain("主项子项一子项二"); expect(xml).toContain("欧洲储能报告"); expect(xml).toContain("草稿"); expect(xml).toContain("<w:tbl>"); expect(xml).toContain('w:val="superscript"');
