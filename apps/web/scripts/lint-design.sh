...
# ── U12 已并入 `.harness/scripts/lint-tailwind-color-tokens.mjs`（2026-09-23） ─────────────
# 同一个事实（「颜色类名必须是 tailwind.config.ts 里真的有的 token」）原来有两道门：这里的
# U12 只看 `-foreground`、**逐行**判——一行里有一个合法的 `text-muted-foreground`，同一行上
# 裸的 `hover:text-foreground` 就被 `grep -v` 一起滤掉（全仓 17 处 hover 从来没生效，这道门
# 一处没报）；那一道按 token、跨行、覆盖全部颜色，基线已清零。同一事实不得声明在两处，
# 留下严格的那一道。它接在 `verify:harness:raw` 里；豁免清单 `foreground-legacy-allowlist.txt`
# 在 #3894 已清空，随之删除。
