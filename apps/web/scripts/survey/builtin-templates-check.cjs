...
(async()=>{
 ...
 let region=page.getByRole('region',{name:'内置模板'});await expect(region.getByRole('article')).toHaveCount(12);
 ...
 const downloadWait=page.waitForEvent('download');await page.getByRole('button',{name:'导出 Word',exact:true}).click();const download=await downloadWait;await download.saveAs(out+'/report.docx');if(fs.statSync(out+'/report.docx').size<1000)throw Error('empty Word');
 await page.getByRole('button',{name:'导出 PDF',exact:true}).click();const printFrame=page.frames().find(frame=>frame!==page.mainFrame());if(!printFrame)throw Error('missing print report');await expect(printFrame.locator('body')).toContainText('企业高管');if(/问卷列表|导出 Word|设计问卷/.test(await printFrame.locator('body').innerText()))throw Error('report export contains editor UI');

 await page.getByTestId('survey-section-nav').getByRole('link',{name:'报告模板',exact:true}).click();await expect(page.getByRole('region',{name:'内置模板'}).getByRole('article')).toHaveCount(6);await page.screenshot({path:out+'/report-builtins.png',fullPage:true});
 console.log('PASS: six complete questionnaires, six modules and six report presets visible; builtin edit/save personal copy/reload/source isolation; real survey publish/anonymous response/report with no simulated data or editor instructions.');
