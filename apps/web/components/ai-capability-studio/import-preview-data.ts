...
export function makeImportPreview(input: z.infer<typeof SkillImportRequestSource>): ImportPreviewView {
  ...
  const singleFile = source.kind === "https-file" || (source.kind === "github" && source.selection === "single-file");
  ...
  return SkillImportPreview.parse({
    ...
    candidates: (singleFile ? ["research-brief"] : ["research-brief", "meeting-notes"]).map(name => ({
      candidateId: name, name, manifestPath: "SKILL.md",
      files: [{ path: "SKILL.md", digest, sizeBytes: 180 }, ...(singleFile ? [] : [{ path: "references/style.md", digest, sizeBytes: 60 }])],
