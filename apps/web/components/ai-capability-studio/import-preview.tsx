...
export function CapabilityImportPreview() {
  ...
  const inspect = () => {
    ...
    const input = kind === "zip" ? { kind, uploadId: "demo-upload" } : kind === "https-file" ? { kind, url, authConnectionId: null } :
      { kind: "github", repositoryUrl: url, requestedRef: ref, path: path || null, selection: path ? (path.endsWith("SKILL.md") ? "single-file" : "subdirectory") : "repository-root", authConnectionId: connection === "private" ? "demo-private-connection" : null };
  ...
  const advance = (action: Parameters<typeof advanceImportBatch>[2]) => {
    if (batch && preview) setBatch(advanceImportBatch(batch, preview, action));
  };
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-4 rounded-container border border-border bg-card p-5">
          ...
          <div className="flex flex-wrap gap-2">{[["github", "GitHub"], ["zip", "ZIP 文件"], ["https-file", "HTTPS 单文件"]].map(([value, label]) => <Button key={value} disabled={busy} variant={kind === value ? "secondary" : "outline"} data-testid={`import-kind-${value}`} onClick={() => { setKind(value!); invalidate(); }}>{label}</Button>)}</div>
          {kind !== "zip" ? <><label htmlFor="import-url" className="block text-12">{kind === "github" ? "GitHub仓库地址" : "HTTPS文件地址"}</label><Input id="import-url" data-testid="import-url" disabled={busy} value={url} onChange={event => { setUrl(event.target.value); invalidate(); }} /></> : <><label htmlFor="import-zip" className="block text-12">选择本地ZIP</label><Input id="import-zip" data-testid="import-zip" type="file" accept=".zip" disabled={busy} onChange={event => { setFileName(event.target.files?.[0]?.name ?? ""); invalidate(); }} /><p className="text-12 text-muted-foreground">{fileName || "尚未选择文件"}。正式版本由服务端验证解压大小、路径、符号链接和内容哈希。</p></>}
          {kind === "github" && <><label htmlFor="import-ref" className="block text-12">分支、Tag或Commit</label><Input id="import-ref" data-testid="import-ref" disabled={busy} value={ref} onChange={event => { setRef(event.target.value); invalidate(); }} /><label htmlFor="import-path" className="block text-12">包内目录或SKILL.md路径（空白表示仓库根）</label><Input id="import-path" data-testid="import-path" disabled={busy} value={path} onChange={event => { setPath(event.target.value); invalidate(); }} /><p className="text-12">读取连接（示例）</p><Select data-testid="import-connection" disabled={busy} value={connection} options={[{ value: "public", label: "公开来源，无连接" }, { value: "private", label: "团队只读连接 · 演示" }, { value: "expired", label: "已失效连接 · 演示" }]} onValueChange={value => { setConnection(value); invalidate(); }} /></>}
          ...
          <Button disabled={busy} variant="primary" data-testid="import-inspect" onClick={inspect}><GitBranch className="h-4 w-4" aria-hidden />查看演示预检</Button>
        </section>
        <section className="space-y-4 rounded-container border border-border bg-card p-5">
