...
export const runFailureFixture = RuntimeFailureAttributionRefs.parse({ runId: "run-preview-01", agentId: "research-agent", agentVersionId: "agent-version-1", skillVersionIds: ["skill-version-1"], modelConfigRef: { capabilityModelId: "research-model", configRevision: "1" }, mcpSnapshotRef: { snapshotId: "11111111-1111-4111-8111-111111111111", digest: "a".repeat(64) }, failureCode: "MODEL_CALL_FAILED" });
...
export function RunFailurePreview({ currentModelRevision }: { currentModelRevision: number }) {
  ...
  const refs = withReferences ? runFailureFixture : { ...runFailureFixture, modelConfigRef: null, mcpSnapshotRef: null };
  return <section className="space-y-3 rounded-container border border-border bg-card p-5" data-testid="run-failure-preview">
    ...
    <div className="flex flex-wrap gap-2"><Button variant="ghost" onClick={() => { setAccessible(!accessible); setTarget(null); }}>{accessible ? "模拟运行记录无权限" : "恢复演示访问"}</Button><Button variant="ghost" onClick={() => { setWithReferences(!withReferences); setTarget(null); }}>{withReferences ? "模拟缺少依赖引用" : "恢复演示引用"}</Button></div>
    {!accessible ? <p role="alert" className="text-13">该记录不可访问或不存在。请联系管理员确认访问权限。</p> : <>
      ...
      <Link href={`/preview/ai-capability-studio/workbench?fromRun=${encodeURIComponent(refs.runId)}`} className="inline-block text-13 text-primary">返回 Skill 工作台重新试跑</Link>
    </>}
  </section>;
}
