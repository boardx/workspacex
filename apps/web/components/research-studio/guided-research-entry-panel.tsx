...
export function GuidedResearchEntryPanel({
  ...
  return <section className="space-y-5" data-testid="guided-research-import-panel">
    <div>
      <p className="text-12 font-medium text-primary">步骤 2 · 导入需求</p>
      <h1 className="mt-1 text-24 font-semibold tracking-tight">导入研究需求</h1>
      <p className="mt-2 text-sm text-muted-foreground">通过文本、文件或语音说明研究目标；所有后续 Markdown 都以这份需求为准。</p>
    </div>
    ...
    <div className="rounded-xl border border-border bg-card p-4 sm:p-5">{brief}</div>
    <div className="flex flex-wrap justify-end gap-2"><Button variant="outline" disabled={disabled} onClick={onRegenerate}><Sparkles className="size-4" aria-hidden />重新生成本步骤</Button><Button variant="outline" disabled={disabled} onClick={onSave}>保存草稿</Button><Button variant="primary" aria-label="确认并继续" disabled={disabled} onClick={onContinue}>下一步：确认研究主题 <ArrowRight className="size-4" aria-hidden /></Button></div>
  </section>;
}
