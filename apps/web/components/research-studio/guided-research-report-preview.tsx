...
export function GuidedResearchReportPreview({ state, interrupted = false }: { state: GuidedResearchRuntime; interrupted?: boolean }) {
  ...
  const document = researchReportDocument({ ...preview, sections }, state.sources, state.outline, { provisional: true, aliases: state.reportSourceAliases });
  ...
  return <section className="space-y-5 rounded-xl border border-border bg-card p-5" data-testid="research-report-preview" aria-busy={!failed}>
    ...
    <p className="text-12 text-muted-foreground">已接收 {sections.filter((section) => section.body).length} / {state.outline.filter((section) => section.enabled).length} 个章节的内容</p>
