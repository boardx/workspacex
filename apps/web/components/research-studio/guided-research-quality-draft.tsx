...
export function GuidedResearchQualityDraft({ state }: { state: GuidedResearchRuntime }) {
  ...
  const validationNotice = <aside className="space-y-3 rounded-lg border border-border bg-muted/30 p-4 text-12" aria-label="报告待核验事项">
  ...
  return <section className="space-y-4" data-testid="research-quality-draft">
    {!state.reportDraft && validationNotice}
    {state.reportDraft && <GuidedResearchReportDocument validationNotice={validationNotice} limitations={state.reportPartial || state.reportEvidenceWarnings?.length ? "部分检索或证据核验未成功，报告存在证据缺口，相关结论需进一步核实。" : undefined} document={researchReportDocument(state.reportDraft, state.sources, state.outline)} provisional />}
  </section>;
}
