...
export function GuidedResearchReportDocument({ document, provisional = false, historical = false, idPrefix = "", actions }: { document: ReportDocument; provisional?: boolean; historical?: boolean; idPrefix?: string; limitations?: string; actions?: ReactNode; validationNotice?: ReactNode }) {
  ...
  return <div className="space-y-6"><div className="flex flex-wrap items-center gap-3">{actions && <div className="min-w-0 flex-1">{actions}</div>}{!historical && <details className="relative ml-auto w-fit"><summary className="cursor-pointer rounded-md border border-border bg-card px-4 py-2 text-13 font-medium focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring">导出报告</summary><div className="absolute right-0 z-30 mt-2 flex w-64 flex-wrap gap-2 rounded-lg border border-border bg-card p-3 shadow-lg"><Button variant="outline" disabled={exporting} onClick={() => void word()}>{exporting ? "正在导出…" : "下载 Word"}</Button><Button variant="outline" onClick={() => { if (root.current) { try { printResearchPdf(root.current, document.title || "研究报告"); } catch { setExportError("无法打开打印窗口，请重试。"); } } }}>导出 PDF</Button><span className="w-full text-right text-12 text-muted-foreground">PDF 在打印窗口中选择“另存为 PDF”</span>{exportError && <p role="alert" className="text-12 text-destructive">{exportError}</p>}</div></details>}</div>{historical && <p className="text-12 text-muted-foreground">历史报告</p>}<article ref={root} className="mx-auto grid w-full min-w-0 gap-8 bg-card text-card-foreground xl:grid-cols-[12rem_minmax(0,1fr)] xl:gap-x-10" data-testid={anchorId(provisional ? "research-report-preview-text" : "research-report-document")}>
