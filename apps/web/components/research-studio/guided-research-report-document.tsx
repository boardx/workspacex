...
export function GuidedResearchReportDocument({ document, provisional = false, historical = false, idPrefix = "" }: { document: ReportDocument; provisional?: boolean; historical?: boolean; idPrefix?: string }) {
  ...
  return <article className="mx-auto w-full min-w-0 max-w-4xl space-y-8 rounded-xl border border-border bg-card px-5 py-7 text-card-foreground sm:px-8 sm:py-10" data-testid={anchorId(provisional ? "research-report-preview-text" : "research-report-document")}>
    <header className="space-y-4 border-b border-border pb-6"><p className="text-12 font-medium tracking-wide text-muted-foreground">研究报告{historical ? " · 历史内容（非本轮结果）" : provisional ? " · 草稿" : ""}</p><h2 className="text-24 font-semibold leading-relaxed tracking-tight sm:text-28">{document.title || "研究报告"}</h2>{(provisional || historical) && <p className="text-12 text-muted-foreground">{historical ? "上一轮保存的内容，仅供查看，不属于本轮报告。" : "内容仍在生成或校验中，不代表最终报告。"}</p>}</header>
    ...
    {document.summary && <section id={provisional ? undefined : anchorId("research-report-summary")} className="space-y-3 rounded-lg bg-muted/20 p-4 sm:p-5"><h3 className="text-18 font-semibold">执行摘要</h3><ResearchReportMarkdown text={document.summary} references={document.references} /></section>}
    {document.introduction && <section id={provisional ? undefined : anchorId("research-report-introduction")} className="space-y-3" data-testid="research-report-introduction"><h3 className="text-20 font-semibold">研究范围与方法</h3><ResearchReportMarkdown text={document.introduction} references={document.references} /></section>}
    {document.sections.map((section, index) => <section id={provisional ? undefined : anchorId(`research-report-section-${index}`)} key={`${section.sectionId}-${index}`} className="min-w-0 space-y-4 border-t border-border pt-7" data-testid="research-report-chapter"><h3 className="text-20 font-semibold leading-relaxed">{index + 1}. {section.title}</h3><ResearchReportMarkdown text={section.body} references={document.references} /></section>)}
    {document.conclusion && <section id={provisional ? undefined : anchorId("research-report-conclusion")} className="space-y-3 border-t border-border pt-7" data-testid="research-report-conclusion"><h3 className="text-20 font-semibold">综合结论</h3><ResearchReportMarkdown text={document.conclusion} references={document.references} /></section>}
    {document.references.length > 0 && <section id={provisional ? undefined : anchorId("research-report-references")} className="space-y-3 border-t border-border pt-7" aria-label="参考来源" data-testid="research-report-references"><h3 className="text-18 font-semibold">参考来源{provisional ? " · 待最终校验" : ""}</h3><ol className="list-decimal space-y-3 pl-5 text-12 leading-relaxed">{document.references.map((reference) => <li key={reference.number} id={provisional ? undefined : anchorId(`research-reference-${reference.number}`)}><a href={reference.url} target="_blank" rel="noopener noreferrer" className="break-words text-primary underline underline-offset-2 transition-colors hover:decoration-primary">{reference.title}</a><p className="mt-1 break-all text-muted-foreground">{reference.url}</p></li>)}</ol></section>}
  </article>;
}
