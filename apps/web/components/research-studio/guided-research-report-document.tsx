...
export function GuidedResearchReportDocument({ document, provisional = false, historical = false, idPrefix = "" }: { document: ReportDocument; provisional?: boolean; historical?: boolean; idPrefix?: string; limitations?: string; validationNotice?: ReactNode }) {
  ...
  return <div className="space-y-3">{!historical && <div className="flex flex-wrap justify-end gap-2"><Button variant="outline" disabled={exporting} onClick={() => void word()}>{exporting ? "正在导出…" : "下载 Word"}</Button><Button variant="outline" onClick={() => { if (root.current) { try { printResearchPdf(root.current, `${document.title || "研究报告"}${provisional ? "-草稿" : ""}`); } catch { setExportError("无法打开打印窗口，请重试。"); } } }}>导出 PDF</Button><span className="w-full text-right text-12 text-muted-foreground">PDF 在打印窗口中选择“另存为 PDF”</span>{exportError && <p role="alert" className="text-12 text-destructive">{exportError}</p>}</div>}{(provisional || historical) && <p className="text-12 text-muted-foreground">{historical ? "历史报告" : "草稿"}</p>}<article ref={root} className="mx-auto w-full min-w-0 max-w-4xl space-y-8 rounded-xl border border-border bg-card px-5 py-7 text-card-foreground sm:px-8 sm:py-10" data-testid={anchorId(provisional ? "research-report-preview-text" : "research-report-document")}>
    <header className="border-b border-border pb-6"><h2 className="text-24 font-semibold leading-relaxed tracking-tight sm:text-28">{document.title || "研究报告"}</h2></header>
