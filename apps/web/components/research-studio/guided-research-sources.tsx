    ...
    <div className="flex items-center gap-2 px-4 py-3">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <a className="min-w-0 break-words text-14 font-medium text-primary underline decoration-primary/30 underline-offset-4 transition-colors hover:decoration-primary" href={source.url} target="_blank" rel="noreferrer" aria-describedby={open ? descriptionId : undefined}>{source.presentation?.title ?? source.title}</a>
        ...
        <p className="mt-1 text-12 text-muted-foreground">{new URL(source.url).hostname}</p>
        <p className="mt-2 line-clamp-3 text-12 leading-relaxed">{source.presentation?.summary ?? (source.content.length > 240 ? `${source.content.slice(0, 240)}…` : source.content)}</p>
...
export function GuidedResearchSources({ sources, disabled, onAdd, onRemove }: {
  ...
  return <section className="space-y-3" aria-label="研究来源" data-testid="research-sources">
    ...
    <p className="text-12 text-muted-foreground">与研究主题相关的资料。点击标题查看原文。</p>
