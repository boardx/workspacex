            ...
            <div key={item.id} className="flex min-w-0 items-center gap-2">
              <button
                ...
                className={cn(
                  ...
                  index === current ? "font-medium text-background-foreground" : "text-muted-foreground",
                )}
              >
                <span className={cn(
...
function ResearchHome({ onNavigate }: { onNavigate: (step: GuidedResearchStep, sessionId?: string) => void }) {
  ...
  return <section data-testid="research-home-page" className="mx-auto flex w-full max-w-screen-2xl flex-col gap-6 px-5 py-6 md:px-8 lg:px-10">
    <StudioHistoryHeader business="研究" description="从一个明确的问题开始，拆方向、定大纲、检索资料并生成带引用的完整报告。" count={history?.length} createTestId="research-create" onCreate={() => setCreateOpen(true)} />
    ...
    <section className="space-y-3" data-testid="research-history" aria-label="历史研究">
      ...
      {!loadFailed && visible.length > 0 && <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{visible.map(item => <StudioHistoryCard key={item.sessionId} testId={`research-history-${item.sessionId}`} title={item.title}
        status={<Badge tone={item.status === "completed" ? "primary" : item.stage === "researching" ? "warning" : "neutral"}>{item.status === "completed" ? "已完成" : item.stage === "researching" ? "研究中" : "待继续"}</Badge>}
        description={item.brief.goal} tags={item.tags}
        metadata={<><span>{item.sourceCount} 个来源</span><time>{new Date(item.updatedAt).toLocaleDateString("zh-CN")}</time></>}
        primaryAction={<Button variant="primary" size="sm" onClick={() => onNavigate(item.status === "completed" ? "report" : stageToStep(item.resumeStage), item.sessionId)} data-testid={`${item.status === "completed" ? "research-view" : "research-continue"}-${item.sessionId}`}>{item.status === "completed" ? "查看报告" : "继续研究"}</Button>}
        ...
        <div className="space-y-1.5"><span className="text-10 text-muted-foreground">{item.progress}%</span><Progress value={item.progress} /></div>
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
        ...
        <Card className="h-fit"><CardHeader><CardTitle className="flex items-center gap-2 text-14"><ListTree className="h-4 w-4" aria-hidden />研究计划摘要</CardTitle></CardHeader><CardContent className="space-y-3 text-11 text-muted-foreground"><div className="flex justify-between"><span>报告章节</span><strong className="text-background-foreground">{sections.length}</strong></div><div className="flex justify-between"><span>研究问题</span><strong className="text-background-foreground">{sections.reduce((sum, item) => sum + item.questions.length, 0)}</strong></div><div className="flex justify-between"><span>预计检索</span><strong className="text-background-foreground">20–30 个来源</strong></div><p className="rounded-md bg-muted p-3 text-10 leading-relaxed">开始后系统会按章节并行搜索，过程中可离开页面，稍后从研究首页继续。</p></CardContent></Card>
      </div>
    ...
    >
      ...
      <div className="space-y-4" data-testid="research-report" data-layout="full-width-report">
        <Card className="h-fit">
          ...
          <CardContent className="flex flex-wrap gap-2">{outline.map((item, index) => <a key={item.id} href={`#report-${index}`} className="rounded-md border border-border px-3 py-2 text-11 text-muted-foreground transition-colors hover:bg-muted hover:text-background-foreground">{index + 1}. {item.title}</a>)}</CardContent>
        </Card>
