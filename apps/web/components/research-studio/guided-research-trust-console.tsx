...
export function GuidedResearchTrustConsole({ runtime, pending, onSteer, onResolveConflict, compact = false }: { runtime: TrustRuntime; pending: boolean; onSteer: (action: "pause" | "resume") => void; onResolveConflict: (decision: { conflictId: string; action: "retain_uncertainty" | "prefer_source"; sourceId?: string; rationale: string }) => void; compact?: boolean }) {
  ...
  return <section aria-label="可信研究控制台">
    {compact && <div data-testid="research-steering-controls" className="mb-3 flex gap-2"><Button size="sm" variant="outline" disabled={pending || runtime.controlStatus === "paused"} onClick={() => onSteer("pause")}>暂停研究</Button><Button size="sm" variant="outline" disabled={pending || runtime.controlStatus !== "paused"} onClick={() => onSteer("resume")}>继续研究</Button></div>}
    ...
    <Card className={panel === "activity" ? "block" : compact ? "hidden 2xl:block" : "hidden md:block"}><CardHeader><CardTitle className="text-base">实时活动</CardTitle></CardHeader><CardContent className="space-y-3">
      {!compact && <div data-testid="research-steering-controls" className="flex gap-2"><Button size="sm" variant="outline" disabled={pending || runtime.controlStatus === "paused"} onClick={() => onSteer("pause")}>暂停研究</Button><Button size="sm" variant="outline" disabled={pending || runtime.controlStatus !== "paused"} onClick={() => onSteer("resume")}>继续研究</Button></div>}
