...
export function GuidedResearchTrustConsole({ runtime, pending, onSteer, onResolveConflict }: { runtime: TrustRuntime; pending: boolean; onSteer: (action: "pause" | "resume") => void; onResolveConflict: (decision: { conflictId: string; action: "retain_uncertainty" | "prefer_source"; sourceId?: string; rationale: string }) => void }) {
  ...
  return <section aria-label="可信研究控制台">
    <div className="mb-3 grid grid-cols-3 gap-2 md:hidden" role="tablist" aria-label="可信研究视图">{([['coverage', '覆盖'], ['activity', '活动'], ['evidence', '证据']] as const).map(([value, label]) => <Button key={value} size="sm" variant={panel === value ? "primary" : "outline"} role="tab" aria-selected={panel === value} onClick={() => setPanel(value)}>{label}</Button>)}</div>
    <div className="grid gap-4 md:grid-cols-3">
    <Card className={panel === "coverage" ? "block" : "hidden md:block"}><CardHeader><CardTitle className="text-base">章节覆盖</CardTitle></CardHeader><CardContent data-testid="research-coverage-matrix" className="space-y-2">
    ...
    <Card className={panel === "activity" ? "block" : "hidden md:block"}><CardHeader><CardTitle className="text-base">实时活动</CardTitle></CardHeader><CardContent className="space-y-3">
