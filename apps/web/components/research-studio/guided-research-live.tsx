...
const steps = ["brief", "directions", "outline", "research", "report"] as const;
const labels = { brief: "确认主题", directions: "研究方向", outline: "报告大纲", research: "资料研究", report: "研究报告" };
...
export function GuidedResearchLive({ sessionId, onBack, initialNode }: { sessionId: string; onBack: () => void; initialNode?: Command["node"] }) {
  ...
  async function run(action: Command["action"], extra: Partial<Command> = {}) {
    ...
    try {
      ...
      setState(next);
      const target = next !== received || action === "confirm" || action === "complete" || action === "apply" ? next.currentNode : node;
  ...
  return <div className="max-w-none space-y-4" data-layout="signed-desktop" data-testid={`research-flow-${node === "research" ? "search" : node}`}>
    ...
    <GuidedResearchStepLayout assistant={<section className="flex h-full min-h-0 flex-col rounded-lg border border-border bg-card p-4" data-testid="research-skill-assistant">
      ...
      <div className="flex gap-2"><Input aria-label="研究对话" value={message} disabled={busy} onChange={(event) => setMessage(event.target.value)} data-testid="research-skill-input" placeholder="讨论研究目标或修改建议…" /><Button disabled={busy || !message.trim()} onClick={() => void run("message", { message, ...(draft ? { draft } : {}) })} aria-label="发送研究消息"><Send className="size-4" aria-hidden /></Button></div>
    </section>}>
      <div className="space-y-4">
        ...
        {node === "research" && <>
          <div className="flex gap-2"><Button disabled={busy} onClick={() => void run("start")}>开始真实检索</Button><Button variant="outline" disabled={busy || !state.tasks.some((task) => task.status === "failed" || (expired && task.status === "running"))} onClick={() => void run("retry")}>重试失败任务</Button></div>
          ...
          <div className="space-y-2">{state.tasks.map((task) => <Card key={task.id}><CardContent className="p-3 text-12"><p>{task.query}</p><p className="mt-1 text-muted-foreground">{{ pending: "等待检索", running: "正在检索", succeeded: "已完成", failed: "检索失败" }[task.status]} · 尝试 {task.attempts} 次</p>{task.errorCode && <p className="mt-1 text-destructive">{errors[task.errorCode] ?? "任务执行失败，请重试。"}</p>}</CardContent></Card>)}</div>
        ...
        <div className="flex justify-end gap-2"><Button variant="outline" disabled={busy || !validDraft || node === "report"} onClick={() => draft && void run("save", { draft })}>保存草稿</Button><Button disabled={busy || !validDraft || !state.generatedNodes.includes(node) || state.currentNode !== node || state.completed} onClick={() => void run(node === "report" || node === "research" ? "complete" : "confirm", draft ? { draft } : {})}>{node === "report" ? "完成研究" : "确认并继续"}</Button></div>
      </div>
