...
import {
  BLUEPRINTS, BLUEPRINT_STATS, configTotal, hasEnoughSatisfactionSamples,
  type BlueprintRow,
} from "@/lib/mock/tpl";
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-4 p-4">
      <header className="flex flex-wrap items-end justify-between gap-2">
        <div>
          <h1 className="text-20 font-bold tracking-tight">项目蓝本</h1>
        ...
        <Button size="sm" variant="primary" onClick={() => onToast("新建蓝本三选一：从空白 / 套用已有 / 从历史项目反向生成")} data-testid="tpl-new-blueprint">
          <Sparkles aria-hidden className="h-3.5 w-3.5" /> 新建蓝本
        </Button>
      </header>
      ...
      >
        <ul className="flex flex-col gap-2" data-testid="tpl-blueprint-list">
          {BLUEPRINTS.map((b) => (
            <BlueprintRowCard key={b.id} row={b} total={total} onToast={onToast} onDanger={(action) => setPending({ row: b, action })} />
          ))}
        </ul>
        ...
        <footer className="mt-2 flex flex-col gap-1.5 rounded-lg border border-dashed border-border p-3" data-testid="tpl-new-entries">
          <span className="text-11 font-medium text-muted-foreground">新建蓝本三入口</span>
...
function BlueprintRowCard({
  ...
  return (
    <li className="flex flex-col gap-2 rounded-lg border border-border bg-card p-3 transition-colors duration-200 hover:border-primary/30" data-testid="tpl-blueprint-row">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-11 text-muted-foreground">
