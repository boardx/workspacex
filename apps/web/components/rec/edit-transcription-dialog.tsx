  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Portal>
    ...
    <Dialog.Content data-testid="rec-edit-dialog" className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border border-border bg-card p-7 shadow-lg">
      ...
      <form className="mt-6 flex flex-col gap-5" onSubmit={submit}>
        ...
        <div className="flex flex-col gap-2"><Label htmlFor="rec-edit-tags">标签（可选）</Label><div className="flex min-h-14 flex-wrap gap-2 rounded-md border p-2">{tags.map((tag) => <Badge key={tag} tone="neutral" className="gap-1">{tag}<button type="button" aria-label={`移除标签 ${tag}`} onClick={() => setTags((current) => current.filter((item) => item !== tag))}><X className="h-3 w-3" /></button></Badge>)}<Input id="rec-edit-tags" data-testid="rec-edit-tags" value={tagDraft} disabled={tags.length >= 5} maxLength={20} className="h-8 min-w-32 flex-1 border-0" onChange={(e) => setTagDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addTag(); } }} /></div></div>
