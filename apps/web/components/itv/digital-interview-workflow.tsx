...
function ExpertDetailDialog({ expert, open, onOpenChange }: { readonly expert: DigitalExpertCatalogRow | undefined; readonly open: boolean; readonly onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent data-testid="itv-expert-detail-dialog" closeTestId="itv-expert-detail-close" className="max-w-xl"><DialogHeader><DialogTitle>{expert?.displayName ?? "专家详情"}</DialogTitle><DialogDescription>{staticExpert ? "静态专家档案" : "模型生成专家快照"}</DialogDescription></DialogHeader>{expert && <div className="space-y-4 text-sm"><DetailField testId="itv-expert-detail-role" label="角色" value={expert.role} /><div data-testid="itv-expert-detail-domains"><p className="text-xs font-medium text-muted-foreground">领域</p><div className="mt-2 flex flex-wrap gap-2">{expert.domains.map((domain) => <span key={domain} className="rounded-full bg-muted px-2.5 py-1 text-xs">{domain}</span>)}</div></div>{staticExpert?.location && <DetailField testId="itv-expert-detail-location" label="地区" value={staticExpert.location} />}{staticExpert?.bio && <DetailField testId="itv-expert-detail-bio" label="简介" value={staticExpert.bio} />}{staticExpert?.typicalAdvice && <DetailField testId="itv-expert-detail-advice" label="典型建议" value={staticExpert.typicalAdvice} />}<DetailField testId="itv-expert-detail-boundary" label="材料边界" value={expert.materialBoundary} /></div>}</DialogContent></Dialog>;
}
