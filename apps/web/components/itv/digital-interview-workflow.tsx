...
function QuestionStep({ draft, onChange, onAdd, onDelete, onConfirm }: { draft: MockDigitalInterviewDraft; onChange: (questionId: string, text: string) => void; onAdd: (expertId: string) => void; onDelete: (questionId: string) => void; onConfirm: () => void }) {
  return <div><h2 className="text-xl font-semibold">确认针对性问题</h2><p className="mt-2 text-sm text-muted-foreground">每位专家默认 3 个问题，你可以逐条编辑、删除并继续补充追问。</p><div className="mt-5 space-y-5">{draft.selectedExpertIds.map((expertId) => { const expert = findMockDigitalExpert(expertId); const questions = draft.questions.filter((question) => question.expertId === expertId); return <section data-testid="itv-question-group" key={expertId} className="rounded-xl border border-border p-5"><header><h3 className="font-semibold">{expert?.displayName}</h3><p className="mt-1 text-xs text-muted-foreground">{expert?.role} · {questions.length} 个问题</p></header><div className="mt-4 space-y-4">{questions.map((question, index) => <div key={question.questionId} className="rounded-lg bg-muted/35 p-4"><div className="mb-2 flex items-center justify-between gap-3"><label htmlFor={question.questionId} className="text-xs font-medium">问题 {index + 1} · {question.purpose}</label><button data-testid="itv-delete-question" type="button" aria-label={`删除${expert?.displayName}的问题 ${index + 1}`} onClick={() => onDelete(question.questionId)} className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"><Trash2 className="size-4" /></button></div><textarea id={question.questionId} data-testid="itv-question-input" value={question.text} onChange={(event) => onChange(question.questionId, event.target.value)} placeholder="输入针对这位专家的问题" className="min-h-24 w-full rounded-lg border border-input bg-background p-3 text-sm" /></div>)}</div><Button data-testid="itv-add-question" type="button" variant="outline" className="mt-4" onClick={() => onAdd(expertId)}><Plus className="size-4" />添加问题</Button></section>; })}</div><Button data-testid="itv-confirm-questions" className="mt-5" variant="primary" disabled={!draft.questions.length || draft.questions.some((question) => !question.text.trim())} onClick={onConfirm}>确认问题并进入访谈</Button></div>;
}
