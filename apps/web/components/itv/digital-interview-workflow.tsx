...
export function PersistentDigitalInterviewWorkflow({ initialView }: { readonly initialView: DigitalInterviewWorkflowView }) {
  ...
  function requestReportGeneration() { requestConfirmation("report"); }
...
function LiveQuestionStep({ expertIds, candidates, questions, learningGoals, onChange, onConfirm }: { readonly expertIds: readonly string[]; readonly candidates: readonly DigitalExpertCatalogRow[]; readonly questions: readonly DigitalInterviewQuestion[]; readonly learningGoals: readonly DigitalInterviewResearchBrief["learningGoals"][number][]; readonly onChange: (questions: readonly DigitalInterviewQuestion[]) => void; readonly onConfirm: () => void }) {
  const addQuestion = (expertId: string) => onChange([...questions, { questionId: `manual-${crypto.randomUUID()}`, expertId, order: questions.length + 1, text: "", purpose: "手动问题", section: "core", goalIds: learningGoals.slice(0, 1).map((goal) => goal.goalId) }]);
  return <div><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-medium text-primary">专家提纲</p><h2 className="mt-1 text-2xl font-semibold">让每位专家回答他们最擅长的问题</h2><p className="mt-2 text-sm text-muted-foreground">编辑后的问题会被写入「访谈提纲.md」；未确认的改动不会覆盖已确认版本。</p></div><span className="rounded-full bg-muted px-3 py-1.5 text-xs text-muted-foreground">{questions.length} 个问题</span></div><div className="mt-6 grid gap-5 lg:grid-cols-[13rem_minmax(0,1fr)]"><aside className="rounded-xl border border-border bg-background p-4"><p className="text-xs font-medium text-muted-foreground">已选专家</p><div className="mt-3 space-y-2">{expertIds.map((expertId) => <div key={expertId} className="rounded-lg bg-muted p-3 text-sm font-medium">{candidates.find((candidate) => candidate.expertId === expertId)?.displayName ?? expertId}</div>)}</div></aside><div className="space-y-4">{expertIds.map((expertId) => <section data-testid="itv-question-group" key={expertId} className="rounded-xl border border-border p-4"><h3 className="font-semibold">{candidates.find((candidate) => candidate.expertId === expertId)?.displayName ?? expertId}</h3><p className="mt-1 text-xs text-muted-foreground">围绕职责、证据和反例组织追问。</p>{questions.filter((question) => question.expertId === expertId).map((question, index) => <div key={question.questionId} className="mt-3 rounded-lg bg-muted/50 p-3"><label htmlFor={question.questionId} className="text-xs font-medium text-muted-foreground">问题 {index + 1} · {question.purpose}</label><textarea id={question.questionId} rows={2} data-testid="itv-question-input" value={question.text} onChange={(event) => onChange(questions.map((candidate) => candidate.questionId === question.questionId ? { ...candidate, text: event.target.value } : candidate))} className="mt-2 w-full resize-y rounded-lg border border-input bg-background px-3 py-2 text-sm leading-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" /><div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-xs text-muted-foreground">章节<select data-testid="itv-question-section" value={question.section} onChange={(event) => onChange(questions.map((candidate) => candidate.questionId === question.questionId ? { ...candidate, section: event.target.value as DigitalInterviewQuestion["section"] } : candidate))} className="rounded-md border border-input bg-background px-2 py-1.5 text-sm text-foreground"><option value="warmup">开场</option><option value="core">核心</option><option value="counterexample">反例</option><option value="closing">收尾</option></select></label><fieldset><legend className="text-xs text-muted-foreground">覆盖目标</legend><div className="mt-1 flex flex-wrap gap-2">{learningGoals.map((goal) => <label key={goal.goalId} className="inline-flex items-center gap-1 text-xs"><input data-testid="itv-question-goal" type="checkbox" checked={question.goalIds.includes(goal.goalId)} onChange={() => onChange(questions.map((candidate) => candidate.questionId === question.questionId ? { ...candidate, goalIds: candidate.goalIds.includes(goal.goalId) ? candidate.goalIds.filter((id) => id !== goal.goalId) : [...candidate.goalIds, goal.goalId] } : candidate))} />{goal.statement}</label>)}</div></fieldset></div></div>)}<Button type="button" variant="outline" size="sm" className="mt-3" onClick={() => addQuestion(expertId)}><Plus className="size-4" aria-hidden />添加追问</Button></section>)}</div></div><div className="mt-6 flex justify-end"><Button data-testid="itv-confirm-questions" variant="primary" disabled={!questions.length || questions.some((question) => !question.text.trim() || question.goalIds.length === 0)} onClick={onConfirm}>确认提纲并开始访谈</Button></div></div>;
}
