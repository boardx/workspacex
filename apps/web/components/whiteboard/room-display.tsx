...
export function RoomDisplay(){
  ...
  const connect=useCallback(async()=>{setError('');try{const decoded=JSON.parse(payload) as {orgId?:unknown;pairingId?:unknown;code?:unknown};const next=await joinRoom(decoded as never);sessionStorage.setItem(storageKey(next.sessionId),JSON.stringify(next));setGrant(next);}catch{setError('配对载荷无效、已过期或已经使用。请让主持人重新创建。');}},[payload]);
  useEffect(()=>{if(!grant)return;let active=true;const poll=async()=>{try{const next=await readRoom(grant.sessionId,{orgId:grant.orgId,token:grant.token});if(!active)return;if(next.snapshot)Y.applyUpdate(docRef.current!,bytes(next.snapshot),'room-snapshot');setState(next);setError('');}catch{if(active){setError('会议室连接已过期或被主持人断开。');setGrant(null);setState(null);sessionStorage.removeItem(storageKey(grant.sessionId));}}};void poll();const timer=window.setInterval(()=>void poll(),1500);return()=>{active=false;window.clearInterval(timer);};},[grant]);
