...
export function WhiteboardRenderer({ objects, selected, activeId = null, viewport, onPointerDown }: { objects: WhiteboardObject[]; selected: string[]; activeId?: string | null; viewport: WhiteboardViewport; onPointerDown: (event: PointerEvent, id: string) => void }) {
  const byId = new Map(objects.map(object => [object.id, object]));
  const retained = new Set(selected); if (activeId) retained.add(activeId);
  const rendered = selectVisibleObjects(objects, viewport, retained);
  return <>{[...rendered].sort((a, b) => Number(['frame','group'].includes(b.kind)) - Number(['frame','group'].includes(a.kind))).map(o => {
    ...
    if (o.kind === 'connector') {
      ...
      if (!a || !b) return null;
      return <svg key={o.id} className="pointer-events-none absolute inset-0 h-full w-full overflow-visible" role="img" aria-label={`连接线：${a.text || a.kind} 到 ${b.text || b.kind}`}><line x1={a.geometry.x + a.geometry.width / 2} y1={a.geometry.y + a.geometry.height / 2} x2={b.geometry.x + b.geometry.width / 2} y2={b.geometry.y + b.geometry.height / 2} stroke="currentColor" strokeWidth="2" /></svg>;
    }
