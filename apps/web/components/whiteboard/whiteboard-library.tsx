...
export function WhiteboardLibrary() {
  ...
  return <section data-testid="whiteboard-library" className="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-8">
    <header className="flex flex-wrap items-center gap-3"><div className="flex-1"><h1 className="text-24 font-semibold">Board</h1><p className="text-14 text-muted-foreground">组织中的白板，默认仅创建者可见。</p></div><Link data-testid="board-room-entry" href="/studio/board/room" className="text-14 underline">会议室大屏</Link><Link href="/projects" className="text-14 underline">返回工作区</Link><Button data-testid="board-refresh" disabled={busy} onClick={() => void refresh()}>刷新列表</Button></header>
    ...
    {selected && <section data-testid="board-detail" className="space-y-4 rounded-container border border-border bg-card p-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-20 font-semibold">{selected.name}</h2><Button asChild><Link data-testid="board-open-live" href={`/studio/board/${selected.id}`}>打开全屏白板</Link></Button></div><p className="break-all text-12 text-muted-foreground">白板 ID：{selected.id}</p><p className="text-13">资源信息已从服务端读取。此页面尚未提供画布编辑。</p>
