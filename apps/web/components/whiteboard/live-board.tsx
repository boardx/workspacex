...
export function LiveBoard({ boardId }: { boardId: string }) {
  ...
  const roomSessionRef=useRef<{id:string;epoch:number}|null>(null),roomSessionEpoch=useRef(0),viewportOperation=useRef(0),viewportTimer=useRef<number|null>(null),mounted=useRef(true);
  const setActiveRoom=useCallback((value:string|null)=>{viewportOperation.current+=1;if(viewportTimer.current){window.clearTimeout(viewportTimer.current);viewportTimer.current=null;}if(value===null){roomSessionRef.current=null;setRoomSession(null);return;}roomSessionRef.current={id:value,epoch:++roomSessionEpoch.current};setRoomSession(value);},[]);
  ...
  const viewport=useCallback((value:{x:number;y:number;zoom:number})=>{const active=roomSessionRef.current;if(!active||!presenterScope)return;const operation=++viewportOperation.current;if(viewportTimer.current)window.clearTimeout(viewportTimer.current);viewportTimer.current=window.setTimeout(()=>{viewportTimer.current=null;if(!mounted.current||viewportOperation.current!==operation||roomSessionRef.current!==active||presenterScopeRef.current!==presenterScope)return;void publishRoomViewport(boardId,active.id,value).catch(cause=>{if(!mounted.current||viewportOperation.current!==operation||roomSessionRef.current!==active||presenterScopeRef.current!==presenterScope)return;if(isAuthoritativeRoomEnd(cause)){clearPresenterSession(presenterScope,active.id);setActiveRoom(null);}});},120);},[boardId,presenterScope,setActiveRoom]);
  useEffect(()=>{mounted.current=true;return()=>{mounted.current=false;viewportOperation.current+=1;if(viewportTimer.current){window.clearTimeout(viewportTimer.current);viewportTimer.current=null;}roomSessionRef.current=null;};},[]);
  useEffect(() => {
