...
import { batchDecideWhiteboardProposals,decideWhiteboardProposal,listWhiteboardProposals } from '@/lib/live-whiteboard-proposals';
...
const decisionLabel:Record<Proposal['commandDecisions'][number]['status'],string>={pending:'待处理',applied:'已接受',rejected:'已拒绝'};
...
/** Review surface only: it displays persisted provenance and never synthesizes or auto-accepts AI output. */
export function ProposalPanel({boardId,role,online,pendingChanges=false}:ProposalPanelProps){
  ...
  useEffect(()=>{epoch.current++;setItems([]);setSelected({});setLoaded(false);setError('');setNotice('');requestIds.current.clear();},[boardId]);
  ...
  const decide=async(proposal:Proposal,action:'accept'|'reject')=>{
    ...
    try{const result=await decideWhiteboardProposal(boardId,proposal.id,action,requestId);if(token!==epoch.current)return;
      ...
      setNotice(result.status==='conflicted'?'白板已发生变化，此建议未应用。请基于最新白板重新提交建议。':action==='accept'?'建议已应用到白板。':'建议已拒绝，白板内容未改变。');
  ...
  const toggle=(proposalId:string,index:number)=>setSelected(current=>{const indexes=new Set(current[proposalId]??[]);if(indexes.has(index))indexes.delete(index);else indexes.add(index);return{...current,[proposalId]:[...indexes].sort((a,b)=>a-b)};});
  const selections=items.flatMap(item=>{const commandIndexes=(selected[item.id]??[]).filter(index=>item.commandDecisions[index]?.status==='pending');return commandIndexes.length?[{proposalId:item.id,commandIndexes}]:[];});
  const decideBatch=async(action:'accept'|'reject')=>{
    if(locked.current||!writable||!selections.length)return;
    const signature=JSON.stringify([action,selections]);let requestId=requestIds.current.get(signature);if(!requestId){requestId=crypto.randomUUID();requestIds.current.set(signature,requestId);}
    locked.current=true;mutation.current++;const token=epoch.current;setBusy('batch');setError('');setNotice('');
    try{const result=await batchDecideWhiteboardProposals(boardId,{requestId,action,selections});if(token!==epoch.current)return;
      const changed=new Map(result.proposals.map(item=>[item.id,item]));setItems(values=>values.map(item=>changed.get(item.id)??item));setSelected({});requestIds.current.delete(signature);
      setNotice(action==='accept'?'所选修改已在一个事务中应用到白板。':'所选修改已在一个事务中拒绝，白板内容未改变。');
    }catch(e){if(token===epoch.current)setError(e instanceof Error?e.message:'批量决定失败，请手动重试');}
    finally{locked.current=false;if(token===epoch.current)setBusy(null);}
  };
  return <aside aria-label="AI 建议" className="w-80 max-w-full rounded-xl border bg-background p-3 text-foreground shadow-sm">
    ...
    {open&&<div className="mt-3 max-h-[65vh] space-y-3 overflow-y-auto">
      <p className="text-sm text-muted-foreground">这里展示人类、API 与已验证 AI 运行提交的建议。可逐项选择，并将多个建议作为一个原子批次接受或拒绝。</p>
      ...
      {role!=='viewer'&&<div className="flex gap-2"><Button size="sm" disabled={!writable||busy!==null||!selections.length} onClick={()=>void decideBatch('accept')}>批量接受所选</Button><Button size="sm" variant="outline" disabled={!writable||busy!==null||!selections.length} onClick={()=>void decideBatch('reject')}>批量拒绝所选</Button></div>}
      ...
      {items.map(item=><section key={item.id} className="space-y-2 rounded border p-2" aria-label={`建议：${item.title}`}>
        <p className="font-medium">{item.title} · {item.status==='pending'&&item.commandDecisions.some(value=>value.status!=='pending')?'部分已处理':statusLabel[item.status]}</p>
        <p className="text-sm text-muted-foreground">提交者：{item.provenance.submittedBy}</p>
        {item.provenance.participant.kind==='agent'
          ? <div className="text-sm text-muted-foreground"><p>AI 参与者：{item.provenance.participant.agentName}（已验证）</p><p>Agent {item.provenance.participant.agentId} · Run {item.provenance.participant.runId}</p><p>{item.provenance.participant.provider} / {item.provenance.participant.model} · 版本 {item.provenance.participant.modelVersion}</p></div>
          : <p className="text-sm text-muted-foreground">人类/API 提案 · 已认证操作者：{item.provenance.participant.actorId}</p>}
        <p className="text-sm text-muted-foreground">基于版本 {item.baseEpoch}/{item.baseSeq}</p>
        <ol className="space-y-1 text-sm">{item.commands.map((command,index)=><li key={index} className="flex items-start gap-2 whitespace-pre-wrap break-words"><input type="checkbox" aria-label={`选择「${item.title}」第 ${index+1} 项`} checked={(selected[item.id]??[]).includes(index)} disabled={!writable||busy!==null||item.commandDecisions[index]?.status!=='pending'} onChange={()=>toggle(item.id,index)}/><span>{index+1}. {commandSummary(command)}（{decisionLabel[item.commandDecisions[index]!.status]}）</span></li>)}</ol>
        {item.status==='conflicted'&&<p className="text-sm text-destructive">版本已变化，未覆盖现有内容；需要重新生成并审阅建议。</p>}
        {item.status==='pending'&&item.commandDecisions.every(value=>value.status==='pending')&&role!=='viewer'&&<div className="flex gap-2"><Button size="sm" disabled={!writable||busy!==null} onClick={()=>void decide(item,'accept')}>接受整组建议</Button><Button size="sm" variant="outline" disabled={!writable||busy!==null} onClick={()=>void decide(item,'reject')}>拒绝建议</Button></div>}
