...
import { inspectRemoteImageUrl, type BoardContentData, type BoardShapeVariant, type BoardStructuredKind } from "./board-content-adapter";
...
export function CollaborativeThinkingEditor({ boardId, clientId, doc, readOnly, title, status, onTitleChange, onBack, onSelectionChange, onAwareness, peers = [], currentUserId }: CollaborativeThinkingEditorProps) {
  ...
  const commitEdit = useCallback((value: string, close = true): boolean => {
    ...
    if (current.text !== value) {
      const rich = content?.type === "tile" || content?.type === "web-tile" ? { ...content, title: value } : content?.type === "template" ? { ...content, name: value } : content?.type === "icon" ? { ...content, name: value } : null;
  ...
  const importImage = useCallback((file: File, point = centerPoint(viewport)) => {
    if (!file.type.match(/^image\/(png|jpeg|webp|gif|svg\+xml)$/)) { setNotice("无法添加图片：请选择 JPG、PNG、WEBP、GIF 或 SVG 文件。"); return; }
    createContentAt(point, { version: 1, type: "image", status: "failed", assetId: null, sourceUrl: null, mimeType: file.type as Extract<CanonicalContentObject, { type: "image" }>["mimeType"], intrinsicWidth: 0, intrinsicHeight: 0, crop: { x: 0, y: 0, width: 1, height: 1 }, opacity: 1, borderColor: "#FFFFFF", borderWidth: 0, cornerRadius: 0, fileName: file.name, replacementOf: null, failureCode: "ASSET_UPLOAD_NOT_CONFIGURED" }, file.name);
    setNotice("图片尚未上传：资产上传服务未接入。白板只保存失败状态和文件名，原文件保持不变，可稍后重试。");
  }, [createContentAt, viewport]);
  ...
  }, [createContentAt, imageDialog, imageUrl, replaceContent, viewport]);
  ...
  const updateStructuredSample = () => {
    if (!selectedObject || !selectedContent) return;
    const next: CanonicalContentObject | null = selectedContent.type === "tile" ? { ...selectedContent, description: `${selectedContent.description || "说明"} · 已更新`, status: "active", tags: [...new Set([...selectedContent.tags, "board"])], fields: [...selectedContent.fields.filter((field) => field.key !== "owner"), { key: "owner", label: "负责人", value: "WorkspaceX" }] }
      : selectedContent.type === "web-tile" ? { ...selectedContent, title: `${selectedContent.title} · 已验证`, fetchStatus: "ready" }
      : selectedContent.type === "table" ? { ...selectedContent, rows: [...selectedContent.rows, { id: crypto.randomUUID(), cells: Object.fromEntries(selectedContent.columns.map((column) => [column.id, "新值"])) }] }
      : selectedContent.type === "icon" ? { ...selectedContent, color: selectedContent.color === "#18181B" ? "#2563EB" : "#18181B" }
      : selectedContent.type === "template" ? { ...selectedContent, parameters: { ...selectedContent.parameters, updated: "true" } }
      : null;
    if (next) replaceContent(selectedObject.id, next);
  };
