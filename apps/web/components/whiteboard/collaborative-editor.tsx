...
import { buildArrangeCommands, buildFormatCommands, copyObjects, readObjects, type ArrangeOperation, type BulkFormat, type WhiteboardObject, type WhiteboardCommand } from '@repo/whiteboard-core';
...
import { BulkArrangeToolbar } from './bulk-arrange-toolbar';
type Point = { x: number; y: number };
type Gesture = { mode: 'move' | 'box' | 'draw' | 'pan'; start: Point; current: Point; ids: string[]; points: Point[]; offset: Point };
export interface CollaborativeEditorProps { doc: Y.Doc; readOnly: boolean; readOnlyReason?: string; title: string; status: string; onTitleChange?: (title: string) => void; onBack?: () => void; onSelectionChange?: (ids: string[]) => void; onAwareness?: (cursor: Point | null, ids: string[]) => void; peers?: WhiteboardConnectionState['peers']; currentUserId?: string; reviewPanel?: ReactNode }
...
export function CollaborativeEditor({ doc, readOnly, readOnlyReason, title, status, onTitleChange, onBack, onSelectionChange, onAwareness, peers = [], currentUserId, reviewPanel }: CollaborativeEditorProps) {
  ...
  function execute(commands: WhiteboardCommand[], success = '') { if (readOnly) return false; try { model.execute(commands); setNotice(success); return true; } catch { setNotice('操作未应用：请检查对象是否仍存在或内容是否超出限制。'); return false; } }
  function selectObject(id: string, additive: boolean) { setSelected(current => additive ? current.includes(id) ? current.filter(value => value !== id) : [...current, id] : [id]); }
  function bulkArrange(operation: ArrangeOperation, label: string) {
    const result = buildArrangeCommands(model.objects, selected, operation);
    if (!result.ok) { setNotice(`无法${label}：${result.detail}`); return; }
    execute(result.commands, `已${label} ${result.affectedIds.length} 个对象；一次撤销可恢复整批。`);
  }
  function bulkFormat(format: BulkFormat, label: string) {
    const result = buildFormatCommands(model.objects, selected, format);
    if (!result.ok) { setNotice(`无法${label}：${result.detail}`); return; }
    execute(result.commands, `已${label} ${result.affectedIds.length} 个对象；一次撤销可恢复整批。`);
  }
  ...
  return <section data-testid="collaborative-editor" className="flex h-full min-h-0 flex-col bg-background text-background-foreground">
    ...
    {selected.length >= 2 && <BulkArrangeToolbar selectedCount={selected.length} readOnlyReason={readOnly ? readOnlyReason ?? '当前会话没有编辑权限。' : undefined} onArrange={bulkArrange} onFormat={bulkFormat} />}
    <div className="relative min-h-0 flex-1 overflow-hidden"><div ref={surface} data-testid="board-live-surface" className="absolute inset-0 touch-none overflow-hidden bg-panel-alt" onPointerDown={e=>down(e)} onPointerMove={move} onPointerLeave={() => { cursor.current=null; onAwareness?.(null,selected); }} onPointerUp={finish} onPointerCancel={()=>setGesture(null)}><div className="absolute inset-0 origin-top-left" style={{transform:`translate(${offset.x}px,${offset.y}px) scale(${zoom})`}}><WhiteboardRenderer objects={displayed} selected={selected} onPointerDown={down} onKeyboardSelect={selectObject}/>{peers.filter(peer=>peer.actorId!==currentUserId).map(peer=><div key={peer.actorId} className="pointer-events-none">{peer.selected.map(id=>{const selectedObject=model.objects.find(item=>item.id===id);if(!selectedObject)return null;const g=selectedObject.geometry;return <div key={id} data-testid={`peer-selection-${peer.actorId}-${id}`} className="absolute rounded-control border-2 border-dashed border-primary" style={{left:g.x,top:g.y,width:g.width,height:g.height,transform:`rotate(${g.rotation}deg)`}}/>;})}{peer.cursor&&<div data-testid={`peer-cursor-${peer.actorId}`} className="absolute text-primary" style={{left:peer.cursor.x,top:peer.cursor.y}}><span aria-hidden="true">↖</span><span className="rounded-control bg-primary px-1 text-11 text-primary-foreground">{peer.actorId}</span></div>}</div>)}{gesture?.mode==='draw' && <svg className="pointer-events-none absolute inset-0 h-full w-full overflow-visible"><polyline points={gesture.points.map(p=>`${p.x},${p.y}`).join(' ')} fill="none" stroke="currentColor" strokeWidth="2"/></svg>}{gesture?.mode==='box' && <div className="pointer-events-none absolute border border-primary bg-muted opacity-50" style={{left:Math.min(gesture.start.x,gesture.current.x),top:Math.min(gesture.start.y,gesture.current.y),width:Math.abs(gesture.current.x-gesture.start.x),height:Math.abs(gesture.current.y-gesture.start.y)}}/>}</div></div>
    {reviewPanel && <div className="absolute right-3 top-3 max-w-[calc(100%-1.5rem)]">{reviewPanel}</div>}
