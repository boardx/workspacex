...
export function CollaborativeEditor({ doc, readOnly, title, status, onTitleChange, onBack, onSelectionChange, onAwareness, peers = [], currentUserId, followViewport, onViewportChange, workshop, auxiliaryPanelOpen = false }: CollaborativeEditorProps) {
  ...
  return <section data-testid="collaborative-editor" className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-background text-background-foreground">
    ...
    <div role="toolbar" aria-label="白板工具" className="flex max-h-12 shrink-0 flex-nowrap gap-1 overflow-x-auto border-b border-border p-1 sm:max-h-28 sm:flex-wrap sm:overflow-y-auto sm:p-2">
