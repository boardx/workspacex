...
export function reportChartLabels(block: survey.CompiledSurveyBlock) {
...
export const reportChartHeight = (block: survey.CompiledSurveyBlock) => block.type === "bar"
  ? Math.max(180, reportChartLabels(block).reduce((height, label) => height + Math.max(44, wrap(label).split("\n").length * 18 + 12), 50)) : 400;
