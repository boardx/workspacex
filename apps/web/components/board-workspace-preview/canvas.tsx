...
export function WorkspaceCanvas({ options, seeded = true }: { options: Options; seeded?: boolean }) {
  ...
  useEffect(() => {
    ...
    if (seeded) c.add(new Rect({ originX: 'left', originY: 'top', left: 100, top: 130, width: 210, height: 190, fill: '#fff1a8', rx: 4, ry: 4 }), new Textbox('让每个想法\n都被看见', { originX: 'left', originY: 'top', left: 120, top: 165, width: 165, fontSize: 24, fill: '#292929', editable: true }), new Rect({ originX: 'left', originY: 'top', left: 385, top: 210, width: 210, height: 190, fill: '#d8e8ff', rx: 4, ry: 4 }), new Textbox('一起画出\n下一步', { originX: 'left', originY: 'top', left: 405, top: 245, width: 165, fontSize: 24, fill: '#292929' }));
    resize();
    ...
    c.on('mouse:down', event => {
      ...
      if (o.tool === 'sticky') { c.add(new Rect({ originX: 'left', originY: 'top', left: p.x, top: p.y, width: 180, height: 160, fill: o.color, rx: 4, ry: 4 })); const text = new Textbox('写下一个想法', { originX: 'left', originY: 'top', left: p.x + 16, top: p.y + 25, width: 148, fontSize: 22, fill: '#292929' }); c.add(text); c.setActiveObject(text); }
