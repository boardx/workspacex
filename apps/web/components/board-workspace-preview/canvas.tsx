...
export function WorkspaceCanvas({ options, seeded = true, draft, onDraft }: { options: Options; seeded?: boolean; draft?: string; onDraft: (draft: string) => void }) {
  ...
  return <div ref={host} className="absolute inset-0 touch-none" data-testid="workspace-fabric-surface" tabIndex={0} role="application" aria-label={options.readonly ? "只读白板画布，无法编辑对象。" : "白板画布。空白处双击创建便利贴；Tab 连续创建；Enter 添加当前工具对象；Delete 删除选中对象。"} onKeyDown={event => {
    ...
    {['#fff1a8', '#d8e8ff', '#f9d5e5', '#d6efcf'].map((color, index) => <button key={color} type="button" data-testid={`sticky-color-${index}`} aria-label={`便利贴颜色：${['黄', '蓝', '粉', '绿'][index]}`} aria-pressed={selected.backgroundColor === color} className="h-10 w-10 rounded-lg border-2 border-border focus-visible:outline focus-visible:outline-2" style={{ backgroundColor: color }} onClick={() => updateSticky({ backgroundColor: color })}>{selected.backgroundColor === color ? '✓' : ''}</button>)}
    <label className="text-sm">字号<select aria-label="便利贴字号" data-testid="sticky-font-size" className="h-10 bg-background" value={selected.fontSize} onChange={event => updateSticky({ fontSize: Number(event.target.value) })}>{[16, 22, 28, 36].map(size => <option key={size} value={size}>{size}</option>)}</select></label>
    <label className="text-sm">对齐<select aria-label="便利贴对齐" data-testid="sticky-alignment" className="h-10 bg-background" value={selected.textAlign} onChange={event => updateSticky({ textAlign: event.target.value as PreviewSticky['textAlign'] })}><option value="left">左</option><option value="center">中</option><option value="right">右</option></select></label>
    <button type="button" data-testid="sticky-duplicate" className="h-10 rounded-lg border border-border px-3 text-sm" onClick={() => { if (canvas.current && !current.current.readonly) { selected.exitEditing(); activateSticky(canvas.current, adjacentSticky(selected, true)); } }}>复制</button>
