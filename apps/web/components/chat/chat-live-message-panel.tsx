...
import {
  agentLabel, agentRoleLabel, resolveActiveSkillMount, agentSkillLabel, messageTime,
  AgentRunStatus, computeFollowUpSuggestions, appendUnique, newClientMessageId, FailureState,
  EMPTY_SKILL_MOUNTS, EMPTY_SKILL_NAMES, type FollowUpSuggestion,
  RUN_POLL_FIRST_DELAY_MS, RUN_POLL_BACKOFF, RUN_POLL_MAX_DELAY_MS, RUN_POLL_BUDGET_MS,
  type RunObservation,
} from "@/components/chat/chat-live-message-panel-helpers";
...
export function ChatLiveMessagePanel({
  ...
  const loadPage = React.useCallback(async (
    ...
    try {
      ...
      // 卡住不动、没有任何提示。这里从刚读回的持久消息里找回它——判定规则本身抽在
      // `findPendingRunId`（`lib/agent-run.ts`）：唯一事实源，session-switch
      // task-state-loss fix（copilotkit-v2 轨道那条恢复逻辑）复用的是同一份，不是
      // 各写各的。找到就重新挂上 `activeRunId` 让下面已有的轮询 effect（487 行起）
      // 接管——若那个 run 其实已经是终态，poll 一次就会发现并停止，不会产生错误
      // 状态，只多打一次 GET。
      if (mode === "replace" && !archived) {
        const pendingRunId = findPendingRunId(result.messages);
        if (pendingRunId !== null) setActiveRunId(pendingRunId);
      }
    } catch (failure) {
      if (generation.current !== requestGeneration) return;
...
/**
 * agentLabel / agentRoleLabel / EMPTY_SKILL_MOUNTS / EMPTY_SKILL_NAMES /
 * resolveActiveSkillMount / agentSkillLabel / messageTime / AgentRunStatus /
 * RUN_STATUS_TEXT / statusTone / FollowUpSuggestion / computeFollowUpSuggestions /
 * appendUnique / newClientMessageId / FailureState 现在都在
 * `chat-live-message-panel-helpers.tsx`（2026-08-30 文件规模拆分搬出，逐字节未改
 * 行为，本文件顶部已 import 回来供内部使用）——它们只消费 props（不闭包依赖本组件
 * 的任何内部状态），天然可独立成文件。
 *
 * `describeMessageFailure` 的再导出（原因见旧头注：避免 markdown-message ↔
 * chat-diagram-canvas-modal 之间成环）原样留在本文件——`chat-skill-mount-panel.tsx`/
 * `chat-read-screen.test.tsx` 的既有 import 路径不变。
 */
