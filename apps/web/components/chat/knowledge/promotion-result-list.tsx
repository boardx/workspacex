...
/**
 * 「存入个人空间」逐条结果（uc-18-4 R3-5 / R4-E4：部分成功，不整批回滚）。
 * 五种 outcome：promoted / merged_into_existing / coexisting / needs_choice（给合并|并存选择）/ rejected（带原因）。
 * outcome 与拒绝码均来自契约 `KgPromotionItemResult`，本组件只做投影。
 */
const REJECT_REASON_ZH: Record<string, string> = {
  KG_PROMOTE_REQUIRES_ACCEPTED: "需先在会话里确认（accepted）才能晋升",
  KG_EVIDENCE_REVOKED: "证据源已被删除，无法晋升",
  KG_CONTESTED_NEEDS_RESOLUTION: "冲突结论需先解决冲突",
  KG_CLAIM_NOT_FOUND: "结论已不存在",
};
    ...
    <div className="flex flex-col gap-2" data-testid="kg-promotion-results">
      {data.results.map((r) => {
        ...
        switch (r.outcome) {
          case "promoted":
            return (
              <ResultRow key={r.claimId} testid={`kg-promo-${r.claimId}`} tone="success" icon={<CheckCircle2 className="h-3.5 w-3.5" />} label={label} note="已存入个人空间" />
            );
          case "merged_into_existing":
            return (
              <ResultRow key={r.claimId} testid={`kg-promo-${r.claimId}`} tone="muted" icon={<GitMerge className="h-3.5 w-3.5" />} label={label} note="已合并到已有的同义结论" />
            );
          case "coexisting":
            return (
              <ResultRow key={r.claimId} testid={`kg-promo-${r.claimId}`} tone="muted" icon={<Copy className="h-3.5 w-3.5" />} label={label} note="与已有结论并存（标注了适用差异）" />
            );
          case "needs_choice":
            return (
