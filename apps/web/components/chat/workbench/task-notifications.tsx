...
export function TaskNotifications({sessionToken,onOpenThread,onRefresh}:TaskNotificationsProps){
  ...
  return <div className="relative px-3" data-testid="task-notifications">
    <Button ref={triggerRef} variant="ghost" size="sm" className="relative h-8 w-8 p-0"
      ...
      {total>0&&<span data-testid="task-notifications-badge" aria-hidden="true"
        className="absolute -right-0.5 -top-0.5 min-w-4 rounded-full bg-destructive px-1 text-10 leading-4 text-destructive-foreground">{total}</span>}
    </Button>
    ...
    {open&&<div ref={popoverRef} id="task-notifications-popover" data-testid="task-notifications-popover"
      ...
      className="absolute left-3 right-3 z-50 mt-1 max-h-80 space-y-2 overflow-y-auto rounded-control border border-border bg-popover p-2 shadow-md">
