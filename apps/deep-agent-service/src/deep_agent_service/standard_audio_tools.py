"""Transcribe an authorized audio attachment through the existing ASR gateway."""
import asyncio
import json
...
_SCHEMA=json.loads((Path(__file__).parent/'generated/standard_audio_schema.json').read_text())
...
class StandardAudioError(RuntimeError):
...
async def _parse(runtime,args):
    try:
        ...
        url=f"{base}/internal/agent-runs/{quote(callback['run_id'],safe='')}/audio-transcribe"
    ...
    except Exception:raise StandardAudioError('Audio transcription failed, mode is unsupported, or outcome is unknown. Do not resubmit automatically.') from None
