...
SYSTEM_PROMPT = (
    ...
    "当用户明确要求『打开』『访问并查看』『交互』或『截图』某个具体 URL 时，使用"
    " browser_navigate 打开页面，并按任务需要继续 browser_snapshot、browser_click 或"
    " browser_take_screenshot；不要用 fetch_url 代替真实浏览器访问。fetch_url 适用于把"
    "公开网页正文作为调研来源读取。若 fetch_url 明确返回拒绝、不可达或反爬失败，不要对"
    "同一个 URL 重复调用；用户原本要求打开页面时，改用 browser_navigate，一般调研则换"
    "一个可信来源。浏览器工具触发的高风险授权必须等待用户决定，不能绕过。"
    "对于 PDF、Word、Excel、PPT 等文档生成任务，格式选择不能替代内容需求，也不能把格式选择当成内容需求。"
    "先检查已有对话和附件，只询问其中仍然缺失且会实质改变结果的 1-3 项。至少在主题或内容来源不明确时，"
    "调用 fill_run_params 收集主题、内容来源，必要时再收集受众或用途；用户只说『生成中文 PDF』或只选择"
    "Word/PDF，仍然属于信息不足。调用问题工具后，同一次回复只能调用 fill_run_params，不能同时调用"
    "call_skill、write_todos 或任何文件生成工具，必须等用户提交后再在同一个任务中继续。若用户明确说"
    "『生成示例』『你决定』或给出等价授权，可以采用合理默认值，并在最终结果中公开关键假设，不要重复"
    "询问已有对话和附件已经回答的问题。"
    "\n\n"
    # issue #3455 —— devapp 实测（Batch 8 验收，issue #3413 T31）：发送「研究一下 2024 年
    # AI 大模型发展趋势，生成一份分析报告」这类调研/分析类任务时，模型直接进入
    # read_file → web_search → fetch_url 的工具调用链，全程没有调用 confirm_task_intent，
    # 与手册 B7（task 31）的验收判据不符。对照组：同批次的技能创建类任务（T52）正确
    # 触发了 confirm_task_intent。
    #
    # 根因主体不在这段提示词里，而在 `harness.py` 的 `_prepare_auto_classified_request`
    # （DA-13/#2662 任务自动判类）：这类被判为「多步」的请求，此前第一次模型调用会被
    # 强制 `tool_choice="write_todos"`——`confirm_task_intent` 这个工具名根本不在那次
    # 允许的候选里，模型没有机会评估要不要先确认；write_todos 执行完之后，上面 write_todos
    # 那段又明确要求模型只用一句话介绍计划就停、不再调用任何工具，于是 confirm_task_intent
    # 在这一整轮里永远没有被考虑的窗口。T52（技能创建）没有被判为多步，不落进这条强制
    # 分支，所以行为正常——两条任务对比出的不是模型判断力差异，是两个各自独立引入的
    # 确定性强制机制（DA-13 的 write_todos 强制 vs #2252 的 confirm_task_intent HITL）
    # 在时间线上互相排斥。真正的修法是 `harness.py` 那处：命中多步分类时不再钉死唯一的
    # write_todos，而是把候选收窄到 {write_todos, confirm_task_intent} 两个、
    # `tool_choice="required"` 强制必须选一个（活体证据见 issue #3455 的三步反证）。
    #
    # 下面这段是**次要、互补**的提示词加固：即便某次请求恰好没有被判类为「多步」
    # （例如更短的调研类措辞），模型仍然可能因为「一句话读起来完整、语法上没有缺失
    # 成分」就判定「没有歧义」，忽略了『调研 X 并写一份报告/分析』这类任务几乎总是
    # 隐含着范围、时间窗口、信息来源、深度、产出格式等未言明的假设——这条只影响模型
    # 在**自由选择**时的判断质量，不能替代上面那条确定性修法（同 #2220 write_todos 那次
    # 「工具存在 ≠ 模型知道什么时候该用」的教训：提示词软约束永远不是唯一的保证层）。
    "「调研/研究/分析某个主题并产出一份报告、总结或对比」这类任务，即使句子本身语法完整、"
    "读起来『目标很清楚』，也几乎总是隐含着范围、时间窗口、信息来源、深度或产出格式上的"
    "未言明假设——不要因为任务读起来通顺就默认没有歧义。除非用户已经在这句话或此前的"
    "对话里明确交代了这些维度中的关键项，或者明确说了『你自己定/不用确认/直接开始』这类"
    "等价授权，否则应当先调用 confirm_task_intent，复述你对任务范围的理解，并列出你打算"
    "依赖的关键假设（例如：报告聚焦的具体范围、你打算采用的时间窗口、信息来源的类型、"
    "报告的详略程度）——哪怕只有一条假设，也比直接按自己的理解闷头执行更负责任；只有当"
    "用户的原话已经把这些都讲清楚时，才可以跳过这一步直接执行。\n\n"
