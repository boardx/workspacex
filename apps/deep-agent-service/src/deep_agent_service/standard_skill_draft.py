"""Produce a complete draft package without granting publication authority."""
import asyncio
import json
...
_SCHEMA=json.loads((Path(__file__).parent/'generated/standard_skill_draft_schema.json').read_text())
...
async def _parse(runtime,args):
    try:
        ...
        url=f"{base}/internal/agent-runs/{quote(callback['run_id'],safe='')}/skill-draft"
    ...
    except Exception:raise SkillDraftError('Skill draft unavailable or refused; no result confirmed. Do not automatically retry.') from None

def skill_draft_tool():
    ...
    return StructuredTool(name=_SCHEMA['toolName'],args_schema=_SCHEMA['toolInput'],func=sync,coroutine=run,
        description='Create a complete JSON skill draft package from explicit workspace files. This only creates an artifact draft, not an enabled skill. Runtime declarations and file integrity are checked; code dependencies and fixtures are not executed by this tool. Publish the returned workspacePath with wx_artifact_publish, then an authorized administrator may import the exact artifact/version through the existing import workflow.')
