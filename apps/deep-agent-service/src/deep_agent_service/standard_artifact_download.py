"""Native artifact download entry; the gateway supplies and rechecks requester identity."""
import asyncio,json
...
_SCHEMA=json.loads((Path(__file__).parent/'generated/standard_artifact_download_schema.json').read_text())
class StandardArtifactDownloadError(RuntimeError):pass
async def _invoke(args,runtime):
 try:
  ...
  async with asyncio.timeout(deadline):
   async with httpx.AsyncClient(timeout=deadline,follow_redirects=False,trust_env=False) as client:
    async with client.stream('POST',f"{base}/internal/agent-runs/{quote(callback['run_id'],safe='')}/standard-artifact-download/invoke",headers={'x-deep-agent-internal-key':callback['key'],'accept-encoding':'identity'},json=body) as response:
 ...
 except Exception:raise StandardArtifactDownloadError('Artifact download unavailable or request refused; no download reference confirmed') from None
def artifact_download_tool():
 ...
 return StructuredTool(name=_SCHEMA['toolName'],description='Issue a short-lived, one-time download reference for an artifact version visible to the current run requester. The reference remains bound to that requester and is not a public URL.',args_schema=_SCHEMA['toolInput'],func=sync,coroutine=run)
