...
def document_parse_tool():
    ...
    return StructuredTool(name=_SCHEMA['toolName'],args_schema=_SCHEMA['toolInput'],func=sync,coroutine=run,
        description='Convert a current-run original /inputs file to verified Markdown in /workspace using offline AnyDoc. outputMode=chunks additionally returns a verified structurePath for PDF/DOCX/PPTX/XLSX: PDF table cells carry page and point-space boxes; DOCX carries paragraph/table indexes without invented page numbers; PPTX carries slide/element indexes; XLSX carries sheet/cell addresses and formulas without recalculation. Adjacent PDF fragments are grouped only by an explicitly warned repeated-header heuristic. Explicit ocr=true keeps the existing Tesseract rendered-page pixel boxes and confidence. Unsupported formats are refused. Read returned workspace files; they are not published attachments.')
