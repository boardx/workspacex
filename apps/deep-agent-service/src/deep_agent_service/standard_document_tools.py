...
def document_parse_tool():
    ...
    return StructuredTool(name=_SCHEMA['toolName'],args_schema=_SCHEMA['toolInput'],func=sync,coroutine=run,
        description='Convert a current run original /inputs file to verified Markdown in /workspace using offline AnyDoc. Only markdown output is supported. Default extraction uses AnyDoc without page coordinates. Explicit ocr=true supports PDF/PNG/JPEG via offline Tesseract and returns structurePath with real rendered-page pixel boxes and confidence; read it for page locations. OCR may misrecognize text. Cross-page table reconstruction is not guaranteed. Read the returned textPath to inspect content; this is a workspace file, not a published attachment.')
