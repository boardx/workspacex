...
async def main():
 ...
 async with native_factory.native_graph_context(config) as graph:
  async for mode,event in graph.astream({'messages':[{'role':'user','content':prompt}]},config={**config,'recursion_limit':200},stream_mode=['custom','values']):
   ...
   if mode=='values':
    ...
    partial={'calls':[{'name':c['name'],'args':c['args']} for m in event['messages'] for c in getattr(m,'tool_calls',[])],'skillActivity':stages,'results':[{'toolCallId':m.tool_call_id,'name':m.name,'content':m.content,'status':getattr(m,'status',None)} for m in event['messages'] if m.type=='tool']}
