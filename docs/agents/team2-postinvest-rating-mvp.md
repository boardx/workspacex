...
- ❌ 新增后端端点/工具——文件解析、算分、出报告全部走已有平台能力。
- ❌ 机械阻断的人工确认关口（HITL）——本版靠任务书用自然语言要求模型在数据不足时
  先问人，不做机械阻断；真正机械阻断需要接 `deep-agent-hitl`，是下一档。
- ❌ 历史同期对比、受限渠道行业背景检索的白名单强制、定期评级提醒、反馈闭环版本链
  ——这些是 Phase 16 完整需求（`phases/phase-16-postinvest-rating-agent/`）里才有的
  能力，本 ad-hoc MVP 不做。
- ❌ 未登录公开访问。
...
1. **算分正确性依赖模型把脚本写对。** 任务书给了公式和两个自检算例，但模型仍可能
   ...
   但不是形式化证明。
2. **本会话环境无法端到端验证真实模型输出质量。** 需要在接了真实 Postgres + 真实模型
