...
> 状态：**提案，待人类评审与拍板**（2026-09-24）。不代表任何能力已实现或已签核。
...
5. **路线图**（§8）：四个波次。**W1 不依赖任何新连接器**，只靠 phase-15 工作台 + 平台级安全策略 + 最小评测，就能上线 20 个协作类 skill（写需求、周报、runbook、知识库文章、研究综合……）；W2 接四类连接器与组织档案，推出第一个专家智能体；W3 扩运行时（Python/Office/SQL/调度），打开金融建模、SEO、后台 agent；W4 才对外开放社区（子注册表 + 验证徽章 + 创作者提交）。
6. **要人拍板的 6 件事**（§9），最关键的是：商业研究稿 D4 定"技能包内容闭源售卖"，而本方案的内容大量来自 Apache/MIT 开源——**开源导入的内容本身不能闭源卖，能卖的是验证、本地化、组织档案与托管执行**。
...
| **技能包作者 / 社区贡献者** | 写了好 skill 没有分发渠道、不知道质量如何 | 提交即获得自动 lint、安全扫描、评测跑分；达到门槛获得验证徽章与曝光 | 第一个 skill 通过 L3 并出现在目录里（H2 才承诺作者体验，见 §9-D2） |
...
flowchart TB
  ...
  subgraph W1[W1 协作类 20 个 skill 上线 · 零连接器]
    S15[phase-15 工作台<br/>导入/草稿/试跑/发布]:::todo
    SP[C11 平台安全策略收敛]:::todo
    E1[C12 最小评测<br/>L1-L3 + L5 金样例]:::todo
  ...
  subgraph W2[W2 连接器 + 档案]
    ...
    PF[C5 组织/领域档案<br/>访谈式 onboarding]:::todo
  ...
  subgraph W4[W4 对外社区]
    RG[C13 子注册表 + 徽章 + 30 天趋势]:::todo
    PR[组织私有目录 / 白名单]:::todo
    SUB[创作者提交 + L6 专家认证]:::todo
    UP[上游回馈 PR]:::todo
  end
...
- W2 的档案（C5）与连接器（C3/C4）**并行**；法务专家智能体两者都要。
...
| 波次 | 出口判据 |
|---|---|
| W0 | §9 六项有人类书面决定；上游登记表入库（仓库 / SHA / 许可 / 结论）；许可门 lint 对 Proprietary / NC / 无 LICENSE 输出红 |
...
| W4 | 外部作者从提交到上架 ≤ 公布的审核时限；目录每个条目展示最高验证层、评测提升、扫描时间、许可、维护活跃度；上游 bump 失败能自动回滚 |
...
| 编号 | 决策 | 冲突 / 背景 | 建议 |
|---|---|---|---|
| **D-EC1** | **开源导入的内容怎么卖** | 研究稿 D4：「运行时开源、内容与评测集闭源」。但本方案首批内容来自 Apache-2.0 / MIT 上游，改写后仍须保留许可与 NOTICE，**不能闭源再卖** | 分两类：①开源导入内容**免费提供**，卖点是验证、本地化、托管执行与企业治理（研究稿"售卖"栏里的后三项不受影响）；②自研内容（中国法本地化 playbook、行业评测集、组织档案模板）按 D4 闭源售卖。需要回写 D4 |
| **D-EC2** | **社区何时对外** | 研究稿 H1 不承诺技能包作者体验（H2，2028–2031）；本方案 W4 开放作者提交 | W1–W3 只做**策展**（我们导入、我们验证），不开放外部提交；W4 才对外，时间点与 H2 对齐或经决策提前 |
| **D-EC3** | **第一个专家智能体选哪个** | 候选：产品经理助理（最稳、零连接器）/ 商事法务（价值感最强、需本地化）/ 财务建模（与投后客群一致、需 Python 沙箱） | 产品经理助理先上（W1，验证全链路），法务作为第一个"重磅"（W2） |
| **D-EC4** | **本地化优先级** | 研究稿 D8 为"中国优先"；而上游内容几乎全是美国法、美国准则 | 法务、财务、HR 三个领域的 skill 在本地化前**标注适用法域**，不对中国用户默认启用 |
| **D-EC5** | **许可证白名单** | 需一次性定死，写进 L2 门控 | 允许 Apache-2.0、MIT、BSD、CC-BY-4.0（署名）；拒绝 Proprietary、CC-NC、Commons Clause、GPL/AGPL（内容与代码皆拒，独立部署的外部服务除外）、无 LICENSE |
...
1. 人类就 §9 六项给出决定（W0）。
2. 开 issue：`[W0] 上游内容登记表 + 许可门 lint`、`[W0] SKILL.md 格式对齐 agentskills（allowed-tools、宽松解析）`、`[W1] 平台级安全策略收敛（上游 9 条 + SMB 14 份共享规则 → 一份）`、`[W1] 评测 runner 最小版（借 skill-creator）`。
3. 由 requirement-author 把 §3 C1/C2/C11/C12 与 §6.1 首批 20 个写进 phase-15 的 `requirements/`，生成 feature。
4. 对 W2 的四类连接器各做一次 provider 可用性实测（尤其国内网络下的 Google / Slack / HubSpot 远程 MCP）。
