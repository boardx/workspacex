...
> 纪律）；涉及 UI 的条目（CE-030/031）开工前必须先有该契约束的 `design-signoff.md`
> （ADR-023），不能跳过签核直接画界面。
>
...
| CE-001 | P0 | ⬜ 未开始 | CE-030（Claim 审核工作台）所属契约束的 `design-signoff.md` | CE-000 | 三节签核（UI/用例/API 契约）齐全，人类签字，不能 agent 自签 |
...
| CE-031 | P0 | ⬜ 未开始 | `ontology_objects` 表（真实建，今天完全不存在） | CE-030 | 人/项目/研究对象/需求/决策等实体，`ontology_edges` 挂到真实 entity_id 上，不是纯字符串 |
...
## Epic CE-E4 — Agent 工作流（P4：LangGraph + checkpoint + Context Pack 联动）

| ID | P | 状态 | 交付物 | 依赖 | 完成契约 |
|---|---|---|---|---|---|
| CE-040 | P0 | ⬜ 未开始 | `DeepAgentModelProvider`（TS 侧，接 `apps/deep-agent-service`） | 无（deep-agent-service 骨架已在 #739 落地，缺对接层） | 对照已真实接入的 `DeepResearchModelProvider`（建线程→提交 run→轮询→读 state）写同构实现；反证：真实提交一次深度研究请求，拿到真实结果 |
...
| CE-042 | P0 | ⬜ 未开始 | checkpoint 保存与恢复（今天全仓 0 实现） | CE-040 | Agent 运行中定期落 checkpoint；反证：真实 kill 掉一个跑到一半的深度研究进程，从 checkpoint 恢复，不是从头重跑 |
