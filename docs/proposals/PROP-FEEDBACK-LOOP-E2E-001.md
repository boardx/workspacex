...
## 6. D4 的后续（2026-09-02，FB-5）

上面第 4 节 D4 的建议——「第一版不做附件，把附件连同脱敏一起放到 FB-4 之后」——
在 2026-08-15 的正式签核里被采纳（`design-signoff.md` 的 D4：不做）。

2026-09-02，人类在另一次会话里直接授权重开这一条：本轮就做图片附件（连同语音输入
一起），**且明确不等 E2 脱敏**——不是"先做脱敏方案的简化版（如 EXIF 剥离）"，而是
"这一轮完全不做脱敏，直接出功能"，脱敏本身作为已登记的已知限制留给未来。完整决策
记录见 `design-signoff.md` 的「D4 重开（2026-09-02）」一节——那里是这条决策的权威
记录，本节只是让读这份提案的人不要停在"第一版不做附件"这句已经过期的结论上。
