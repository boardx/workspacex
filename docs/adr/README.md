...
| ADR-109 | 数字访谈报告采用可恢复增量流 | Proposed |
...
| ADR-115 | board-fabric-renderer-with-yjs-canonical-model | Board 主画布使用 Fabric.js 7.4 做增量视觉投影；Yjs/`whiteboard-core` 是唯一事实源，Fabric JSON 不持久化，React 提供无障碍镜像，`DiagramModel` 只用于导入。**Proposed** |
