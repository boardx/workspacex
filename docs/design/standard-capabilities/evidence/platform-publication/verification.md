...
Follow-up: standard-document 1.1.0 is now selected by the same manifest list. A new real database run persisted all three files and replayed without duplicate versions; both platform publication tests passed. The group output is `../ci/json-staging-document-seed.txt`. This adds the sixth manifest; the earlier five-manifest run remains historical evidence.
