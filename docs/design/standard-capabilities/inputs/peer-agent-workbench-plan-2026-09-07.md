...
日期：2026-09-06。状态：设计输入 / 实施提案，尚未创建实施 feature、签核或执行代码重构。

## 开发进度图（更新于 2026-09-07）

本图是本升级项目的进度展示入口。初始化依据为本计划已记录的代码调查，尚未重新核实远端部署。灰色表示已有代码可复用，不等于本次验收通过；绿色只用于有证据的已完成项。目前完成了计划文档，S0–S12 尚未完成实施验收，不计算虚假的完成百分比。
...
flowchart TB
  ...
  P --> S0["S0 待复核：基线与测试环境\n此前环境受阻，开工需重测"]:::pending
  S0 --> S1["S1 待执行：真实组件原型\nUI / 用例 / API 签核"]:::pending
  S1 --> S2["S2 待执行：统一身份与事件契约\n固定前后端开发边界"]:::pending

  subgraph FE["前端工作线"]
    S8["S8 待重构：执行时间线\n默认折叠 · Thinking 摘要 / Tool / Skill"]:::pending
    S9["S9 待整合：输入器与任务导航\n跨会话状态提醒"]:::pending
    S10["S10 待整合：成果工作区\n预览 / 版本 / 继续修改"]:::pending
    S8 --> S9
    S8 --> S10
  ...
  subgraph API["API / 数据工作线"]
    S3["S3 待补齐：持久事件与命令\n队列 / 快照 / 回放"]:::pending
    S4["S4 待重构：真实流式事件\nThinking 通道 / 工具状态 / AG-UI 桥"]:::pending
    S3 --> S4
  end

  subgraph ENGINE["内核与运行控制工作线"]
    S5["S5 待接通：停止 / 暂停 / 恢复\n服务端确认与 checkpoint"]:::pending
    S6["S6 待补齐：本轮插话与排队\n安全边界应用及 ACK"]:::pending
    S7["S7 待整合：计划与审批\n可恢复的问题和决定"]:::pending
    S5 --> S6 --> S7
  end
...
2. 使用统一标签：`待执行`（白）→ `进行中`（蓝）→ `待验证/评审`（黄）→ `已完成`（绿）。`阻塞`（红）需记录具体原因与下一动作。不能因 coding Agent 返回 done 就标绿。
3. 工作包包含多条 feature 时：任一开工则显示进行中；全部实现但验证或 PR 门未过显示待验证/评审；全部达到仓库完成定义才显示已完成。发布节点另需实际发布和发布后证据。
4. 每次认领、完成实现、验证结束、评审结论、合并或阻塞变化时更新本节 Mermaid 标签/class，并追加下面变更记录。对外汇报复用这张图，不维护另一个手工百分比看板。
5. 变更记录写实际消耗的输入/输出/缓存 Token（运行时能提供才填）；没有计量就写未采集，不能由代码行数估出“已消耗”。可据首批任务校准剩余预算。
6. 本次仅建立跟踪图，没有启动开发 Agent、创建实施任务或安排后台定时更新。后续执行时随上述事件更新；图不会自己读取 GitHub。
