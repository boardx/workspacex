...
1. **清 `e2e-full` 存量红线**：按 §14.1 左列执行，记录基线 run。
   基线记录见 [issue #2996](https://github.com/boardx/workspacex/issues/2996)（本方案不复制其中的数字）。
   ⚠ 分诊顺序有讲究：「元素压根不出现 / 输入框点不到 / 回环无产出」是**没跑到**，
   不是「行为不符合契约」。先定位这类的单一共因，再回头看真正走到业务断言的那几条；
   否则一个装置问题会被放大成几十个假缺陷，并逐条污染 AT-* 的 `FAIL`。
