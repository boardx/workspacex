...
Re-run the identical command to resume. Each tenant persists a bounded UUID snapshot,
cursor and discovery cycle. A full cycle that inserted work starts a fresh snapshot;
only a full cycle with zero new items marks discovery exhausted. This catches a Board
that became visible below an earlier UUID cursor without duplicating work. A worker
claims with `FOR UPDATE SKIP LOCKED` and an owner/token/epoch fenced lease. Expired
leases return to retry, and a stale process cannot finish or release a reclaimed lease.
Job configuration is immutable for the rollout ID; changing pressure limits requires a
new rollout ID.

The runner schedules tenants round-robin. PostgreSQL is the authority for global and
per-tenant concurrency, round-robin contenders and launch rate: a locked limiter row
and durable global lease make the limits apply across CLI processes. The repository
also rechecks the persisted control state at claim and before every Board phase. Each
invocation is bounded by pages, Boards, phases and retries.
...
Pause and cancel are durable. They atomically invalidate the exact active lease tokens;
a worker that finishes the current phase observes a stale lease and returns normally
without writing a second outcome. Cancel marks all non-terminal items cancelled. Every
control and applied outcome appends a tenant-isolated audit event.
