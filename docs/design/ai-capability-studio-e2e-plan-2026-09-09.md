...
最新进展（2026-09-10 02:00）：设计 PR #3239 已推送 da4ecce43，恢复流程 UI 13 项、共享 guard web157/API143 项通过；仍是提案。当前两个 CI job 被 Google apt 源 Hash Sum mismatch 阻断，重试请求暂不被 GitHub 接受，等待 workflow 结束后重试。三个本地并行 worker 触发用量限制，后续工作不按四席持续可用估算；下表人日预算不变，日历交付期需视可用席位与外部等待复估。来源基线与草稿结果闭环见阶段材料 remaining-runtime-closures.md。
...
flowchart TB
    ...
    subgraph DESIGN["设计材料并行准备（本次复估合计 10 人日）"]
        ...
        C32["私库连接授权与撤销生命周期<br/>预计 1 人日 · 授权与失效恢复原型已补，待复审"]
        C33["试跑日志/输入/产物与失败详情读取<br/>预计 0.75 人日 · 已发布试跑下载提案已补，草稿结果待补"]
        C34["三方文件内容/历史恢复/来源修复<br/>预计 0.75 人日 · 绑定修复原型已补，可信基线待补"]
        C35["上传历史/限制详情/临时对象处理<br/>预计 0.5 人日 · 取消与清理原型已补，待复审"]
