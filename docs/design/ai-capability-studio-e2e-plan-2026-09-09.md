...
绿色 A 表示范围与验收方案已写入本文；绿色 C1 表示 Skill 工作台和导入原型通过17项组件/状态测试及3次变异反证，已有浏览器截图见阶段UI材料，均不代表生产功能完成。复审修正的原生后退/键盘复验单列Y，Mac锁屏是工具确认的真实阻塞。原C拆为C1–C4和Y，合计仍为3人日。C3已有14项契约测试，Model/MCP操作delta和完整覆盖仍需补齐。X 的阻塞依据是新身份尚未完成注册合并、enroll和有效凭据接入，不能冒用其他身份。G 正常等待完整材料，不把尚未提请的签核写成用户阻塞。
