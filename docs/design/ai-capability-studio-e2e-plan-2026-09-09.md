...
flowchart TB
    ...
    A --> C["UI 原型、用例与 API 契约<br/>预计 3 人日 · 进行中：已有方案，待原型与契约"]
...
绿色 A 仅表示范围与验收方案已写入本文，不代表契约已签或功能已实现。B 已开始隔离环境准备；C 已有方案文档，原型与正式契约尚未完成。X 的阻塞依据是当前会话没有被分配注册身份，已查到的 dev-ai-runtime / dev-chat-e2e 在当前 registry 为停用，不能擅自冒用。G 尚在正常等待设计材料，不把尚未提请的签核写成用户阻塞。
...
三个云端任务已确认仓库可读。主任务已接管工作台原型并完成本地运行、7项状态测试和浏览器主路径/七态验证，截图索引见阶段 design-proposal/capability-studio/ui.md。云端任务继续处理导入契约一致性、导入交互矩阵和独立PR审查。原型与schema草案仍未接生产API，因此总图C保持黄色，生产功能节点不提前标绿。
