...
flowchart TB
    ...
    subgraph DESIGN["设计材料并行准备（本次复估合计 10 人日）"]
        ...
        C31["普通仓库兼容分类与适配草稿<br/>预计 1 人日 · 分类契约已补，界面待补"]
        C32["私库连接授权与撤销生命周期<br/>预计 1 人日 · 提案进行中"]
        C33["试跑日志/输入/产物与失败详情读取<br/>预计 0.75 人日 · 未开始"]
        C34["三方文件内容/历史恢复/来源修复<br/>预计 0.75 人日 · 未开始"]
        C35["上传历史/限制详情/临时对象处理<br/>预计 0.5 人日 · 未开始"]
        C36["管理历史读取/首次连接与凭据一致性<br/>预计 0.5 人日 · 未开始"]
        C30 --> C31 --> C3
        C30 --> C32 --> C34 --> C3
        C30 --> C33 --> C3
        C30 --> C35 --> C3
    ...
    class B,C2,C3,C4,C31,C32,LY active;
    class X,Y,LR blocked;
    class C33,C34,C35,C36,G,D,I,U,M,P,F,E,T,R,H,V,J,K,L,Z,LP pending;
```
...
绿色 A 表示范围与验收方案已写入本文；绿色 C1 表示 Skill 工作台和导入原型通过17项组件/状态测试及3次变异反证，已有浏览器截图见阶段UI材料，均不代表生产功能完成。复审修正的原生后退/键盘复验单列Y，Mac锁屏是工具确认的真实阻塞。原C拆为C1–C4和Y，复估合计10人日。C3已有契约反例测试，涵盖来源、版本、AI差异、上传预检、失败草稿血缘和Model/MCP操作delta；独立复审与完整功能覆盖仍需收口。X 的阻塞依据是新身份尚未完成注册合并、enroll和有效凭据接入，不能冒用其他身份。G 正常等待完整材料，不把尚未提请的签核写成用户阻塞。
