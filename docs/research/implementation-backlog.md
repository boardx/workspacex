...
| C8 | 本地零出网拔网 e2e | ✅ | 独立观察者装在 `net.Socket.prototype.connect` + `dns.lookup` 上「拔网」跑代表性本地流程（运行时探活 + 本地模型调用 + 读账本），断言非回环连接 0 次；反证：混入遥测式外连 / 承诺内外连 / 外部域名解析均被抓到。静态半边 `lint-local-zero-egress.mjs` 接入 `verify:harness:raw` |
...
| E7 | 根级 compose + 升级命令 | ✅ | R17：根 `compose.yaml` 用 include 组合、`scripts/upgrade.sh`（备份→迁移→健康检查→回滚提示）、`lint-self-host-compose`。浏览器运行时仍单独起；完整升级待真机验证 |

### 轨道 F · devportal

| # | 项 | 状态 | 依赖 |
|---|---|---|---|
| F1 | 公开层拆域 | ◐ | R17：按主机名路由 + 公开页不引用协作层数据的门控。**公开域名待人定**，在此之前 CD 带 `--allow-placeholder`，拆分不生效 |
