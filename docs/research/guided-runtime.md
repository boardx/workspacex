...
## Outline-driven chapter reports (#2868)

Reports are generated in separate model calls for each enabled outline chapter, followed by a synthesis call for the title and executive summary. Chapter context prioritizes its search-task evidence and includes a bounded set of retained source excerpts. Prompts require the chapter's questions to be answered through findings, analysis, implications and recommendations; missing evidence is explicitly described. A length target encourages depth, but is not grounds to fabricate facts. New chapters must contain multiple subheadings and substantive paragraphs, preserve the requested section ID, and pass source validation.

All chapter calls belong to the same durable command. The report stream accumulates actual provider fragments in outline order. Each model attempt is persisted; reloading the browser resumes the stored preview and does not dispatch another generation command. Failure in any chapter or final synthesis retains an unfinished preview and never publishes a complete report.

New report bodies identify sources beside the relevant claims with `[[source:<id>]]` markers. The UI renders validated retained-source references as linked superscript numbers and produces one reference entry per normalized URL. Repeated sources reuse their number. Markdown exports retain the same citation mapping. Existing reports with section-level source IDs remain readable through section-level references. Source records, not model-supplied URLs, remain authoritative.
