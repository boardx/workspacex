# WorkspaceX 开源方案（v32.3）—— 完整解决方案、各方体验与可执行门控
...
| 售卖 · 市场通道 | 签名、审核、结算、分成 |
| **不交付 · 内部运营平面** | 协调平面（coord-gateway 与 coord-* 包）、**devportal 的协作层与平台层**、发布与部署控制台、事故面板、agent 车队状态、CI 指标、状态页、密钥轮换台账、**GTM 活动与运营元数据、对外案例、我们自己的组织大脑**（三者的数据边界见下节） |
...
| **② 运营元数据** | 部署与事故、PR 队列与门控、readiness/dashboard/scorecard 快照、agent 车队状态、CI 指标、密钥轮换台账（只记元数据）、功能开关控制面、**GTM 活动排期与漏斗统计、归因、A/B 结果、商机阶段与金额、客户组织的不可逆 ID**、**我们自己的组织大脑** | **Cloudflare** |
...
**必须改名。** 本仓已因 MAAU 一词两义吃过亏（画布产物 vs 分发单元）。
两个组织大脑同名，文档与代码必然各说各的。建议我们自己那个另起名称，
并在 `PROJECT.md` 登记为单一事实源。

⚠ **一个尚未回答的问题**：我们自己的组织大脑，**是不是就用我们自己的产品**？
...
| **D14** | **我们自己的组织大脑用不用自家产品** | 用了它就跑在产品栈上、不在 Cloudflare；不用则要回答「为什么自己不用」。**这条未决，且会推翻上面的放置决定** |
| **D15** | **我们自己的组织大脑叫什么** | 必须与产品的「组织大脑」区分命名，并在 `PROJECT.md` 登记。本仓已因 MAAU 一词两义吃过亏 |
