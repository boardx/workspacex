# WorkspaceX 开源方案（v32.6）—— 完整解决方案、各方体验与可执行门控
...
| 售卖 · 市场通道 | 签名、审核、结算、分成 |
| **不交付 · 内部运营平面** | 协调平面（coord-gateway 与 coord-* 包）、**devportal 的协作层与平台层**、发布与部署控制台、事故面板、agent 车队状态、CI 指标、状态页、密钥轮换台账、**GTM 活动与运营元数据、对外案例、团队记忆**（三者的数据边界见下节；团队记忆的 canonical 在可移植层，边缘只放投影——D14 v32.6） |
...
| **② 运营元数据** | 部署与事故、PR 队列与门控、readiness/dashboard/scorecard 快照、agent 车队状态、CI 指标、密钥轮换台账（只记元数据）、功能开关控制面、**GTM 活动排期与漏斗统计、归因、A/B 结果、商机阶段与金额、客户组织的不可逆 ID**、**客户实例上报的运行信号**、**团队记忆的投影** | **Cloudflare** |
...
| 承载 | `knowledge-ontology.md` 的四张 PG 表，每行带 tenant/privacy/permission，RLS 隔离 | **同一套四张 PG 表**——团队记忆就是我们自己那个实例的组织大脑（D14 v32.6）。边缘只有它的投影 |
...
（所以团队记忆就装在产品的组织大脑里，dogfooding 成立），**同时是产品栈上的一个闭源扩展**，
**跑在边缘并连接到所有客户实例**。
...
| 层 | 是什么 | 在哪 |
|---|---|---|
| ① 产品本体 | 一个真实 WorkspaceX 实例，跑我们自己的组织；团队记忆 = 它的组织大脑 | **可移植层**（PG + pgvector + AGE） |
| ② 运营扩展 | 闭源技能包与应用：车队、事故、GTM、CRM 看板、发布控制台 | 与①同栈 |
| ③ 边缘呈现与联邦 | Workers / DO / Pages：汇聚、路由、缓存、定时、呈现 | **Cloudflare** |

①**不能**搬上边缘：组织大脑要 PG 四表 + pgvector + AGE，Cloudflare 三样都没有，
...
| ~~D14~~ | ~~团队记忆用不用自家产品~~ | **已定（v32.6，人类决策）**：**既是也不是**。① 产品本体（含团队记忆）是一个真实 WorkspaceX 实例，在可移植层；② 运营扩展闭源、与①同栈；③ 边缘只承载投影与联邦。v32.5 的「不 dogfooding」是错的，已推翻。详见 `super-instance-design.md` |
