# WorkspaceX 开源方案（v32.2）—— 完整解决方案、各方体验与可执行门控
...
| 售卖 · 市场通道 | 签名、审核、结算、分成 |
| **不交付 · 内部运营平面** | 协调平面（coord-gateway 与 coord-* 包）、**devportal 的协作层与平台层**、发布与部署控制台、事故面板、agent 车队状态、CI 指标、状态页、密钥轮换台账 |
...
| 可以放（元数据与控制） | 理由 |
|---|---|
| 发布与部署控制台：各环境版本、部署历史、CD 触发与回滚、漂移探针 | 生产出事时正要用 |
| 可用性黑盒探针 | 从边缘探测比从生产内部探测更接近用户视角 |
| Andon 与事故面板：状态、谁在处理、时间线 | 事故时生产可能不可用 |
| PR 队列与合并门控（`pr-queue`、`merge-gate`、`doctor` 结果） | 数据源本来就在 GitHub，不在生产 |
| `readiness` / `dashboard` / `scorecard` 快照 | 现在只是本地跑加 `.harness/state` 文件，只有跑的人看得见 |
| agent 车队状态：锁、心跳、worktree 与 docker 清理 | 协调平面的自然延伸 |
| CI 与构建指标：时长、失败率、flake 排行 | 与客户数据无关 |
| 对外状态页与公告 | 状态页必须独立于生产 |
| 密钥轮换台账（哪个密钥、谁轮的、何时到期） | **只记元数据，绝不存值** |
| 功能开关与配额的控制面 | 下发目标是生产，控制面在这里 |

**绝不放**：任何客户内容（材料原件、报告正文、对话、转录、证据片段）；客户个人信息
（姓名、邮箱、组织通讯录）；任何凭据的值；计费权威账本（只读视图可以，钱的事实源不该活在专有存储里）；
客户要审计的证据（承诺的是可离线核验的证据包，放在我们自己的专有云里不好交代）。

**三块灰色地带先定规则再动**：用量只放聚合指标不放逐条事件；工单只放索引与状态、正文留可移植层；
客户组织名单只放不可逆 ID 不放名称。
