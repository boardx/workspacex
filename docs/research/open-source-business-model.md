# WorkspaceX 开源方案（v32.17）—— 完整解决方案、各方体验与可执行门控
...
| D4 | 技能包内容处置 | ~~运行时开源、内容与评测集闭源~~ **已定（v32.11，人类决策 D-EC1）**：分两类。①从开源上游导入的内容（Apache-2.0 / MIT）按上游许可**免费提供**，卖验证、本地化、托管执行与企业治理；②自研内容（本地化 playbook、行业评测集、组织档案模板）仍闭源售卖。详见 `PROP-EXPERT-COMMUNITY-001.md` §9 |
...
| ~~D27~~ | ~~技能包作者生态何时开放~~ | **已定（v32.11，人类决策 D-EC2）：立即开放**，不等 H2。开放路径与质量门见 `PROP-EXPERT-COMMUNITY-001.md` §5、§8 |
...
| **v32.11** | 2026-09-24 | **四个决策已定（人类决策）**：D1 Apache-2.0；D13 公开层拆独立域名；D16 只传运行事实；D17 同步投影进产品、权威仍是仓库。挡关键路径的决策全部清空，剩下的人类关口是签核、凭据确认与组织指派 |
| **v32.12** | 2026-09-24 | **更正上一版一句话**：v32.11 说 B6、C1「不需要人、可以直接开工」，不对——两者都藏着决策（主应用混有售卖功能；技能包内容在归属表里属售卖）。补三项人类决策 D18–D20；归属表落成代码单源 `lib/ownership.mjs`，按它给包标许可证并建门控；发现 vendored 的 `fabric-markdown` 没有记录上游许可证 |
| **v32.13** | 2026-09-24 | 再记四项人类决策 D21–D24：C3 开始（先盘点）；上报出厂默认只开健康信号；`cloud-deploy` 开源；DCO 只管外部。`cloud-deploy` 按归属标 Apache-2.0 并附正文 |
| **v32.14** | 2026-09-24 | 记人类决策 D25（配额开源）、D26（`ee-*` 边界）；建 `lint-ee-boundary` 并接入验证链；`apps/api`、`apps/web`、`dev-mode-accounts` 标 Apache-2.0 并附正文 |
| **v32.15** | 2026-09-24 | 人类指令下线投后管理报告与投后评级（#4012）：`maau-postinvest-report` 删除，归属表不再有售卖包，D18 与归属表的冲突随之消失 |
| **v32.16** | 2026-09-24 | **两项人类决策（出自 `PROP-EXPERT-COMMUNITY-001.md` §9）**：D4 改为分两类——开源导入内容按上游许可免费提供、自研内容闭源售卖（D-EC1）；新增 D27，技能包作者生态**立即开放**，技能包作者从 H2 提前到 H1（D-EC2），4.6 入口改为社区注册表仓库、负责人写成显式缺口。合并自 main：与本文既有 D18（8 个缺许可技能包的开源/售卖决策）编号冲突，新决策改记为 D27 |
| **v32.17** | 2026-09-24 | 4.6「谁负责」由显式缺口改为已定：项目发起人兼任生态负责人（D-EC9） |
