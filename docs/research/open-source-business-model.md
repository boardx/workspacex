# WorkspaceX 开源方案（v32.21）—— 完整解决方案、各方体验与可执行门控
...
| 承诺 | 门控 | 状态 |
|---|---|---|
| 每个技能包都能判定身份与许可 | `lint-maau-manifest` | 已建。R4 更正了它一半的误报；R12 给 8 个技能包补上 Apache-2.0（D18），存量清零，**已转 strict 接入验证链** |
...
| 开源包的许可证与归属表一致，且附官方正文 | `lint-package-license` | 已建（R11），接入验证链；5 个包归属未定，逐个列出且不许标许可证 |
| 外部贡献都签署 DCO | `check-dco` | 已建（R13）：`.github/workflows/dco.yml` 对 fork PR 逐个提交检查；仓库内 PR 不在范围 |
| 名称与 logo 不被 fork 冒用 | `TRADEMARKS.md` 政策（Apache-2.0 §6 不授予商标权） | 未建：政策已写（B8），权利人名称与联系邮箱仍是 `〔待定` 占位，属组织决策；是否冒用靠人不靠脚本 |
| 退出自由 | `export-org-data`（`apps/api/scripts/`），导出命令存在且 e2e 跑通 | 已建（E5）：按 `org_id` 自动发现租户表导出为 NDJSON + manifest（跳过的表逐张列原因），纯函数单测 + 真实 PG 用例；用法见 `docs/deployment/data-export.md`。「不断供」靠合同不靠脚本 |
...
代码标识符（`@repo/maau-*`、`capability_id`、目录名）**本版不动**——
...
| 7 | 技能包作者 | 供给 | — | **是**（v32.11 提前到 H1，人类决策 D-EC2） |
...
**v32.11**：技能包作者经人类决策提前到 H1（D-EC2，社区立即开放），H1 变为 6 类半；
它带来的运营负荷与负责人缺口见 4.6「谁负责」与 `docs/proposals/PROP-EXPERT-COMMUNITY-001.md` §8。
承诺不到的地方写清楚，比假装覆盖更可信。
...
| 格 | 内容 |
|---|---|
| 入口 | 社区注册表仓库（向它提 PR 提交技能包，见 `PROP-EXPERT-COMMUNITY-001.md` §8）；产品内能力库随 phase-15 接入 |
...
| 谁负责 | 项目发起人兼任生态负责人（2026-09-24 人类决策 D-EC9）；Claude 负责自动门控与初审，提交量上来后再任命专人 |
...
| 地平线 | 承诺哪些角色的体验 | 明确不承诺 |
|---|---|---|
| **H1 2026-2028** | 个人用户、买家、IT 与安全、最终读者、维护者、**技能包作者**（v32.11 提前）；首次贡献者与安全披露通道各做一半 | 业务采购方、被评估企业（底线合规除外）、伙伴、销售实施 |
| **H2 2028-2031** | 伙伴、业务采购方、销售实施 | — |
...
| ~~D1~~ | ~~核心许可证~~ | **已定（2026-09-24，人类决策）**：**Apache-2.0**。对外发布后不可逆；解锁 B6（license 字段）、B7（CLA/DCO）、B8（商标政策）与 C1（8 个技能包补许可） |
| ~~D18~~ | ~~8 个缺许可的技能包开源还是售卖~~ | **已定（2026-09-24，人类决策）**：**全部开源 Apache-2.0**，含 `maau-venture-valuation`（行业诊断）。⚠ 后者与本方案「付费只换行业内容」的默认原则不同，属人类明确的例外决策 |
| ~~D19~~ | ~~apps/api 与 apps/web 现在标不标 Apache-2.0~~ | **已定（2026-09-24，人类决策）**：**先不标**，等 C3 划出企业版边界 |
| ~~D20~~ | ~~贡献者协议 DCO 还是 CLA~~ | **已定（2026-09-24，人类决策）**：**DCO** |
| ~~D21~~ | ~~C3 企业版边界现在开始吗~~ | **已定（2026-09-24，人类决策）**：**开始，先盘点再拆**——第一轮只交拆分方案，人确认后才动代码 |
| ~~D22~~ | ~~上报四项同意的出厂默认值~~ | **已定（2026-09-24，人类决策）**：**只有健康信号默认开**，其余三项默认关；写进契约 `TELEMETRY_CONSENT_DEFAULTS` |
| ~~D23~~ | ~~`packages/cloud-deploy` 归哪一类~~ | **已定（2026-09-24，人类决策）**：**开源**——自托管客户要用；其中阿里云专属部分待改成通用写法 |
| ~~D24~~ | ~~仓库内部提交也签 DCO 吗~~ | **已定（2026-09-24，人类决策）**：**不用，只管外部**——内部贡献受雇佣或服务合同约束 |
| ~~D25~~ | ~~token 配额（F160–F162）开源还是售卖~~ | **已定（2026-09-24，人类决策）**：**开源** |
| ~~D26~~ | ~~企业版代码边界怎么划~~ | **已定（2026-09-24，人类决策）**：企业版代码放 `ee-*` 目录，由门控保证开源代码不依赖它；审计日志与平台管理留在开源版；据此 `apps/api`、`apps/web` 标 Apache-2.0 |
| ~~D27~~ | ~~S2 上报契约是否签核~~ | **已定（2026-09-24，人类决策）**：**照现稿签核**。`instance-telemetry.ts` 转 ACCEPTED 并正式导出；D9→D10→D11 解锁 |
| ~~D28~~ | ~~健康信号上报周期~~ | **已定（2026-09-24，人类决策）**：**每天一次**，契约常量 `TELEMETRY_REPORT_INTERVAL_SECONDS` 是唯一事实源 |
| ~~D29~~ | ~~投后清库脚本谁来跑~~ | **已定（2026-09-24，人类决策）**：**接进 `deploy.sh` 自动跑**（幂等，不删用户对话） |
| ~~D30~~ | ~~第一个价值时刻的时间预算~~ | **已定（2026-09-24，人类决策）**：**15 分钟**，从组织首次登录起（契约 `FIRST_VALUE_BUDGET_MINUTES`） |
| ~~D31~~ | ~~技能库收敛成哪三个入口~~ | **已定（2026-09-24，人类决策）**：**按场景**——查资料·读材料 / 用户与业务洞察 / 写给别人看；底层能力默认隐藏（契约 `skillEntryPoints`） |
| D32 | devportal 公开层用哪个域名 | **未定**（2026-09-24 人类：先不定）。拆分代码已合入但不生效，CD 以占位放行并告警 |
| ~~D33~~ | ~~E1 价值时刻漏斗契约是否签核~~ | **已定（2026-09-24，人类决策）**：**照现稿签核**；漏斗计数并入 S2 `usage` 分节（`TelemetryUsage.firstValueFunnel`）。E3 解锁 |
| D6 | 贡献协议 | 取决于 D1：DCO 摩擦小但放弃改许可证的权利 |
| D2 | 仓库结构 | 单仓公开，`ee/` 商业许可 |
| D3 | EE 切线深度 | 仅治理与合规，**承诺只进不退** |
| D4 | 技能包内容处置 | ~~运行时开源、内容与评测集闭源~~ **已定（v32.11，人类决策 D-EC1）**：分两类。①从开源上游导入的内容（Apache-2.0 / MIT）按上游许可**免费提供**，卖验证、本地化、托管执行与企业治理；②自研内容（本地化 playbook、行业评测集、组织档案模板）仍闭源售卖。详见 `PROP-EXPERT-COMMUNITY-001.md` §9。**与 D18 的冲突已裁决（2026-09-24，人类决策）：D18 优先**——仓库里现有的 8 个技能包（含自研的行业诊断）全部开源 Apache-2.0；② 只约束今后新写的自研内容 |
...
| **D-EC2** | **技能包作者生态何时开放** | **已定（v32.11，人类决策 D-EC2）：立即开放**，不等 H2。开放路径与质量门见 `PROP-EXPERT-COMMUNITY-001.md` §5、§8 |
| ~~D13~~ | ~~devportal 公开层拆域还是砍掉~~ | **已定（2026-09-24，人类决策）**：**拆出独立域名**。公开层与协作层在归属、受众、可见性、数据源上都相反；现状整域 Access 门禁，公开层公众到不了。解锁 F1 |
...
| ~~D16~~ | ~~联邦层运营的是运行事实，还是客户内容~~ | **已定（2026-09-24，人类决策）**：**只传运行事实**。客户内容与个人信息一律不上报；实例主动上报，运营面对实例零读权限。R7 起草的上报契约方向确认，解锁 S3（D9） |
| ~~D17~~ | ~~平台大脑开发过程那一半放哪~~ | **已定（2026-09-24，人类决策）**：**同步投影进产品，权威仍是仓库文件**（`super-instance-design.md` §2.2 读法 B）。解锁 D12 |
...
| **v32.11** | 2026-09-24 | **两项人类决策（出自 `PROP-EXPERT-COMMUNITY-001.md` §9）**：D4 改为分两类——开源导入内容按上游许可免费提供、自研内容闭源售卖（D-EC1）；新增 D-EC2，技能包作者生态**立即开放**，技能包作者从 H2 提前到 H1，4.6 入口改为社区注册表仓库、负责人写成显式缺口 |
| **v32.12** | 2026-09-24 | 4.6「谁负责」由显式缺口改为已定：项目发起人兼任生态负责人（D-EC9） |
| **v32.13** | 2026-09-24 | **四个决策已定（人类决策）**：D1 Apache-2.0；D13 公开层拆独立域名；D16 只传运行事实；D17 同步投影进产品、权威仍是仓库。挡关键路径的决策全部清空，剩下的人类关口是签核、凭据确认与组织指派 |
| **v32.14** | 2026-09-24 | **更正上一版一句话**：v32.13 说 B6、C1「不需要人、可以直接开工」，不对——两者都藏着决策（主应用混有售卖功能；技能包内容在归属表里属售卖）。补三项人类决策 D18–D20；归属表落成代码单源 `lib/ownership.mjs`，按它给包标许可证并建门控；发现 vendored 的 `fabric-markdown` 没有记录上游许可证 |
| **v32.15** | 2026-09-24 | 再记四项人类决策 D21–D24：C3 开始（先盘点）；上报出厂默认只开健康信号；`cloud-deploy` 开源；DCO 只管外部。`cloud-deploy` 按归属标 Apache-2.0 并附正文 |
| **v32.16** | 2026-09-24 | 记人类决策 D25（配额开源）、D26（`ee-*` 边界）；建 `lint-ee-boundary` 并接入验证链；`apps/api`、`apps/web`、`dev-mode-accounts` 标 Apache-2.0 并附正文 |
| **v32.17** | 2026-09-24 | 人类指令下线投后管理报告与投后评级（#4012）：`maau-postinvest-report` 删除，归属表不再有售卖包，D18 与归属表的冲突随之消失 |
| **v32.18** | 2026-09-24 | 裁决 D4（D-EC1「自研内容闭源售卖」）与 D18（8 个技能包全部开源）的冲突：人类决定开源，D18 优先，D-EC1 ② 只约束今后新写的自研内容 |
| **v32.19** | 2026-09-24 | 记人类决策 D27（S2 签核）、D28（每天上报）、D29（清库接进部署）；新增商标政策（B8） |
| **v32.20** | 2026-09-24 | R17 批次：记 D30、D31、D32（未定）；backlog B8 C8 D1 D9 D10 D12 E1 E2 E4 E5 E6 E7 F1 推进 |
| **v32.21** | 2026-09-24 | 记 D33；R17 第三批：D2 D3 D4 D6 D7 D11 与 D9 设置页 |
