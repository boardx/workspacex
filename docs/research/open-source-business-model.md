# WorkspaceX 开源方案（v32.5）—— 完整解决方案、各方体验与可执行门控
...
实测 18 个技能包，**12 个有问题，共 18 处**，而且缺口呈现出一个结构性形状：
...
**清单的两半分裂在两批人身上，没有一个技能包两样俱全。**
自研的声明身份不声明许可，外来的声明许可不声明身份。也就是说在今天，
**没有任何一个技能包能机械回答它是开源还是付费**——而整个商业模式建立在这个区分上。
...
| 运营平面不含客户内容 | 运营面 schema 白名单门控 | 未建 |
| 运营平面不含个人信息 | 字段级检查：人名/邮箱/电话形态即红 | 未建 |
| 生产不依赖运营平面 | 依赖方向检查 | 未建 |
...
| **团队记忆** | **我们自己的知识库**：ADR、sprint 历史、方法论、踩坑经验、GTM 与案例知识 | **组织大脑**是**产品概念**，客户把产出物沉淀进本组织知识库的能力（`brain-promotion`、`batchConfirmAndWriteBackToBrain`） |
...
| 售卖 · 市场通道 | 签名、审核、结算、分成 |
| **不交付 · 内部运营平面** | 协调平面（coord-gateway 与 coord-* 包）、**devportal 的协作层与平台层**、发布与部署控制台、事故面板、agent 车队状态、CI 指标、状态页、密钥轮换台账、**GTM 活动与运营元数据、对外案例、团队记忆**（三者的数据边界见下节） |
...
| **② 运营元数据** | 部署与事故、PR 队列与门控、readiness/dashboard/scorecard 快照、agent 车队状态、CI 指标、密钥轮换台账（只记元数据）、功能开关控制面、**GTM 活动排期与漏斗统计、归因、A/B 结果、商机阶段与金额、客户组织的不可逆 ID**、**团队记忆** | **Cloudflare** |
...
| | 产品的组织大脑（客户的） | 团队记忆（我们的） |
|---|---|---|
| 内容 | 客户的材料、结论、决策链 | 我们的 ADR、sprint 历史、方法论、经验沉淀 |
| 承载 | `knowledge-ontology.md` 的四张 PG 表，每行带 tenant/privacy/permission，RLS 隔离 | **运营平面（Cloudflare）**，D14 已定 |
...
**已改名（v32.4）：我们自己那份叫「团队记忆」**，并在 `PROJECT.md` 登记为单一事实源。
同批把商业与分发单元从此前的 MAAU 改叫**技能包**——见下节术语。

### D14 已定：运营平面装的是**我们自己的私有数据**，不 dogfooding

（2026-09-23 人类决策）这个后台放的是**我们产品的私有数据**——闭源的那一面：
团队记忆、方法论、GTM 与案例知识、商机与漏斗、运营元数据。不跑在自家产品栈上。

**必须划清一条界，否则「我们产品的私有数据」这句话会被读成允许放客户数据。**
「私有」在这里指**权属与开放度**，不是指数据敏感等级：

| 「我们产品的私有数据」**是** | **不是** |
|---|---|
| 我们自己产生、自己是控制者的数据：ADR、方法论、踩坑经验、GTM 活动与漏斗、对外案例、商机阶段与金额 | 客户在产品里产生的内容（组织大脑里的材料、结论、决策链）——那是**受托保管**，不是我们的 |
| 闭源不交付的那一面：运营控制台、协调平面、车队状态 | 客户的个人信息，以及我们自己 CRM 里的人名/邮箱/电话/沟通原文——**归第 ③ 层，留境内** |
| 计价与用量的**聚合**、客户组织的不可逆 ID | 计费的**账本正本**、客户可审计的证据 |

换句话说：**权属是我们的 → 可以上；受托于客户的 → 不能上；是自然人的 → 不管权属归谁都留境内。**
前两栏的划法不覆盖第三条——我们自己的 CRM 联系人虽然权属是我们的，仍然归第 ③ 层。

**这条没有推翻 v32.3 的放置决定，反而把它坐实了**：团队记忆确定落在运营平面，
上一节「承载：待定」那一格可以填上了。

**代价要说在前面**：一个卖 AI 工作台的公司，自己最重要的知识不放在自己做的组织大脑里。
客户问起来这是个真问题，需要一个能说出口的答案——最可能的那个是「组织大脑是给协作团队
沉淀会议与产出物的，我们的团队记忆是工程与 GTM 资产，两者形态不同」。
**这个答案是否站得住，本方案不替销售拍板**，但它现在是一个要准备的问答，不是一个悬着的架构问题。
...
| ~~D14~~ | ~~团队记忆用不用自家产品~~ | **已定（v32.5，人类决策）**：**不 dogfooding**。运营平面装我们自己的私有数据（权属是我们的那一面），团队记忆确定落在 Cloudflare。「私有」指权属与开放度，**不是**敏感等级——受托于客户的不能上，自然人的个人信息不管权属归谁都留境内。代价：需准备「为什么自己不用组织大脑」的销售问答 |
| ~~D15~~ | ~~团队记忆叫什么~~ | **已定（v32.4）**：团队记忆；商业与分发单元此前叫 MAAU → 技能包 |
...
| 周 | 做什么 | 验收 |
|---|---|---|
| 1–2 | 补齐 18 处技能包清单缺口，门控转 `--strict` | `lint-maau-manifest --strict` 绿 |
