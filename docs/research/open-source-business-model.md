# WorkspaceX 开源方案（v32.4）—— 完整解决方案、各方体验与可执行门控
...
| 承载 | `knowledge-ontology.md` 的四张 PG 表，每行带 tenant/privacy/permission，RLS 隔离 | 待定 |
...
同批把商业与分发单元从技能包改叫**技能包**——见下节术语。

⚠ **一个尚未回答的问题**：团队记忆，**是不是就用我们自己的产品**？
如果是（dogfooding），它就跑在产品栈上而不在 Cloudflare——这与本节的决定冲突，需要重新选。
如果不是，等于说「我们最重要的知识不放在自己做的组织大脑里」，
这对一个卖 AI 工作台的公司是个需要正面回答的问题。见决策 D14。
...
| **D14** | **团队记忆用不用自家产品** | 用了它就跑在产品栈上、不在 Cloudflare；不用则要回答「为什么自己不用」。**这条未决，且会推翻上面的放置决定** |
| ~~D15~~ | ~~团队记忆叫什么~~ | **已定（v32.4）**：团队记忆；商业单元技能包 → 技能包 |
