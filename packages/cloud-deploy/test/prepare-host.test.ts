...
vi.mock("../src/trusted-tree", () => ({ assertTrustedTree: vi.fn() }));
...
it("prepares private canonical files and records first-release AppArmor ownership",async()=>{const f=await fixture();const result=await prepareHost(f.config,f.manifest,f.options,f.services,f.source);expect(result.readyForProvision).toBe(false);expect(result.cloudVerified).toBe(false);expect(result.status).toBe("files-ready-ingress-installation-required");expect(f.calls.filter(a=>a[1]==="pull")).toHaveLength(6);expect(await readFile(join(f.options.runtimeDirectory,"ingress/private-key.pem"),"utf8")).toBe(privateKeyPem);expect((await stat(join(f.options.runtimeDirectory,"ingress/private-key.pem"))).mode&0o077).toBe(0);const owner=JSON.parse(await readFile(join(f.root,".workspacex-apparmor-owner.json"),"utf8"));expect(owner).toEqual({schemaVersion:1,installationId:result.installationId,profile:"workspacex-native-sessions"});expect((await stat(join(f.root,".workspacex-apparmor-owner.json"))).mode&0o077).toBe(0);expect(JSON.stringify(result)).not.toContain("BEGIN PRIVATE KEY");expect(f.calls.some(a=>["systemctl","nginx","apt","yum"].includes(a[0]!))).toBe(false);});
...
it("allows a second release runtime owned by the same installation to replace the global profile",async()=>{const f=await fixture(),config=deploymentExample("production");const first=await prepareHost(config,f.manifest,f.options,f.services,f.source);const secondOptions={...f.options,runtimeDirectory:join(f.root,"runtime-next")};const second=await prepareHost(config,f.manifest,secondOptions,f.services,f.source);expect(second.installationId).toBe(first.installationId);expect(f.calls.some(a=>a[0]==="apparmor_parser"&&a.includes("--replace"))).toBe(true);expect(await stat(join(secondOptions.runtimeDirectory,"prepare-receipt.json"))).toBeTruthy();});
it("rejects another installation trying to take over an existing global AppArmor profile",async()=>{const first=await fixture();await prepareHost(first.config,first.manifest,first.options,first.services,first.source);const foreign=await fixture();foreign.setProfile("workspacex-native-sessions (enforce)\n");await expect(prepareHost(foreign.config,foreign.manifest,foreign.options,foreign.services,foreign.source)).rejects.toThrow("APPARMOR_PROFILE_ALREADY_EXISTS");expect(foreign.calls.some(a=>a[0]==="apparmor_parser"&&(a.includes("--replace")||a.includes("--add")))).toBe(false);});
...
it("rejects an untrusted checkout tree before running Git or mutating the host",async()=>{const f=await fixture();vi.mocked(assertTrustedTree).mockRejectedValueOnce(new Error("UNTRUSTED_RELEASE_TREE"));await expect(prepareHost(f.config,f.manifest,f.options,f.services,f.source)).rejects.toThrow("UNTRUSTED_RELEASE_TREE");expect(f.calls).toHaveLength(0);});
