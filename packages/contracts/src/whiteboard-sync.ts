...
export const WhiteboardSoakRun = z.object({ runId: z.string().uuid(), exactSha: z.string().regex(/^[a-f0-9]{40}$/), environmentFingerprint: z.string().regex(/^[a-f0-9]{64}$/), purpose: WhiteboardSoakPurpose,requiredDurationMs:z.number().int().positive(),requiredOfflineMs:z.number().int().positive(),expectedClients:z.number().int().positive(),expectedWriters:z.number().int().positive(),expectedReconnects:z.number().int().nonnegative() }).strict();
...
export const WhiteboardSoakLedgerPayload = z.object({
  ...
  exactSha: z.string().regex(/^[a-f0-9]{40}$/), environmentFingerprint: z.string().regex(/^[a-f0-9]{64}$/),requiredDurationMs:z.number().int().positive(),requiredOfflineMs:z.number().int().positive(),expectedClients:z.number().int().positive(),expectedWriters:z.number().int().positive(),expectedReconnects:z.number().int().nonnegative(),startedAtMs:z.number().int().nonnegative(),finishedAtMs:z.number().int().nonnegative(),
