 ...
 *   共用这一份，**不许各自定义**。`lint-omission-reason.mjs` 机械检查：
 *   代码里不得出现表外的 reason 字面量。
 *
 * ⚠ **2026-07-28 迁位（一致性复核 B-5）**：本文件原在 `apps/web/lib/`，
 *   导致 `packages/contracts` 用 `../../../apps/web/...` 反向 import——
 *   **契约包依赖 app 包，方向反了**。契约是 app 的上游，不能倒过来。
 *   现迁入契约包；`apps/web/lib/omission-reason.ts` 改为**再导出**，既有 import 路径不变。
 */
