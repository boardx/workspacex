import { z } from "zod";
import { McpRunSnapshotView } from "./mcp-runtime-snapshot";
export const NATIVE_SESSION_RESOLVE_MAX_BYTES=2*1024*1024;
...
export const NativeSessionResolved = sandbox.created.extend({ interruptOn: z.record(z.boolean()), packageDigest: z.string().regex(/^[a-f0-9]{64}$/), inputs: NativeInputManifest.default([]), mcpSnapshot: McpRunSnapshotView.optional() }).strict();
