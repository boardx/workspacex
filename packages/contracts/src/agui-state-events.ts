 ...
 *
 *      ⚠ **issue #3389 —— 这条承诺曾被短暂收窄过，实测反证后原样恢复，留档在此。**
 *      当时的想法是：多气泡轮次（#3243 的分步产出）没有任何单条气泡等于落库那行，
 *      按字面执行等号只能退回撤回重发，于是改成「这些气泡拼起来逐字等于落库那行 ⇒
 *      映射指向最后一条」，理由是「把用户看到的字和库里的行对错」已结构性不可能。
 *
 *      **实测推翻**（chat-read 车道，同 SHA 基线对照）：不撤回意味着 wire 上留下 N 条
 *      气泡，而本事件只有**一个** `streamingMessageId` 字段——一条映射只能认领一条气泡。
 *      前端 `restore-final-messages.ts` 的权威读于是只换掉被认领的那条，**其余原样留着**，
 *      一轮里同一段话出现两次。三条在基线上绿的 spec 当场转红
 *      （基线 3 failed/117 passed → 带该收窄 7 failed/113 passed）。
 *
 *      ⇒ 这条承诺背后不是措辞，是「一条映射只能认领一条气泡」这个协议事实。要让多气泡
 *      轮次也不闪，得先让映射能认领多条（协议扩展），那是另一个 issue。
 *
