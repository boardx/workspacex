 ...
 *
 *      ⚠ **issue #3389 起，这条承诺守护的那个危险已经结构性消失，措辞相应收窄。**
 *      `finish()` 现在只在「wire 上已经流出去的那些气泡，按
 *      `assistant-body-composition.ts` 的 `composeAssistantBodies` 拼起来**逐字等于**
 *      落库那行」时才走这条不撤回的路径——而落库那行本来就是用**同一份**函数拼的
 *      （`joinTurnAssistantBodies`）。于是每条气泡的正文都是落库那行逐字包含的一段，
 *      落库那行也不含任何用户没看见的字：「把用户看到的字和库里的行对错」**不可能
 *      发生**，不是「相信它不会发生」。拼不出等号（两份正文真的分叉）时照旧撤回重发。
 *
 *      因此：单条气泡自己就承载整段落库正文时，映射照旧指向它（等号原样成立）；
 *      多条气泡各承载一段时（#3243 的分步产出）映射指向 wire 上**最后一条**气泡，
 *      **不再**为了凑出等号去撤回重发——那次撤回正是 #3389 里用户看到的那段 ~1.07s 空白。
 *      第 2 条那道红线（不得制造自映射）一字未动，见 `execution-journal-relay.ts` 的
 *      `finish()`。本收窄已在 issue #3389 上留档待复核。
