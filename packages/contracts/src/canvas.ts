 ...
 *
 * - `cols`：贴纸横向列数，1 列＝竖排长列表，10 列＝密集小方格，纯排版偏好。
 * - `max`：最多渲染条数（同时是提示词的条数上限），上限宽裕但不失控。
 * - `tone`：贴纸色板下标（0 黄 / 1 粉 / 2 绿 / 3 蓝），色板本体在前端。
 *
 * 2026-09-03 人类反馈（issue #2604）：三视角模型/黄金圈法则/汉堡沟通模型三个内置
 * 模板，列数设到 8（旧上限）、条数 8 时想再加 2 条到 10，第 9 条却自动换列/换行——
 * 根因就是这里的 `cols.max` 封死在 8，编辑器「显示方式 · 列数」选不到 9/10，
 * 使用者没有办法把一整行摆满 10 条。上限放宽到 10，`COLS_OPTIONS` 派生自这个常量
 * 会自动跟着变成 1–10，不需要在别处再改一份数字。
 */
