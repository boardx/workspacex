...
/**
 * 邮箱地址的**共享形状**——与注册 / 找回口令等 auth 操作里 `z.string().email()` 同一语义
 * （zod 内建 email 判定：拒绝 `a@`、`@b`、无点域名 `a@b`）。
 *
 * 为什么单独导出：前端表单校验（如组织邀请）需要和契约完全同一份判定，不允许在前端
 * 手写第二份邮箱正则（AGENTS.md「同一事实不得声明在两处」）。这是**纯加法导出**，
 * 不改变任何已签核操作的 in/out 形状；`org-admin` 的 `inviteOrgMember.in.email` 目前
 * 仍是 `z.string().min(1)`，服务端权威校验是否收紧为本形状，待人类裁决后另行处理。
 */
