...
export const sourceConnectionExchanges = {
  ...
  acceptGithubSourceCallback: z.object({
    ...
    stored: z.object({ state: z.string().min(32).max(512), consumed: z.literal(false), transaction: GithubSourceConnectionTransaction.refine(t => t.status === "pending") }).strict(),
    response: operations.acceptGithubSourceCallback.out,
  }).strict().refine(({ request, stored, response }) => request.state === stored.state && sameContext(stored.transaction, response) && response.transactionRevision === stored.transaction.transactionRevision + 1, "callback must consume the state-resolved pending transaction exactly once"),
