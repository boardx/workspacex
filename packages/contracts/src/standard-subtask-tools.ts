...
import {SubtaskRunStatus} from './subtask-run';
...
export const SubtaskSpawnInput=z.object({description:z.string().trim().min(1).max(STANDARD_SUBTASK_LIMITS.maxDescriptionChars),
 ...
 idempotencyKey:z.string().min(1).max(256).describe('Stable logical dispatch key; reuse only for the identical description and context references.')}).strict();
