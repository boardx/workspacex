 ...
 * 本地版的**出网事实** —— 三类，逐条说清楚。
 *
 * ## 为什么必须有这一份，而不是一句「请求不出网」
 *
 * 那句话**是假的**。#3716 起本地版刻意挂上了 `fetch_url` / `web_search`（人类当时的
 * 需求就是「本地版也要能读网页」），而它们必然出网：`fetch_url` 由 API 进程直接去取
 * 那个页面（`standard-web-service.ts`，且那条路径显式传 `localOnlyOrg: false`，不走
 * 个人本地组织的出站守卫）。本文件第一版的顶栏文案逐字写着「请求不出网」——那正是
 * 本仓一再要治的那种「写下来就不会再变、读起来却像权威」的假承诺。
 *
 * 所以这里不是一句宣传语，是一张**分类表**：什么永远不出、什么只在你明确要求时出、
 * 什么被挡住。界面逐条渲染它（每条一个 `data-testid`），反证才打得准。
 */
export const LOCAL_EGRESS_FACTS = [
  {
    id: "never",
    when: "永远不出网",
    statement: "模型推理、对话内容、画布与文件、数据库都在这台电脑上。断网照样能用。",
  },
  {
    id: "on-request",
    when: "只在你明确要求时",
    statement: "你让它读某个网址或搜索时，这一次请求会真的出网——那个网址或搜索词会离开这台电脑。不用这两个工具就不会发生。",
  },
  {
    id: "blocked",
    when: "被挡住",
    statement: "云端大模型、AI 出图服务、MCP 外部工具、邮件外发在本地版一律不可用，不是没配而是关掉的。",
  },
] as const;
