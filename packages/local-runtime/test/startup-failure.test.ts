...
import { diagnoseStartupFailure } from "../src/startup-failure";
...
describe("端口被上次的自己占着（实测 8 次）", () => {
  it("给出「收回并重试」，并带上那个端口", () => {
    const d = diagnoseStartupFailure(REAL.portTaken, { hasBackup: false });
    expect(d.unknown).toBe(false);
    expect(d.remedies[0]).toEqual({ kind: "reclaim-port", port: 55432, label: "收回并重试" });
  });

  it("端口号换了也认，且说的是那个服务对用户的名字", () => {
    const d = diagnoseStartupFailure(REAL.sandboxTaken, { hasBackup: false });
    expect(d.remedies[0]).toMatchObject({ kind: "reclaim-port", port: 3310 });
...
describe("不认识的错误", () => {
  it("如实说不认识，不编一个按钮出来", () => {
    const d = diagnoseStartupFailure("EACCES: permission denied, mkdir '/nope'", { hasBackup: true });
