...
export const LOCAL_CAPABILITY_BASELINE: readonly LocalCapability[] = [
  ...
  },
  {
    ...
    label: "逐字输出（流式）",
    state: "unavailable",
    because: "流式是按部署显式开启的行为，本地版尚未开启。",
    remedy: "已在本轮迭代中处理（见 R6）；开启后本地慢模型也会逐字出字。",
    envKeys: ["KERNEL_MODEL_STREAM_ENABLED", "KERNEL_DEEP_AGENT_STREAM_ENABLED"],
  },
  {
    id: "thread-titles",
    label: "会话自动命名",
    state: "unavailable",
    because: "自动命名是按部署显式开启的模型行为，本地版尚未开启。",
    remedy: "已在本轮迭代中处理（见 R6）。",
