...
| 3 | 工具权限确认弹层（三选一） | 03-plan-mode R3.5 / R8 | `awaiting_tool_permission` | `03-tool-permission-card.png` |
...
5. **三个权限按钮的视觉权重**：仅本次/本 run 内都用 outline（对等），拒绝用 destructive。
...
- **03 R8 说权限弹层三选一含「本次 run 内都允许」，但 R5 授权粒度是「单次/本 run 内/组织默认」三档**，
  而 00-overview 明确「组织级默认策略后台不在本 phase」。我在弹层只放前两档（单次、本 run 内），
  未放「以后都允许（组织默认）」——与 R8 的三按钮字面不完全一致，取 00-overview 的范围裁剪为准。
  **若本 phase 仍需运行时触发「以后都允许」入口（只是不做管理后台），需补第三个按钮。**

## 建议人类在束级 design-signoff.md 第 ① 件重点核对的 3 处

1. **权限弹层的按钮档位**（上文矛盾第 2 条）：到底是两档还是三档？这直接决定后端授权存储的枚举，
   是 UI/UC/API 三件必须对齐的点，错了会级联到契约。
