...
2. POST persists one human message/run across identical retries, rejects changed
   payloads, and cursor pagination has no duplicate or missing IDs;
...
4. the fixed Agent/Skill/model snapshot performs exactly one no-tool model call,
...
5. the browser completes registration confirmation, sends a Chat message, sees
