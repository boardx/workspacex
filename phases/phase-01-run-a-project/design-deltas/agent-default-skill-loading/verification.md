...
`apps/web/e2e/copilotkit-v2-skill-mount.spec.ts` 第一条：登录、什么都不挂、发一条消息，
deep-agent 替身回复里出现 `[skill:]MOUNTPROOF-9317`；再经工具行入口挂同一个 skill，第二轮
照样有哨兵、run 不失败。

⚠ 反证：把 `acceptHumanMessage` 里 `orgEnabled` 恒置 `[]`，第一轮哨兵消失，红。
