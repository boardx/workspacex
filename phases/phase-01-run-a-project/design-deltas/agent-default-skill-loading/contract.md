...
- **结论**：用户不挂、agent 不钉 ⇒ 模型收不到任何 skill（#2509 「`list_org_skills`
  恒回没有挂载任何技能」正是这个现象）。
...
- v2 轨道的「技能」入口保留（#2517）：在默认加载之上挂一个已带上的 skill 是幂等的，
  对钉了 skill 的 agent 是临时加一个。
...
6. e2e：`copilotkit-v2-skill-mount.spec.ts` 第一条与 `chat-agent-skill-context.spec.ts`
   第二条都按 §4 改写成「不挂即有、再挂幂等」（旧的挂载前后对照在新规则下不可能成立）。
