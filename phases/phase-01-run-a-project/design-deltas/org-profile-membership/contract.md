...
## 4. 需要你先拍板的两件 —— 已签（2026-08-09，人类确认采纳建议方案）

**① 成员/邀请列表谁能读？→ 成员列表任何组织成员可读；邀请列表限组织 admin。**
理由：知道同事是谁是常见协作需求；未接受的邀请邮箱属于更敏感的信息，收紧到 admin。
⇒ `listOrgMembers` 授权为"任意组织成员"，`listOrgInvites` 授权为"组织 admin"——**两个操作的
授权级别不同**，实现时不要图省事共用一个授权检查。

**② 组织简介的内容边界？→ 纯文本，不支持 markdown。**
`description` 存储与展示均按纯文本处理，不做 markdown 渲染/sanitize。markdown 支持留给
后续一轮，不是本 delta 范围。
