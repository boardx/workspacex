...
### 第 17 轮（2026-08-10，实测 SHA `23416846`）—— rev-uiux 独立评分 **P 组 10/10，首次满分** 🎉

#### 硬门四道全过（评分员自己重跑全部取证）
`verify:base` 39/39 exit 0（load 1.26 起跑）、`verify:chat-read` 3 passed、
`shots:chat-main` 13 张真栈截图、参照 PNG 与原型 html 相对 round 16 零改动
（只有本日志追加了 71 行，H4 判过）。

#### P10 转正——含一次真实点击取证
round 16 的「落地为产物」假按钮修复（PR #872）经独立确认：四张个人屏截图
（reply/tool-call/failure/mobile-detail）里按钮不再出现；且评分员做了
round 16 评分员自报缺失的那一步——**在项目侧真实点了一次落地按钮**：
facilitator 填标题确认 → `POST /artifacts` 返回 200
（`artifactId art-899b10f1…`）→ reload 后产物标题仍在。
「按能力渲染」没有被写歪成「一律不渲染」，两侧都证死了。

#### 评分员留下的非扣分观察（后续轮次/人类参考）
1. 个人线程列表把服务端 `groups` flatMap 成一维，丢了项目侧的
   「今天/本周」分组标题——夹具只有一条线程看不出，建议造跨天线程取证。
2. 顶栏「项目负责人」（组织角色）与「项目角色不适用」同屏矛盾——
   全站 AppShell 术语问题，已在 `CHAT-728-PENDING-DECISIONS.md` 列过。
3. 落地按钮的失败路径（`chat-land-artifact-error-*`）未触发过。
4. 真实 provider（非 loopback 替身）下的流式节奏/工具调用形态在评分
   视野之外——loopback 证明的是渲染路径真实，不是真模型下同样好看。

#### 里程碑
从 round 0 的 D/P 各 1/10 起，17 轮迭代、三次基础设施假阴性确诊、
两次评分冲突不选边、三次人类裁决（#831 门槛 9 分、P7 路径 B、H3 根因修复
方向），P 组按最严判据（H3 绝对绿 + 全部独立评分）达到 10/10。
D 组（项目对话）按用户指示仍暂停，为 main 上记录的 1/10。
