...
> **① ② 已由人类于 2026-08-26 裁决**（各节顶部有逐字转写 + 落地形态，原对照转为决策档案）。
> **③ 是这两条裁决的产物**（不是待决项）。**④ ⑤ 仍未定**——④ 要人类拍一下阈值，
> ⑤ 是实现期探针，不是设计裁决。
...
### ① ✅ **已裁决（人类，2026-08-26）：A —— system 消息注入**
...
> （那次调用的 system prompt 是 skill 正文——#1747 当初为 `script_protocol`
> 选 `configurable` 的原话，见 `deep-agent-model-provider.ts:762-769`）。
> 这不是遗漏，是人类知情后接受的边界。
>
> ⇒ 下面的 A/B 对照**保留为决策档案**，不是待办。B 若将来被真实需求逼出来，
> 从这里接着读。

### ①（档案）「加约束」需要改 Python 服务，还是只改 Node 侧
...
### ② ✅ **已裁决（人类，2026-08-26）：(c) —— 本轮不做，TW-P0-3 封顶 0.7**

> **裁决逐字**：「**(c) 先记 0.7，不做**（采纳你的推荐；**TW-P0-3 封顶 0.7 是他知情后接受的**）」。
>
> **这次裁决是在知情状态下做的**，知情的内容逐条是：
> ① `agent-runtime` 的 `replayAgentRun` **不是**「从 checkpoint 继续跑」，
>    「重放一次已结束的 run」是另一件事；
> ② 那个束**自己**把这条标成 `coverage.md:249` 的**缺口 25**（跨 phase-00 `context-pack`）；
> ③ 引擎侧原语实存（`GET /threads/{id}/history` + `POST /threads/{id}/state` 带
>    `checkpoint_id`，`langgraph_api/api/threads.py:555-575` 实测），**但本仓一行没接**；
> ④ 做它就得触碰已签核的 `agent-runtime` 束。
>
> ⇒ **落地形态**：`RunControlAction` 收敛为三值（第一节 7）；`usecases.md` 的
> `UC-11 restoreCheckpoint` **整条删除**，不留一个恒失败的接口；
> `ui.md` 的 `chat-task-workbench-failure-restore-checkpoint` **按钮不渲染**
> （渲染一个点了报错的按钮 = 反伪造条款的死按钮，判 0）。
>
> ⚠ **这不是把做不到的事写进契约再假装能做，也不是把判据改松。**
> 判据六**没有变**，它仍然要求三个恢复动作；我们**明确选择不做第三个**，
> 缺口登记在 `coverage.md` 缺口 4，TW-P0-3 的分数如实封顶 **0.7**。
>
> ⇒ 下面的三候选对照**保留为决策档案**，是这次知情过程的原始材料。

### ②（档案）`restore-checkpoint`（判据六第三个恢复动作）**指不到任何契约操作**
