...
⚠ **这意味着人类现在有两个选择，请在 `design-signoff.md` ① 节里明示选了哪个**：
- **先补画**：让 `ui-prototyper` 产出下面第三节点名的 8 屏，再签；
- **先签 ②③、① 留 pending**：接受「界面细节后补」，但要留下书面理由——
  ADR-003 想防的正是「feature_list 在任何人看到真实界面之前就被定成权威」。
...
| 恢复检查点 | `chat-task-workbench-failure-restore-checkpoint` | **UC-11** | 🔴 **契约悬空，见下** |

🔴 **「恢复检查点」这个按钮当前指不到任何可用契约**（`domain.md` 第三节 ②）。
三个候选里若选 (c)，**这个按钮不渲染**，锚点不存在，e2e 如实报缺口。
**绝不渲染一个点了报 `RESTORE_NOT_IMPLEMENTED` 的按钮**——那是反伪造条款的死按钮，判 0。
...
| **G-08** | 失败态三个恢复动作（按 (c) 则只画两个） | S6 | 三个候选选哪个直接改这张图 |
...
- [ ] **「恢复检查点」按钮渲不渲染**——取决于 `domain.md` 第三节 ② 的三选一。
