...
> 本文件**不逐张引用 v1 的任何一张**。
>
> 核对办法（`cd phases/phase-01-run-a-project`）：数 `ui-preview/itv-v2` 目录下的 `.png` 文件数 ⇒ **62**；
> 在 §四·1 的索引表里抓出全部 `itv-v2/xxx.png` 文件名去重后计数 ⇒ **62**；两者的差集应为空。
...
|---|---|---|---|
| **G-②** | ⚠ 未产出：**U-5 受访者授权页**（四项独立勾选 + 两条降级文案 + 数据控制方三项 + `[全部拒绝]`/`[确认并进入访谈]` 同等权重）—— 该屏尚未画 | ~~`ui-preview/itv-consent-page.png`~~ | 🔴 **最高** |
| **G-③** | ⚠ 未产出：**U-6 研究员侧受访者名单与只读镜像**（同意书三态列无勾选控件 + `已拆场` 标签 + 七开关）—— 该屏尚未画。🔴 **v1 画过 8 张（`uc-6-3-受访者名单-*`），但 v1 已被推翻 ⇒ 净损失，现在无任何可签材料** | ~~`ui-preview/itv-subject-roster.png`~~ | 🔴 **最高** |
| **G-⑤** | ⚠ 未产出：**U-9 受访者自助门户**（三动作区并列 + 请求进度 + 删除确认页四段说明）—— 该屏尚未画 | ~~`ui-preview/itv-subject-portal.png`~~ | 🔴 |
| **G-⑥** | ⚠ 未产出：**U-10 项目组卡内的观察/访谈对象表**（六列 + 掩码联系方式 + `[AI 建议人选]` 候选卡三出口 + 预约草稿）—— 该屏尚未画（v2 只在新建向导第 2 步做了**投影**，非主入口） | ~~`ui-preview/itv-subject-table.png`~~ | 🟠 |
| **G-⑦** | ⚠ 未产出：**U-11 撤回五步流水线与影响范围**（五步 + 两级 SLA + 「证据已撤回」段落仍在）—— 该屏尚未画（`/consent` 里有演示态实现，但**未纳入 v2 截图**，不是签核材料） | ~~`ui-preview/itv-withdrawal-flow.png`~~ | 🔴 |
...
v1 的 `uc-6-0-详情-虚拟标签占位.png` 在 v2 里**没有对应文件**，
