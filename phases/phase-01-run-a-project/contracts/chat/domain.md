...
### 11. 「更早」线程的归组与检索（uc-8-1 [原型确认缺失]）—— 已裁（2026-09-07）

原型只有 `今天` / `本周` 两组，**无「更早」组、无翻页、无时间筛选**。此前的实现
把「本周之前」的线程直接从响应里丢弃（不返回、也不返回计数），导致 2026-09-07
一次真实事故：一个正常使用的账号只因为当天恰好是 ISO 周的周一，个人对话列表
瞬间清空，被误判为「数据库被清空了」（见 issue #2894 / #2898）。

人类裁决（issue #2898）：**所有历史记录都要可以浏览**，参照 Codex 的历史对话
面板。落地为 `今天` / `本周` / `更早` 三组，「更早」恒返回全部剩余线程，不丢弃
任何一条。**翻页/按时间筛选仍未定义**——当前「更早」是一整桶、无分页，量级
显著变大后需要游标分页，登记为后续缺口，不在本次范围内。
