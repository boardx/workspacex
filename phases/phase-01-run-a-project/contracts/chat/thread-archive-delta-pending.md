...
### 1b. `mutateThread.in.op` 枚举扩容 —— **✅ 已裁决：候选 A（见下方决策 ①）**

候选 A（✅ 已裁决）：加两个值，`archive`/`unarchive`，与 `create`/`rename`/`delete`
同级并列：
...
### 1d. 归档正在运行的会话要不要拦 —— **✅ 已裁决：候选 A，不拦（见下方决策 ②）**

`mutateThread` 当前完全不感知"这条线程是否有活跃 run"——`create`/`rename`/
`delete` 都不检查。归档不新增这个判断面：不拦，`archive`/`unarchive` 与其余
三个既有 op 一样，不检查活跃 run 状态。三个候选的完整对照见下方决策表
（决策档案）。
...
⇒ **✅ 已裁决（②，见下方决策②）：不新增 `ACTIVE_RUN_IN_PROGRESS`**——归档不检查
活跃 run，`err` 分支就是上面这四个，与 `create`/`rename` 同一形状，不比它们多
一个判断面。
...
- （**存档，决策②已裁决为 A 后不适用**）原提醒：`plan-control` I-12 的 `pause`
  语义（暂停一个活跃 run）与决策②如果选中"归档要拦活跃 run"，判断"是否存在
  活跃 run"这件事最好是同一个事实源，不要 `chat` 束和 `plan-control` 束各自
  查一遍——**决策②最终选 A（不拦），归档不判断活跃 run，这条跨束依赖没有发生**。
...
> ## ✅ 人类裁决（2026-08-26，经 coord-main 转达至本会话，逐字转写不归纳）
> coord-main 在协调消息里的原话：
> 「PR #2135 已开，人类三个决策都定了（全部你推荐的选项）：
> ① `mutateThread.op` 加 `archive`/`unarchive` 两个枚举值
> ② 归档不拦活跃 run，只挡未来的用户写操作
> ③ 「删除」降级——未归档线程菜单不显示，只有已归档线程才出现「彻底删除」次要动作」
>
> ⚠ **本节转写的是 coord-main 转达给本会话的消息原文，不是人类在 PR 上的原始逐字**——
> 这条决策的**机械证据**仍然是人类在 PR #2135 上点 Review → Approve 这个动作本身
> （见 `human-decision-packaging.md` 规则二第 4 条），本文件的转写只是把决策内容
> 落回契约材料，**不构成、也不替代**那个 Approve 动作。`design-signoff.md` 的
> `status` 依旧不动，签核仍等人类在 PR 上点 Approve。
>
> ⇒ 落地形态：① A（`archive`/`unarchive` 两个枚举值）、② A（不拦活跃 run）、
> ③ B（删除降级为已归档线程菜单里的次要动作）——均为本文件当初的推荐项，
> 下方三张对照表保留为决策档案，不是待办。

### ① ✅ 已裁决：A —— `mutateThread.in.op` 加 `archive`/`unarchive` 两个枚举值

### ①（档案）`mutateThread.in.op` 该怎么扩容

| 候选 | 怎么做 | 支持 | 代价 |
|---|---|---|---|
| **A（✅ 已裁决）** | 加两个枚举值 `archive`/`unarchive`，与现有 `create/rename/delete` 并列 | 与既有三个动作同一种设计语言，审计事件类型可以直接叫"归档"/"取消归档"，不需要额外读一个布尔参数才知道方向；`err`/`out` 形状零改动 | 枚举多两个值 |
| B（未采纳） | 复用单一 `op:"archive"` + 新增 `archived: boolean` 参数做翻转 | 枚举少两个值 | 前端每次都要显式传 `archived: true/false`，容易漏传/传反；审计事件类型要从"op + archived 字段"两处拼出来，不是单一事实 |

### ② ✅ 已裁决：A —— 归档不拦活跃 run，只挡未来的用户写操作

### ②（档案）归档一个正在运行会话要不要拦

| 候选 | 怎么做 | 支持 | 代价 |
|---|---|---|---|
| A（✅ 已裁决） | 不拦。归档随时可执行；归档只阻断*未来*的用户写操作（`THREAD_ARCHIVED_READONLY` 既有语义），不中断已经在飞的 run——run 该怎么跑完还怎么跑完，写入路径是服务端内部通道，不经过 `mutateThread` | 零新增判断面，复用现有"archived 只挡用户写、不挡系统写"的既有实现事实（`getThread` 对归档线程正常返回、不抛错，说明读侧从来没把 archived 当"完全冻结"） | 用户可能在 agent 还在打字时把线程归档，短暂出现"已归档但列表右上角还在转圈"的观感，不算数据错误，是一个 UX 细节 |
| B（未采纳） | 拦。存在活跃 run 时归档返回新错误码 `ACTIVE_RUN_IN_PROGRESS`，要求用户先等 run 结束（或走 plan-control 的 `pause`）才能归档 | 语义更"干净"：归档态下线程真的完全静止 | 需要新增"判断这条线程当前是否有活跃 run"的查询，且这个判断如果 `plan-control` 束也要用（见④），必须收敛成同一个事实源而不是各查各的——本次要新拉一条跨束依赖 |
| C（未采纳） | 不拦，且归档时顺带强制 cancel 活跃 run（复用 `plan-control` I-12 的 `cancel(action=interrupt)` 原语） | 保证"归档 ⇒ 完全静止"的同时不需要用户额外操作 | 归档这个动作会产生一个隐藏副作用（打断用户可能还想看完的回复），且直接触碰 `plan-control`（未签核）束的 run 取消语义，跨域改动，本轮明确排除在"只出契约文档"范围外 |

⇒ 由于选中 A，`usecases.md` 草案 UC-6 的 `err` 分支**不追加** `ACTIVE_RUN_IN_PROGRESS`
（上方②节原草案里的 "[| 决策②选 B 时追加 ACTIVE_RUN_IN_PROGRESS]" 这一行不生效，
按 A 落地即删除该行）；④跨束复核里关于"是否需要与 `plan-control` 共用活跃 run
判断"的提醒同理不适用——不拦就不需要判断。

### ③ ✅ 已裁决：B —— 「删除」降级为已归档线程菜单里的次要动作

### ③（档案）hover 动作组「删除」怎么共存

| 候选 | 怎么做 | 支持 | 代价 |
|---|---|---|---|
| A（未采纳） | 「删除」完全移除，菜单只保留 图钉/改名/归档，可逆归档是**唯一**清除线程的路径 | 用户不会再误触不可逆操作；符合"归档是主动作"的产品意图，菜单更简单 | 现有"硬删除需要审计 `reason`"这条能力从 UI 上完全消失（后端 `delete` op 仍在，但没有入口触发）——如果确有需要永久删除的场景（如合规要求彻底清除），需要另开路径 |
| B（✅ 已裁决） | 主要动作变 图钉/改名/归档；「删除」降级——**未归档**线程的菜单不显示删除，**已归档**线程的菜单才出现「彻底删除」作为次要/危险区动作 | 保留硬删除能力但加一道"先归档"的天然确认门槛，防误删；符合任务描述里给的第二个建议方向 | 菜单在归档前后长得不一样（两态两份菜单结构），实现和测试面比 A 略大；用户想删一条线程要多点一步（先归档再删） |
| C（未采纳） | 归档与删除保持两个平级主动作，菜单变成 图钉/改名/归档/删除 四项，互不降级 | 改动最小，两条路径都不受影响 | 菜单从三项变四项，「删除」仍然一键可达不可逆操作，没有解决"归档想替代删除成为主动作"的产品意图；与任务描述给的方向不符 |

⇒ 由于选中 B，`thread-list-shell.tsx` 的四态状态机在实现期需要新增"当前
线程是否已归档"这一条件分支来决定菜单渲染哪一套动作项——这是实现细节，
不需要再回来签核，但登记在此供实现 PR 参照。
