...
- `PersonalTranscription`：用户私有转录文档，保存名称、标签、owner、聚合状态与唯一连续正文。
...
- I-2：`PersonalTranscription.content` 是个人逐字稿唯一正文事实源；个人转录不保存或返回分段与时间戳。
...
- I-5：interim 永不落库；final 必须原子追加到正文成功后才能推送给客户端。
- I-6：同一连接内同一 `captureId + upstreamSentenceId` 至多向客户端追加一次。
...
- I-13：只有 owner 且没有 live capture 时可以替换完整正文；修改与开始录音并发时必须失败关闭。
