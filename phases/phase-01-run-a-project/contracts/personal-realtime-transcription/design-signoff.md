...
本束是 issue #945 的正式签核面，覆盖用户私有历史工作台、Fun-ASR 状态机、一次性 ticket、AudioWorklet 与连续正文持久化。它不修改已签 `recording` 束的项目录音语义；项目录音继续使用原契约。

## ① UI

请核对 [ui.md](./ui.md)：历史卡片与新建弹窗是否符合已选方案；详情页是否只保留一个开始/停止按钮与单一正文逐字稿；复制、修改、错误和收尾状态是否足够明确。
