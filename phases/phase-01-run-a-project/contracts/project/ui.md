...
> 🔁 **2026-08-16 delta（过期提醒，非重签）**：A 节这 9 张截图画的是「我在里面/我管着它」
> 两段式布局——Q-6① 已由新裁决（候选 D）推翻为扁平列表 + tags（见
> `requirements/00-project/OPEN-QUESTIONS.md` 该条 delta；`usecases.md` UC-P2 已同步改写）。
> **这 9 张截图目前是过期材料**，本次不重拍（重拍属于第 ① 件材料的正式重签流程，
> 不是一次 feature 实现顺手做的事）。F185 的实现以 `usecases.md` 新形状为准，UI 走真实组件
> 而非这批旧图；旧图保留仅供「两段式曾经是什么样」的历史参照，不作为当前验收依据。
