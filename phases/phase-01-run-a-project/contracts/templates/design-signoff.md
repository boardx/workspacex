...
#   ⚠ 本行**只动 `covers:`**，`status` / `confirmed_by` / `confirmed_at` 一字未改（ADR-023）。
# 2026-08-18（追加 **F961**，dev-project agent）——F960 的前端半边：项目筹备组卡按**已签原型**
#   补齐组员计数、访谈对象摘要与展开后的六列对象表。三条件逐条对照，这一条是**零新增设计面**：
#     1. **UI 已签**：三处改动全部**照着已签原型往回收**，没有一处是新设计——
#        · 组员显示人数：`ui-preview/project-v2/uc-2-2-prep-default.png` 与
#          `apps/web/lib/mock/project.ts` 的 `members: "3 人"` 逐字如此；F950 当时写成
#          姓名拼接是实现时的自由发挥，本次是**修正回原型**，不是改设计。
#        · 访谈对象摘要行：同一张原型图逐字有这一行（「业主侧投资经理」等四组）。
#        · 组卡展开出六列对象表：`uc-2-2` R3 第 5 步逐字「组卡展开：…**观察/访谈对象表**
#          （见第 6 步）」，第 6 步逐字给了六列表头与 `[＋ 加对象]`。
#     2. **契约已签**：只消费 F960 已落地的 `getInterviewSubjects`/`updateInterviewSubjects`
#        两条 operation，形状逐字照契约，**未改一个字段**，无迁移、无新端点、无新 rewrite。
#     3. **零新增设计面**：不新增错误码、不新增字段、不新增屏、不新增交互语义。
#        `[AI 建议人选]`（原型有此按钮）本版**不接**——AI 选人能力全仓无实现，按既有纪律
#        渲染为禁用 + 如实说明，不新造一个假入口，也不因此新增任何契约面。
#   ⚠ 本行**只动 `covers:`**，`status` / `confirmed_by` / `confirmed_at` 一字未改（ADR-023）。
# 2026-08-17（F203，dev-chat-e2e）：「主题与背景」「角色与权限」面板逐项对照原型补齐保真度——
#   零新增契约面自查：仍是 updateDesignFacet 的 content: string 字段（F193/F194 已签核形状，
#   一字未改）；PermissionMatrixValue 从 flat Record<string, boolean> 升级为
#   { groupingMode, leadFrom, cells } 是该已签核字段内部承载的 JSON 形状调整，不是新增
...
#   ⚠ 本行**只动 `covers:`**，`status` / `confirmed_by` / `confirmed_at` 一字未改（ADR-023）。
# 2026-08-18（F205，dev-chat-e2e）：分组三四项（场地与形式 / 项目材料 / 分组打印素材 /
#   组内能力）结构化编辑面板——同 F204 的先例，零新增契约面（仍是 updateDesignFacet 的
#   content: string 字段，不新增契约操作/错误码/迁移），按 covers 追加规则三条件自查后追加。
#   ⚠ 本行**只动 `covers:`**，`status` / `confirmed_by` / `confirmed_at` 一字未改（ADR-023）。
# 2026-08-18（F206，dev-chat-e2e）：分组四/五四项（Agent 编排 / Skill 绑定 / 输出物 /
#   报告模板）结构化编辑面板 + 15 项全覆盖机械门控——同 F204/F205 的先例，零新增契约面
...
#   ⚠ 本行**只动 `covers:`**，`status` / `confirmed_by` / `confirmed_at` 一字未改（ADR-023）。
# 2026-08-18（F208，dev-chat-e2e）：真栈端到端自检用例（16 项全部结构化、真读真写真落库）——
#   纯测试增量，零新增契约面/界面元素/交互语义（只断言 F204–F207 已交付的行为在真栈里成立），
#   按 covers 追加规则三条件自查后追加。
#   ⚠ 本行**只动 `covers:`**，`status` / `confirmed_by` / `confirmed_at` 一字未改（ADR-023）。
covers: [F17, F18, F19, F20, F21, F22, F23, F24, F25, F26, F27, F28, F29, F30, F175, F174, F177, F179, F181, F186, F187, F188, F189, F193, F194, F201, F202, F950, F960, F204, F205, F206, F207, F208, F203, F961]   # 束↔feature 映射的权威（ADR-023 决策三）；改它等于改评审范围
