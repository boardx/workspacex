...
估点 **10**（R3/R6 原 6 点 + R13/R14 追加需求各 2 点，2026-08-23 人类裁决扩增后更新）
...
## R13 追加需求 A（人类 2026-08-23 会话裁决新增）：首屏加载骨架屏→内容过渡
> 原 R3-R6 只覆盖消息到达与面板展开两类编排时刻。人类在 phase-12 签核讨论中把候选
> 扩到三类，本节补记第三类：路由骨架屏渲染完成、真实内容就绪时的过渡动效。
> **超出 F03/F04 原范围，需新开 feature（暂记 F17）。** 完整用例定义见
> `contracts/motion-microinteraction/usecases.md` UC-5（本节不重复展开，用例细节以
> 契约束文档为准，这里只记录需求本身的存在与来源）。
- 前置条件：目标路由已有骨架屏基础设施（本节不要求新建骨架屏组件本身）
- 业务规则：普通 loading spinner 不得被强行包一层过渡动效冒充"编排"（没有骨架屏基础设施时，本条不产生任何动效）
- 依赖：`prefers-reduced-motion` 降级同 R9

## R14 追加需求 B（人类 2026-08-23 会话裁决新增）：附件/长任务上传进度动效
> 同 R13 的追加背景。**超出 F03/F04 原范围，需新开 feature（暂记 F18）**，且依赖
> 后端真实进度事件源（如 DA-15）是否就绪，不能抢跑前端插值伪造进度。完整用例定义见
> `contracts/motion-microinteraction/usecases.md` UC-6。
- 前置条件：上传/长任务已有真实进度事件源
- 业务规则：后端只提供二元"进行中/完成"时，动效降级为不确定态指示器，不得插值出虚假百分比
- 依赖：后端进度事件源就绪时序，需与对应后端能力对齐后再排期
