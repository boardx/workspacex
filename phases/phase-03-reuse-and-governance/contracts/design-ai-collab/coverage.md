...
| V6 | 设计对话回复来自模型；只喂本项目历史 | `designWorkbench.appendProjectChat`（`chat[].source = "model"`） | `design-detail-send` → `design-detail-chat` | ✅ |
| V7 | 合法 writeback 直接写回 problem/criteria/frames，`applied` 如实 | `designWorkbench.appendProjectChat`（`out.reply.applied`；`DesignChatWriteback`） | `design-detail-chat-applied` / `design-detail-spec` / `design-detail-frame-{i}` | ✅ |
| V8 | 模型失败退回 `DESIGN_WORKBENCH_CHAT_REPLY`，标 fallback | `designWorkbench.appendProjectChat`（`out.reply.source = "fallback"`） | `design-detail-turn-fallback` | ✅ |
| V9 | 非 owner 不调模型不写回 | `designWorkbench.appendProjectChat`（`NOT_PROJECT_OWNER`，既有） | `design-detail-chat-error` | ✅ |
