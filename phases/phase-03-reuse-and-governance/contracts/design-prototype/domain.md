...
- ⬜ 迭代 11（**backlog，未开工**）：可点击原型——模型自动连跳转关系 + 预览模式 + 画板流程连线。
  拆解、三个设计取舍与工作量评估见 **issue #2917**。两条硬前提：
  ① 改的是 API 契约（`PrototypeScreen` 变形）⇒ 本束 `design-signoff.md` 第 ③ 件要**人类重新签核**，
  agent 不许自己改 status；② 建议同期把 `frames`/`prototype`/`frame_notes` 三份平行数组收敛成
  单列 `screens jsonb`——每加一项「每屏一份」的数据就要一次迁移 + 两张表各一列 + 两侧长度不变量，
  而 #2900 修的那次数据丢失（只写 frames 就清空 prototype）正是这个形状咬出来的。
