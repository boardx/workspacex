...
| D3 | 附件 4 张图片 → 5 个任意文件（PDF §5.1） | 契约 `.max(4)` + MIME 白名单 + 病毒扫描范围 | 放宽到 5，MIME 扩到 pdf/txt/md/zip/音视频，沿用扫描 |
...
| B2.3 | 附件放宽：契约 `.max(5)`、MIME 白名单扩展、`upload-feedback-attachment.ts` 大小/类型/扫描规则同步；web 端 accept 与拒绝提示同步 | 2 | D3 |
