...
## V85 — 加页插在**当前页之后**
`design-loop.test.tsx`：停在第 1 页点"加一页" ⇒ `addScreen(at: 1)`。
⚠ 反证：写成 `at: frames.length`（追加末尾）⇒ 红。在第 1 页点加页，新页该在它旁边。

## V86 — 复制整页去 id
同文件：`addScreen(at+1, "首页 副本", root)` 且 root 及其孩子都没有 id。
⚠ 反证：不去 id ⇒ 两页各有一个 `id="a"`，按 id 寻址命中第一页。

## V87 — 只剩一页时删页按钮禁用
同文件。契约也会拒（`LIMITS`），但不该让用户点了才知道。

## V88 — 改名发 `renameScreen`，只动一个字段
同文件：双击页签 ⇒ `{op:"renameScreen", screen:0, frame:"首屏"}`。
取消 / 原样不变 / 只有空白 ⇒ **一个请求都不发**。
⚠ 反证：用 remove+add 拼改名 ⇒ 红。那样丢 notes，且 link 偏移做两次不保证还原。

## V89 — 撤销回到**上一版**
同文件：三版历史 ⇒ 恢复 `v2`（不是 `v3`）。没有可回退版本时出现提示条。
⚠ 反证：取 `.at(-1)` ⇒ 恢复到"现在这份"，点了等于什么都没发生——**静默失效**，
用户会以为撤销坏了却说不出哪里坏。

## V90 — 契约里每个 patch op 都必须在给模型的说明里
`packages/contracts/tests/design-prototype.test.ts`：遍历 `PrototypePatchOp.options` 取 op 名，
逐个查 `PROTOTYPE_PATCH_GUIDE`。
⚠ 反证：删掉 guide 里的 addScreen ⇒ 红。
**这条门写出来当场抓到两个真洞**：页级三个 op（迭代 12 起）与 `setLinks`（迭代 11 起）
从来没告诉过模型。能力存在但没人告诉使用者，等于不存在。
