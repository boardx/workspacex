...
- **绝不替别人记**：共享 / 项目会话里其他成员的话不会进你的个人空间，你的话也不会进别人的——写进谁的空间只由证据消息的作者决定，
...
- **已知取舍 1（产品层面，未解决）**：模型写出来的**回答正文**贴在项目会话里、对项目成员可见，**可能复述提问者的个人记忆**；其他成员后续几轮的对话历史里也带着这条回答。这是回答本身可见的直接后果，本轮不做任何遮挡或改写。
- **已知取舍 2（第二条泄漏路径，未修，需人类决定）**：回答本身也是一条聊天消息，**会被抽取**——`kg_enqueue_extraction` 触发器不按 `author_kind` 过滤（只拦 `raw_transcript`、空白、`visibility_scope` 非空和两道开关），抽取器对助手消息照常处理（`model-knowledge-extractor.ts` 的「本条消息（助手说的）」）。所以 A 的回答里复述的个人决定，可能被抽成该项目会话的 L0（`chat_session`）结论：出现在每个成员的知识面板里、被他们后续的提问召回，个人记忆由此**变成共享的项目知识**（新结论有新 id，不带 A 个人结论的 id / 出处，但原文意思出去了）。本轮**没有修**，理由：(a) 修法落在抽取闸门（`kg_enqueue_extraction` 或抽取 tick），与同时进行的 #4283（抽取 / 晋升路径）是同一处；闸门是 `CREATE OR REPLACE FUNCTION`，两个并行迁移各自重写会后写者静默覆盖前者的闸门，需要协调后单独做；(b) 修了会连带丢掉这类回答里本来可以进项目记忆的内容，属于产品取舍。**建议的最小修法**（待决定）：触发器里加一道——`NEW.author_kind = 'agent'` 且这一轮（`NEW.agent_run_id`，回答落库时即已写入）的 `kg_turn_recalls.items` 里含有 `scope_kind = 'personal'` 的结论、且会话挂在项目下 ⇒ 这条回答不入抽取队列（`kg_turn_recalls` 在执行器召回时就已写入，早于回答落库）。备选：照常抽取，但把由这类回答抽出的结论只记进提问者本人的个人空间而不进项目会话 L0。
