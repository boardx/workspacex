...
5. **跨会话改口（issue #4290，第 8 轮）**：同一个人在会话 A 说「我决定关注 211 高校」、之后在会话 B 说「改成关注 985 吧」，此前两条决定都活着、都被强制召回——会话 C 的模型同时拿到互相矛盾的两条（F16 只认同一组实体的数值改口，211 → 985 换了对象不出冲突卡）。
   ...
   **改口句式的边界**（独立评审第 8 轮）：「不再 / 不…了」只在支配框架动词时才算改口（「不再关注…」「不用 X 了」算；「不再犹豫」「不急了」「不用再讨论了」不算）；新句里出现旧对象，只有连在改口句式上（不再 + 动词 + 旧、把旧换成 / 改成…、旧 + 换成 / 算了）才算明说，ASCII 对象按词边界（Go ≠ Google）；新决定的对象与旧对象相同（重说 / 重申）从不取代。
