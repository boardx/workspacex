...
工作台未保存离开提醒已实现并在浏览器验证：点导航先确认，取消后草稿仍在，明确丢弃才导航；另注册标准beforeunload用于浏览器刷新/关闭提醒。最新组件复审修正与反证见review-resolutions.md；原生后退恢复和键盘复验因Mac锁屏待补。

新增 `/preview/ai-capability-studio/governance`，展示当前配置五项准入测试、修改后重测、MCP保留/替换/清除凭据、失败保留原配置、新发现工具明确授权、失败记录跳转与返回工作台。6项状态/组件测试通过，无浏览器截图，所有连接结果为手动模拟。该页补充既有管理界面交互，不代表完整Model配置/停用策略表单或真实权限审批已交付。

待补齐：Model 配置和连接诊断、MCP 发现与逐工具审批、真实 Agent 选择、聊天失败归因、并发冲突、键盘焦点专项验证。权限演示还不能证明后端鉴权。正式束签核前需要将工作台剩余原型数据结构接到审阅后的契约，补全覆盖矩阵。

后续补充：Model配置弹窗已使用真实输入控件，演示revision冲突时保留非敏感输入、清空凭据、加载最新/重新审阅；MCP重连增加端点、凭据替换输入和无凭据时禁用keep。管理原型10项测试通过，均无新的浏览器截图。逐项剩余范围统一见admin-ui-remaining.md，不能将本段解释为上述完整管理能力已上线。

## 普通仓库适配原型（2026-09-10）

新增 `/preview/ai-capability-studio/adapt`，由导入页“没有SKILL.md”示例入口到达。此独立fixture不会读取前页输入或冒充真实来源扫描。显示缺失要求/未知许可/脚本副作用，用户明确选择参考文件并确认后生成SKILL.md模板与references/imported/文件清单，支持当前页面修改说明。改变文件选择会撤销确认；未选脚本不入草稿；无自动执行、AI读取、真实持久化或发布。生产闭环必须接assessment→创建草稿→既有多文件工作台，不可把此独立示例当已完成正式导航。组件2项、与导入/工作台集成15项通过；真实浏览器截图仍待解锁。
