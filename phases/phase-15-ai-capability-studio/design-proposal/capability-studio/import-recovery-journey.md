...
契约位于 skill-import-recovery.ts，复用原Job/Batch/Policy，无生产导出。10项测试覆盖CAS、身份串换、假分页、引用中的丢弃、运行中取消、取消后旧完成、过期及幂等重放。真实列表、认证cursor、worker fencing、事务和GC尚未实施；上传/批次恢复界面仍待补齐，C35保持进行中。
