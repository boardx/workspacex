...
## 已知瑕疵（原型阶段可接受，真实接线需处理）
1. **round-trip 里 `<` 被 HTML 转义**：mermaid 标签 `< 18 个月?` 经画布往返序列化成 `&lt; 18 个月?`
   （见 saved 截图右栏 `D1{"&lt; 18 个月?"}`）。是 `fabric-markdown` 序列化器的既有行为，非本次引入；
   真实保存前应在序列化层解转义。
2. 气泡内只读画布固定高 320px；超大图靠 `fitToContent` 缩放适配，极端复杂图可能偏小——真实接线可给
   「按内容自适应高度」上限。

## 待确认清单（束级 design-signoff.md 第 ① 件签核时逐条看）
1. **保存语义**：编辑后保存是「就地覆盖原 AI 消息里的这张图」还是「派生一个独立 canvas artifact」？
   影响后端契约（本原型不接后端，两种都能承接，但需先定）。
2. **气泡内只读是否符合预期**：目前气泡里不可直接拖动，编辑一律走最大化。若希望气泡内也能轻量微调，需另议
   （会与「误拖」权衡）。
3. **最小工具条范围**：现仅 `选择 / ＋节点 / 删除`。是否需要把 `连线 / ＋便签 / 源码视图` 也搬进最大化面
   （`CanvasStage` 都支持，只是没在本工具条暴露）。
