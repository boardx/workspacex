...
- 独立 feature review 发现并已修复两项发布绕过：`ready` 问卷修改内容后自动退回 `draft`，且报告章节只有真实引用现存题目才算具备数据供给；服务端开始回收前再次执行发布门禁。
- PR 自动审查追加的三项意见已修复：说明/分页不参与回答题映射；文本、图片、分页报告块不能充当数据供给；题目语义校验通过后才允许进入 `ready`。

## 验证边界
- F04 权威验证：3 个文件、10 项测试全部通过。
- 受影响题型回归：31/31 通过；发布失败相关 Web 回归独立复跑 62/62 通过。
- review 修复回归：5 个 API 测试文件、40 项测试全部通过；`@repo/api` 类型检查通过。
