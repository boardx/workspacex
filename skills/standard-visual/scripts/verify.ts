...
const pack=verifySkillStarterPack(await source.load('standard-visual','1.0.0'),{packId:'standard-visual',packVersion:'1.0.0'});
...
assert.equal(await new FileSkillStarterPackSource(undefined).load('standard-visual','1.0.0'),null);
const changed=structuredClone(pack);changed.skills[0]!.files[0]!.contentBase64='dGFtcGVy';assert.throws(()=>verifySkillStarterPack(changed,{packId:'standard-visual',packVersion:'1.0.0'}));
