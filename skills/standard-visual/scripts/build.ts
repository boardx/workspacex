...
const unsigned={schemaVersion:1,packId:'standard-visual',packVersion:'1.0.0',skills:[{stableName:'visual-content',name:'视觉内容制作',semanticVersion:'1.0.0',manifest:{capabilityId:'WX-S017',runtime:'bailian-and-offline-visual'},files}]};
const pack={...unsigned,packDigest:sha256(JSON.stringify(unsigned))};verifySkillStarterPack(pack,{packId:'standard-visual',packVersion:'1.0.0'});
const directory=resolve(root,'../../starter-packs/standard-visual'),path=resolve(directory,'1.0.0.json'),content=JSON.stringify(pack,null,2)+'\n';
