...
const unsigned={schemaVersion:1,packId:'maau-diagnostics',packVersion:'2.0.0',skills:[{stableName:'maau-venture-valuation',name:'AI 原生递归资产与估值预测',semanticVersion:'2.0.0',manifest:{capabilityId:'WX-S022',runtime:'offline-node-pdf-lib'},files}]};
const pack={...unsigned,packDigest:sha256(JSON.stringify(unsigned))};verifySkillStarterPack(pack,{packId:'maau-diagnostics',packVersion:'2.0.0'});
const directory=resolve(root,'../../starter-packs/maau-diagnostics'),path=resolve(directory,'2.0.0.json'),content=JSON.stringify(pack,null,2)+'\n';
