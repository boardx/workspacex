...
async function main(){
 ...
 const pack=verifySkillStarterPack(await source.load('standard-canvas','1.0.1'),{packId:'standard-canvas',packVersion:'1.0.1'});
 ...
 assert.equal(await new FileSkillStarterPackSource(undefined).load('standard-canvas','1.0.1'),null);
 const changed=structuredClone(pack);changed.skills[0]!.files[0]!.contentBase64='dGFtcGVy';assert.throws(()=>verifySkillStarterPack(changed,{packId:'standard-canvas',packVersion:'1.0.1'}));
