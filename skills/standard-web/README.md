...
`node --import tsx skills/standard-web/scripts/build.ts` 生成 `standard-web / 1.1.2` starter；`node --import tsx skills/standard-web/scripts/verify.ts` 用真实 FileSkillStarterPackSource 核对两个 skill 的分发、全文件内容和摘要。部署者需要设置 SKILL_STARTER_PACK_ROOT 指向 skills/starter-packs，经既有受授权导入选择该版本。本分支不修改共享生产 seed 接线，不代表已部署或全部用户已导入。
