...
const unsigned={schemaVersion:1,packId:'standard-document',packVersion:'1.0.0',skills:[{stableName:'document-understanding',name:'文档理解与结构提取',semanticVersion:'1.0.0',manifest:{capabilityId:'WX-S018',runtime:'anydoc-text-markdown'},files}]};
const pack={...unsigned,packDigest:sha256(JSON.stringify(unsigned))};verifySkillStarterPack(pack,{packId:'standard-document',packVersion:'1.0.0'});
const directory=resolve(root,'../../starter-packs/standard-document'),path=resolve(directory,'1.0.0.json'),content=JSON.stringify(pack,null,2)+'\n';
