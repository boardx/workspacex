...
const pack=verifySkillStarterPack(await source.load('standard-document','1.2.0'),{packId:'standard-document',packVersion:'1.2.0'});
...
assert.equal(await new FileSkillStarterPackSource(undefined).load('standard-document','1.2.0'),null);
const changed=structuredClone(pack);changed.skills[0]!.files[0]!.contentBase64='dGFtcGVy';assert.throws(()=>verifySkillStarterPack(changed,{packId:'standard-document',packVersion:'1.2.0'}));
