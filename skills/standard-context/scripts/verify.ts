...
async function main(){
 ...
 const pack=verifySkillStarterPack(await source.load('standard-context','1.1.1'),{packId:'standard-context',packVersion:'1.1.1'});
 ...
 assert.equal(await new FileSkillStarterPackSource(undefined).load('standard-context','1.1.1'),null);
 const changed=structuredClone(pack);changed.skills[0]!.files[0]!.contentBase64='dGFtcGVy';assert.throws(()=>verifySkillStarterPack(changed,{packId:'standard-context',packVersion:'1.1.1'}));
