...
const skills=[['interview-synthesis','访谈与反馈综合','WX-S010','evidence-ledger.md','1.0.0'],['user-research-planning','用户研究规划','WX-S019','planning-template.md','1.0.1'],['maau-canvas','MAAU 模板','WX-S021','canvas-template.md','2.0.0']].map(([stableName,name,id,reference,semanticVersion])=>({
...
const unsigned={schemaVersion:1,packId:'standard-methods',packVersion:'1.2.0',skills};
const pack={...unsigned,packDigest:sha256(JSON.stringify(unsigned))};
verifySkillStarterPack(pack,{packId:'standard-methods',packVersion:'1.2.0'});
writeFileSync(resolve(root,'../starter-packs/standard-methods/1.2.0.json'),JSON.stringify(pack,null,2)+'\n');
