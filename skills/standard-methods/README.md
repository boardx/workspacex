...
复用现有 FileSkillStarterPackSource 和 verifySkillStarterPack，不新增引擎。部署者需将 skills/starter-packs 配置为 SKILL_STARTER_PACK_ROOT 后，走既有受权导入流程选择 standard-methods / 1.2.0。未配置 root 不会自动出现候选包；本提交不表示已部署、已导入或全部终端用户已可用。
