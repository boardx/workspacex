...
- 2026-09-13（#3592）：agent run 到终态与产出文件可下载不是同一个时刻；大文件卡会先渲染为
  `aria-disabled=true` 且无 `href`，待鉴权下载完成才创建页面作用域的 `blob:` URL。真实模型
  验收必须有界等待锚点同时满足“非禁用 + 认证 blob URL”，再做完整字节、魔数和长度断言；
  永久禁用必须在上限内明确失败，不能把终态当作下载就绪信号。
...
- 2026-09-18：`@文件名` 引用的词边界最初只收空白/行尾/标点，中文里「用@MAAU.png做估值」这种紧跟汉字的写法被判"没引用"，`/inputs` 与视觉输入双双为空、模型回"文件不存在"（devapp 二次实测）。边界集合已加入中日韩文字。教训：**中文语境下"词边界"不能照搬空白分词的直觉**——文件名后面紧跟汉字是常态，不是异常（出处：issue #3735）。
- 2026-09-19：用户消息上的图片附件只显示成"文件名 + 大小"卡片，人类反馈「是图片的话要展示出来图片」。`MessageAttachments` 对 `image/*` 改内联缩略图，字节仍走受鉴权的 `/attachments/:id/content` + `useAuthedImageSrc`（裸 `<img src>` 拿不到 Bearer 头），取不到就回落文件卡片。教训：**附件"已发出去"的可见证据要按类型给——图片的证据是图本身，不是一行文件名**（出处：issue #3737）。
