...
HMW 的问题陈述可以先于想法分区生成。聊天围栏不能把缺少 `##` 一律判为无内容；仅当匹配 HMW 定义的非空字段时接受字段型陈述。`parseTemplateText` 把「我们可以／为谁」兼容到「我们可以如何／为给」，规范值优先。不要为了通过校验编造想法便签。

## 保存与发布响应竞争（#3545）

编辑器保存后会把返回的草稿快照交给父组件onSaved；显式发布则关闭编辑器。保存未完成就发布会让晚到的草稿重新打开面板。普通及强制发布入口须等待saving结束，不得用无条件同步服务器行覆盖尚未保存的编辑内容。
