...
## 收尾（空闲机器、MLX、合并了版次差异那条线之后）

| 套件 | 模型调用中位 | wall 中位 | 首块中位 | 质量 |
|---|---|---|---|---|
| 聊天 | 1 | **6 s** | **0.9 s** | — |
| 网址 | 2 | 32 s | 2.6 s | 5/5 用到 fetch_url |
| 画布 | 1 | 36 s | 7.2 s | 围栏 4/5；缺口从「6 个分区」降到「1 个分区或 1 个字段」 |
| 追问 / 反馈 / 起名 | — | 1.1 s / 6.2 s / — | — | 起名 5/5 |

**DMG**（并入版次差异线与浅色主题对比度修复之后重打）：`WorkspaceX-0.2.0-arm64.dmg`，
9 005 420 942 B，sha256 `956faef7de0ee4700953a33f759aac1960a515eb408c57273b5d501f259f77d5`，
`build-info` = `cc5de2eb3`。挂载核对：随包 `qwen3.5:4b` + `qwen3.5:4b-mlx`、`tool_budget` 在、版次能力矩阵在。
⚠ 合并 main 之后必须先 `pnpm install`：main 带进了 `echarts` 依赖，不装就 `next build` 失败，
而 DMG 打包不会因为 web 没构建而报错——它会把上一次的 `.next` 原样带走。
