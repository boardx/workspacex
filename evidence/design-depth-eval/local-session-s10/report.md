...
| S17 | 批注存在服务端：钉一条，换一个全新的浏览器（空存储）打开同一个项目还看得到（深度 S2，#3988） | ✅ | 另一个浏览器（空存储）打开同一个项目，看到「真栈批注 muf8jlmg」和它的钉（真 PGlite 上的 design_project_comments） | [s17-comment-other-browser.png](./s17-comment-other-browser.png) |
...
| S19 | 真实图片：往占位图里上传一张 2400×1800 的照片，刷新后还在（深度 S10，#3988） | ✅ | 原图 PNG 1066 KB（base64）→ 存下的 JPEG data URL 23 KB、宽 480px；刷新后仍在（真 API 请求体 + PGlite） | [s19-real-image.png](./s19-real-image.png) |
