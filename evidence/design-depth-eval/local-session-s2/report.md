...
| 项 | 值 |
|---|---|
| 代码版本 | `f276f2ff`（⚠ 工作区有未提交的改动——被测的不完全是这个提交） |
...
| S17 | 批注存在服务端：钉一条，换一个全新的浏览器（空存储）打开同一个项目还看得到（深度 S2，#3988） | ✅ | 另一个浏览器（空存储）打开同一个项目，看到「真栈批注 muez0sas」和它的钉（真 PGlite 上的 design_project_comments） | [s17-comment-other-browser.png](./s17-comment-other-browser.png) |
