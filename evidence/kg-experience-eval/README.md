...
4. **单用户、顺序相关**：十段旅程共用一个主用户和同一个库，按文件名顺序跑。跨会话记起之后，后面的旅程会召回前面旅程
   说过的话（比如 E5 的回答里会出现 E2 里记下的事），名次与引用条数随顺序变化；换顺序或并行跑，个别检查的结果可能不同。
   冻结的是这个顺序（playwright `workers: 1`、文件名）。
5. **E10「开启记忆前后」**：产品没有关掉记忆的开关。「开启」= 有 7 条记忆可召回、走图；「对照」= 从没聊过的账号（召回照样执行但为空）。
...
- 截图只保留 R0 与最新一轮（中间轮的 `R<n>.json` 里仍有逐条结果与量到的数；中间轮 `.md` 里的截图链接会失效）。
...
| R2 | **9.4** | 评审修订后在最终代码上重跑（E1–E10 各维与 R1 相同）。产品侧：图路函数改为不建任何对象（修掉定义者身份下的提权）、与 F08 逐行对拍；行内「忘掉这条」旁就地写清后果 | 一处，只收紧（评审 N7）：E4 摘录必须非空、抽屉里打开的就是这个引用那一条。截图只留 R0 与 R2，R1 的截图已删（R1.md 里的截图链接因此失效，逐条结果与量到的数仍在 R1.json） |
...
| E10 | `kg_graph_neighbors` 逐跳锚定：94 个节点时 2.7s → 29ms（原来聊十几轮之后每轮都超 2 秒上限降级）。每跳前沿上限 1000 条边以内，结果与 F08 版逐行相同（`graph-neighbors-anchored.test.ts` 把 F08 原版装在旁边对拍）；超过上限时比 F08 少——那是有意的，防一个人人都提到的实体把整张图拉进来。R1 版本在第二跳错误地排除了全部种子（F08 只排除这条路径自己的种子），并且用临时表存中间结果（定义者身份下可被调用方预置的触发器提权），R2 两处都已改正 | `apps/api/migrations/20260925100000_kg_f15_graph_neighbors_anchored.sql` |
