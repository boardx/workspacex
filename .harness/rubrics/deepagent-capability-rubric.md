...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 2026-08-27 | 85450c0d | 0.7 | 0.7 | 1.0 | 0.7 | 0.7 | 1.0 | 0.7 | **1.0** | 0.7 | 0.7 | **7.5** | `.harness/state/deepagent-eval/2026-08-27-85450c0d/`（`golden-evidence/` + `golden-scenarios-pytest-run.txt` = TC-1~TC-5 全 9 条含反证本会话真 Postgres 重跑全过；`live-plan-control-real-engine/01-run-sse-raw.txt` = 本会话真实登录 + 临时接到真实 `langgraph dev`（真 DashScope qwen3.8-max）后，在 `/chat/copilotkit-v2` 手动开启任务模式发真实消息，`read_network_requests` 抓到的生产 `/api/copilotkit/agent/default/run` 原始 SSE；`02-ledger-after-confirm.json` = 点击确认执行后账本快照；`scoring-rationale.md` = 十维逐条依据 + 与上轮 7.5 的 delta 说明） | ① D8 从 0.7→1.0：`harness.py` 本轮把 `SummarizationMiddleware.trim_tokens_to_summarize` 从库默认 4000 钉死为与触发线同值的 60000（修复"只摘尾巴"的 0.3 档缺陷），TC-4 golden 本轮验证 30 轮召回 + 反证（事实确不在近期窗口）双双通过，机制级证据齐全，②驱逐③cache 命中此前已有活体证据且本轮无回归，三项达标 ⇒ 1.0（本轮唯一分数变动，原始和 7.6→7.9，取整仍落 7.5）。② D1 维持 0.7 但证据基础质变：本轮首次拿到"真实模型经 #2224 提示词修复真的调用 write_todos → 生产桥转译独立工具调用事件 → F972-F978 六态面板+计划面板真实渲染"完整活体链路；但同一次探针也发现新缺口——点击"确认并执行"后端把账本 `phase` 翻成 `executing`、前端计时器持续走动，但全程未见任何新 `/run` 请求，40+ 秒后三步骤 `status` 仍全 `pending`，即"确认执行"只翻转账本状态、未真正触发引擎执行，已登记 issue #2250，不满足 1.0 档"执行中逐项更新状态"。③ D2/D6 维持 0.7/1.0：F212-F216（agent-interrupts 三张新 HITL 卡）代码级复核确认 `confirm_task_intent`/`fill_run_params`/`choose_execution_option` 三个工具名在真实 `apps/deep-agent-service` 的 `tools.py` 里不存在对应 `@tool` 定义，`copilotkit-agui.controller.ts` 的 `AguiRunInput` 丢弃 `tools` 字段——与已关闭的 issue #2017（`send_email`）同一类架构缺口，三张新卡在真实引擎链路上不可能被触发，不计入本轮 D2/D6 加分（契约文件自己已如实登记该边界，不构成一票否决）。④ D9 维持 0.7：本轮 SSE 证据里能看到 DA-19f 探针标记（`pathname`+固定 `probe` 字符串）真的送达生产请求的 `context` 字段（比上轮"两次重跑因资源竞争失败"进了一步），但仍是固定测试值而非真实动态 viewport/selection，未达 1.0。⑤ D3/D4/D5/D7/D10 均通过 golden 脚本重跑或代码零 diff 复核确认无回归，维持原分；D7 观察到新增 `RubricMiddleware` 完成前自检机制（TC-3 新增两条含反证全过），但默认关闭（`DEEP_AGENT_PRECOMPLETION_CHECKLIST` 未开）且未在真实模型交互中验证生效，不改变 0.7。 |
