...
export function doctor(args: Args): void {
  ...
  // 三道门（#1136 evidence 形态棘轮、#965 指向物存在性棘轮、#391 反向一致性）的共同
  // 输入：只收本次真正扫到的 phase，避免 `--phase 01` 这类局部运行把「没扫到的 phase」
  // 误判成「不再需要」——那会把陈旧检查变成假阳性门。
  // 存**完整的 Feature**：#391 要读 sprint / evidence 路径，另两道各自只用得到其中
  // 几个字段。刻意只留**一份**快照：多份各推一次，将来任何一边多一个 continue
  // 就会无声分叉。
