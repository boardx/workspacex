...
type Scenario = {
  closedAt: string;
  /** required check `verify-control-plane` 的观测序列：[结论, 相对合入时刻的分钟] */
  vcp: Array<[string, number]>;
  merged?: boolean;
...
function runDoctorWithFakeGh(sc: Scenario): string {
  ...
  const checks = [
    ...sc.vcp.map(([conclusion, min]) => ({ name: "verify-control-plane", status: "completed", conclusion, started_at: T(min) })),
    { name: "verify-affected", status: "completed", conclusion: "success", started_at: T(-30) },
    { name: "verify-full-compile", status: "completed", conclusion: "success", started_at: T(-30) },
    { name: "e2e-full", status: "completed", conclusion: "skipped", started_at: T(-30) },
