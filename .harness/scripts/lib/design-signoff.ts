...
import {
  ...
  frontmatterList as fmList,
  parseFrontmatter,
  frontmatterRawString as fmRawString,
  frontmatterString as fmString,
} from "./frontmatter";

export type SignoffStatus = "pending" | "confirmed" | "missing";

// frontmatter 解析本身已抽到 ./frontmatter.ts（2026-08-26，spec-ref.ts 也要读
// design-signoff.md 的 status 字段）。这里保留 re-export，兼容既有调用点
// （design-signoff.test.ts 等仍从 "./design-signoff" 导入 parseFrontmatter）。
