...
describe("部署链必须把沙箱镜像重建成当前源码那一版", () => {
  it("MinIO dev/deploy 使用同一个可用的、按 digest 锁定的镜像", () => {
    const minioBlock = (path: string) => {
      const compose = readFileSync(path, "utf8");
      return compose.slice(compose.indexOf("  minio:"), compose.indexOf("\n  redis:", compose.indexOf("  minio:")));
    };
    const expected = "cgr.dev/chainguard/minio@sha256:bd014394a80898e68c149f2311fdf8d5a2c2f3bb2c33b9327ae6d02b4b065ae1";
    const imageSource = readFileSync(MINIO_IMAGE_COMPOSE, "utf8");
    expect(imageSource).toContain(`image: ${expected}`);
    for (const consumer of [COMPOSE, DEV_COMPOSE]) {
      const block = minioBlock(consumer);
      expect(block).not.toMatch(/^    image:/m);
      expect(block).toContain("file: docker-compose.minio-image.yml");
      expect(block).toContain("service: minio-image");
    }
  });
