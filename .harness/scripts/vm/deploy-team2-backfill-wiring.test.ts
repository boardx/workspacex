 ...
 *   ③ advisory lock key 与另一个系统 agent 撞上 → 两个模板互相排队（0x7ea3 属于一个已退役的 ad-hoc agent）。
 *
...
describe("team2 补种脚本的部署接线", () => {
  ...
  it("补种步骤排在其他 agent 补种之后，与它们同属一段", () => {
    // 原先锚定的是另一个 ad-hoc agent 的补种步骤，它已随该功能下线删除；
  ...
  it("advisory lock key 是整数，且不复用已退役 agent 留下的 0x7ea3", () => {
    // 那个 ad-hoc agent 的补种脚本已删除，0x7ea3 因此空出来了。这里仍然断言不相等：
    // 机器上**可能还留着**它当初种进去的 agent 行（删脚本不删数据），
