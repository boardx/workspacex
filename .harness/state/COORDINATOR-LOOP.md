...
| 27 | **我自己也踩了共享 stash 栈——连协调者都不安全** | 我在主 checkout 上 `git stash -- <path>` 后又 `git stash pop`，第一条命令因为文件已提交而没建新 stash，`pop` 却弹出了别的 agent 早年留下的一份**过期基线快照**，把 `kernel.module.ts` 冲突成空白版本。核对 `git diff origin/main` 逐字为 0 才确认那份 stash 是废弃物，不是活的工作 | 纪律 17 不只约束 worker，**主 checkout 也不能用 `git stash` 做局部操作**。之后任何要临时挪走某个文件的场景一律用 `git worktree add` 或复制文件到 scratchpad |
| 25 | **「A 不跟着 B 变」的断言，必须在 B 已经变过之后才做** | F81 的第一条反证第一次跑是**绿的**：断言做在只有一个版本的时候，「按 `pinned_version_id` 取」与「取 head」返回同一行，13 条断言在两种实现下都成立。加 `moveHead()` 让上游先定版一次，才真的分得开 | 一般化：**不变性断言的前提是「可变的那一侧真的动过」**。没让 B 动过就断言 A 没变，测的是「A 等于 B 的初始值」。今晚四次「变体第一次跑是绿的」里，这是最隐蔽的一种 |
| 26 | **ratchet 数字：不许推算，也不许沿用自己上一次的实测** | F81 为同一行量了三次（41 → 42 → 43），**三次都是实测、三次都对**——对的是当时那棵树。每次合流都让前一次的测量作废，而**作废的形态在 diff 里就是一次普通文本冲突**：两段注释、两个数字，看起来只要挑一个留下 | 挑一个就漏一张表，而且**仍然全绿**——比较是 `-ge`，floor 少 1 永远不会红。⇒ **合流后必须从空库重跑**。同一处现在有 F49 / F117 / F81 三条连续记录，今晚这个坑被三个不同的人各踩一次 |
| 24 | **「上限数字」类的共享门控，只要两个 feature 并发就必撞** | `verify-rls.sh` 的 ratchet、`lint-permission-paths.mjs` 白名单、`permission-propagation-six-paths.test.ts` 的上限、`all-exceptions.filter.ts` 的封闭枚举计数——**每个加表/加权限路径的 feature 都要把同一个数字往上抬一格**。2026-07-31 下午 F49/F117/F32/F81 四个同时撞，三个 PR 一起 `CONFLICTING` | 短期：**串行化合并队列**，逐个 rebase 并**重新实测**（不许照抄、不许做加法）。长期：把这些上限改成**按 feature 追加的登记表**（`F49: +1 model_admission_tests`），下限 = 求和——追加不同的行才不会天天冲突，而且来源是显式的。⚠ 改这个门控本身要等没有在飞的 PR 时做 |
| 22 | **新增一个 workspace 包，会让所有既有工作树在重装依赖前变红** | F100 并入 `packages/fabric-markdown` 后，`tests/canvas/*` 在每个旧工作树里报 `Failed to load url @repo/fabric-markdown/templates`，**pre-push 对所有人都红**。有 agent 据此判定「`main` 坏了」并用了 `--no-verify` | 合并含新 workspace 包的 PR 之后，**主动通知所有在飞的 worker 在 rebase 后加跑 `pnpm install`**。CI 不会暴露这条（它每次都是全新 clone + 全新 install）——**这是一类只有长期存活的工作树才看得见的红** |
| 23 | **只跑 feature 自己的 verification 命令不足以自查** | F78 的 agent 手写了一个 `RetentionResolution`，形状与契约完全一致 ⇒ **两条 verification 全绿**，只有包级 `test` 里的 `lint-contract-source` 抓住了它 | 单一事实源那类门控挂在**包级 `test`** 上，不在 feature 的 verification 里。收工前必须跑 `TURBO_FORCE=1 pnpm turbo run typecheck lint test --filter=<动过的包>` |
| 20 | **scratchpad 目录在并发 agent 之间是共享的** | F10 的 worker 写了 `pr-body.md`，被 F31 的 worker 覆盖，它照着 `gh pr edit` 推上去 ⇒ **PR #72 的正文变成了 F31 的、`Closes` 指向 #42**。真合并的话会关掉别人的 issue，而 #39 永远不关。已发现并还原 | worker 的临时文件**一律带 owner 前缀**（`w0-files-F31-*.md`）。这是「共享可写状态」的第五种载体——索引、工作树、stash 栈、开发库、**scratchpad** |
| 21 | **ratchet 类的数字只许实测，不许照抄也不许做加法** | `verify-rls.sh` 的租户表下限连续三次失效：F80 加 2 张没抬、F108 加 2 张没抬、F48 抬到 34 时以为只差自己那批。F10 实测得 **38**，而「34 + 自己的 2」＝36——**差的 2 正是 F108 的**。算术里没有人会想起别人漏掉的表 | 规则写进注释：**不加表的 feature 也必须实测**，因为它想抄的那个数字恰恰是藏着别人表的那个。ratchet 静默失效时它看起来一直是绿的 |
| 19 | **「负载高」是个太好用的解释，它会把真红一起吸收掉** | 我整晚把 `@repo/api` 的红判成并发负载。实测：空闲机器、全新库、同一条命令连跑六次 = **绿/红49/红61/红22/绿/绿**——**单跑必绿、连跑就红**，与负载无关（issue #74）。而那堆噪音里**藏着一条真红**：`binding-three-modes.test.ts` 的悬空 promise（表现为「1163 passed / Errors 1 / 退出码 1 / 失败列表为空」），被我归进噪音，直到 F10 的 worker 单独挑出来 | 一个能解释所有异常的解释，通常什么也没解释。**先做控制变量，再下结论**；下不了结论就**把数据立成 issue**，不要照着没复现的诊断改全仓配置 |
