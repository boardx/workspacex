...
| 7 | 消息级操作：逐条复制、👍/👎 评分（`MessageRating`）、对 agent 提反馈（`FeedbackButton`） | 全部有：`chat-message-copy`/`MessageRating`/`chat-agent-feedback` | ~~全无~~ → **已补平**（issue #2054）。⚠ 原判「三个都没有」经实测更正：框架 `CopilotChatAssistantMessage` 默认就渲染一个真会 `copyToClipboard` 的复制按钮，缺的是本仓锚点与被验证过。评分需要真实落库 id，由新增的 `CUSTOM {chat_message_id}` 回显解决（详见 #2054） | ~~明显降级~~ 已平价 | 中 | CK-P3 ✅ |
...
| 9 | Run 进度细节透明度：已耗时计时器（每秒 tick）、阶段文案（`deriveRunPhaseLabel`）、45s longrun 提示、失败重试入口（`retryFailedRun` 双路径）、`AgentRunStatus` 权威状态条、上下文快照 `MessageContextSnapshot`(L1-L3)、每条消息思考链 `MessageThinkingChain` | 全部有 | **部分补平**（issue #2054）：耗时/阶段/longrun/重试已有（数据源分别是 `RUN_STARTED`、`TOOL_CALL_START`、`TEXT_MESSAGE_START`）。`AgentRunStatus` 权威状态条、上下文快照 L1-L3、思考链**仍无，且 v2 侧没有真实数据源**——三者读的都是 `AgentRunView`，而 v2 不轮询 `GET /api/agent-runs/{id}` | 剩余三项：明显降级 | 中 | CK-P4 部分 ✅；剩余三项已在 backlog 如实登记为不做（含前置条件） |
