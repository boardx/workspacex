...
## 第 6 轮（覆盖 `16a9103f`，PR #2011 + PR #2013 合并后的首次统一重新评分）

> ⚠ 编号核实：开工前用 `grep -n "第 .* 轮" .harness/state/copilotkit-v2-ux-acceptance-score.md`
> 核实过文件里已有的最新章节是"第 5 轮"（上面那节，编号有过一次事后改写，见其
> 自己的编号说明）——本节因此记为"第 6 轮"，不是任务书草稿里预设的"第 5 轮"。
> 任务书本身提到"上一轮 PR #2011 与 PR #2013 是并行完成的两条独立修复"，与本节
> 开头核实到的事实一致。

- **评分日期**：2026-08-25。
- **实测 SHA**：`16a9103f`（`origin/main` 分支尖端，`git log --oneline -1 origin/main`
  核对——PR #2013 的合并提交本身，其父提交链包含 PR #2011，二者都已在 main 的血统
  里，`git log --all --oneline | grep -i "2011\|2013"` 与 `gh pr view 2011/2013
  --json state,mergedAt` 双重核对过 `MERGED`，不只信静态痕迹）。
- **方法**：独立隔离 worktree（`scratchpad/wt-da19g-round5`，目录名沿用创建时的
  猜测编号，未碰共享主目录，产品分支已重命名为 `docs/copilotkit-v2-score-round6`
  对齐最终编号），跑 `./init.sh` 确认基础环境通过后，跑一次完整
  `pnpm run verify:chat-read`（真实 Chromium + 真 Postgres/Redis + 真 API + Next
  dev + 确定性 deep-agent/ASR/vision loopback 替身），覆盖全部 18 个匹配
  `playwright.chat-read.config.ts` `testMatch` 正则的 spec 文件（41 个用例，
  5-worker 并行一次）——这是第 5 轮小结里明确建议的"下一轮应补一次正式的全量
  回归逐项复核"，本轮把它做完。

### 全量回归结果：39 passed / 2 failed（均为已知既有噪音，非新回归）

1. `chat-read.spec.ts:245`（"formal Chat with no projectId goes personal"）——
   登录后 `toHaveURL(/\/projects$/)` 30s 超时，`playwright.chat-read.config.ts`
   文件头 `#733` 一节已完整记录同一类"首次编译+短暂不可达"时序竞态指纹（不同次
   失败卡在不同断言点，是这条指纹的特征之一，本轮卡点与该记录描述的现象一致）；
   这条用例与 `/chat/copilotkit-v2`（DA-19g 关注的路由）完全无关，不影响本轮任何
   一项判据的复核范围。
2. `copilotkit-v2-runtime-adapter.spec.ts:216`（"清空 token 后同一条路由必须
   失败"）——失败堆栈 `route.fetch: Test ended` 发生在 `:161` 行同一处，与第 5 轮
   记录逐字节同源（同一测试自己拦截到面板背景 `/suggest` 请求、`route.fetch()`
   还在飞行中测试已收尾）。第 5 轮已用 `--workers=1` 单独重跑坐实过一次这条用例
   本身是好的，本轮不重复隔离重跑，直接引用第 5 轮的结论：并行噪音，非功能回归。

未发现任何新失败、任何此前通过现在失败的用例。

### 第 1 项与第 7 项：本轮任务书点名的核心复核目标

**第 1 项流式反馈**：`copilotkit-v2-stream-frame-timing.spec.ts` 全量并行跑中
真实通过（24.5s），控制台采样序列 `console.log` 输出为
`DA-19g stream frame samples: [0,48,95,115]`——单调递增、4 个真实增长点、跨约
一秒真实展开，与第 5 轮单独重跑记录的 `[0,32,72,112,115]` 属于同一类真实分批
渲染节奏（具体数值随每次运行的 SSE 时序有正常抖动，不是断言容忍的宽松度问题——
断言本身检查的是"单调不减 + ≥4 个观测点 + 无单次跳变吃掉 60%+ 总增量"这三条
结构性质，不检查具体数值）。确认第 5 轮"验证坐实，非产品缺陷"的结论未退步。

**第 7 项错误处理透明度**：`copilotkit-v2-error-banner.spec.ts` 全量并行跑中
真实通过（1.2m）——`copilotkit-v2-error` 横幅真实渲染、文案人类可读（未见任何
裸枚举码字面量）、横幅出现后 `copilotkit-v2-send` 重新可点击且第二轮消息真实
发送成功。确认第 5 轮"发现真实缺陷并已修复"的结论未退步，且是在与第 5 轮不同的
一次全新进程/全新数据库实例上复现，不是同一次运行的记录被重复引用。

### 其余各项：全量回归确认无退步，逐项归因到通过的用例

2. **可见的规划步骤：0.8**（不变）——`copilotkit-agui-state-snapshot.spec.ts`
   真实通过（10.3s）：`STATE_SNAPSHOT` 事件真实驱动 `AgentPlanPanel` 渲染。
3. **可见的工具调用与进度：0.7**（不变）——`copilotkit-v2-tool-rendering.spec.ts`
   两条用例（`write_todos`/`search_documents`）真实通过（28.4s/42.0s）；
   `search_documents` 结果文本断言仍受文件头"已知限制③"约束，不打满分，与
   第 3/4/5 轮记录的保留意见一致。
4. **真实的多步能力：0.3**（不变，结构性限制——loopback 替身是确定性剧本，
   非真实模型推理，多轮以来反复记录，未变）。
5. **语音输入体验：1.0**（不变）——`copilotkit-v2-voice-input.spec.ts` 真实通过
   （34.2s）。
6. **多轮上下文：1.0**（不变）——`copilotkit-v2-runtime-adapter.spec.ts` 的
   DA-19g 多轮上下文回归测试真实通过（14.6s），第二轮回复真的引用第一轮原文。
7. **错误处理透明度：1.0**（不变，见上）。
8. **消息呈现质量：1.0**（不变）——markdown/mermaid 渲染测试真实通过（35.6s）。
9. **控制感：1.0**（不变）——`copilotkit-v2-hitl.spec.ts` 三条路径
   approve（18.0s）/edit（38.9s）/reject（31.2s）全部真实通过，HITL 交互按钮
   真实渲染，用户可在 30s 内直接采取行动，不是干等。
10. **整体连贯性：1.0**（不变）——`copilotkit-v2-hitl-dialog-dismiss.spec.ts`
    真实通过（19.2s），Escape 退出后遮罩真实从 DOM 移除；全量跑 18 个 spec 文件
    未发现假按钮/死链接/风格孤岛类新问题。
...
| 项 | 第 5 轮 | 第 6 轮 | 变化 | 结论 |
|---|---|---|---|---|
| 1 流式反馈 | 0.8 | 0.8 | 0 | 全量跑中真实通过，确认无退步 |
| 2 可见的规划步骤 | 0.8（未复核，带入） | 0.8 | 0 | 本轮正式复核，确认无退步 |
| 3 可见的工具调用与进度 | 0.7（未复核，带入） | 0.7 | 0 | 本轮正式复核，确认无退步 |
| 4 真实的多步能力 | 0.3（未复核，带入） | 0.3 | 0 | 本轮正式复核，结构性限制未变 |
| 5 语音输入体验 | 1.0（未复核，带入） | 1.0 | 0 | 本轮正式复核，确认无退步 |
| 6 多轮上下文 | 1.0（未复核，带入） | 1.0 | 0 | 本轮正式复核，确认无退步 |
| 7 错误处理透明度 | 1.0 | 1.0 | 0 | 全量跑中真实通过，确认无退步 |
| 8 消息呈现质量 | 1.0（未复核，带入） | 1.0 | 0 | 本轮正式复核，确认无退步 |
| 9 控制感 | 1.0（未复核，带入） | 1.0 | 0 | 本轮正式复核，确认无退步 |
| 10 整体连贯性 | 1.0（未复核，带入） | 1.0 | 0 | 本轮正式复核，确认无退步 |

净总分 **8.5 → 8.5**，**+0**——本轮不是"发现了新修复"，是把第 5 轮小结里明确
标注为"假设推算值，未逐项单独打分复核"的 8.5 分，通过一次完整的全量回归
（18 个 spec 文件、41 个用例、39 passed）转成"逐项确认过的实测值"。这本身是
有价值的工作（消除了"总分可能虚高"的不确定性），只是不体现在分数变化上。
...
1. **真实的多步能力（0.3，结构性限制，长期存在，五轮未变）**——loopback 替身是
   确定性剧本，非真实模型推理，不是"下一轮该修"的东西，除非人类决定要接入真实
   模型评测。这是当前唯一分数最低的一项，但**不可通过测试/前端改动提升**——
   除非改变评测策略本身，否则会长期停在 0.3。
2. **可见的工具调用与进度（0.7）**——`search_documents` 卡片结果文本断言仍受
   "已知限制③"约束（不强断言具体结果文本），从第 3 轮起就是待办，三轮未推进，
   建议下一轮真正花时间补齐这条断言的强度（这是当前分数第二低、且**可以**通过
   补测试断言提升的一项）。
3. **流式反馈（0.8）**——UI 帧级独立复核已在第 5 轮补上并在本轮全量回归中再次
   确认，扣分点是"客户端合批粒度比 wire 粗"这一如实登记的观测结果，暂无已知
   进一步提升路径（除非做更细粒度渲染优化，且明确不是本项判据强制要求的）。
4. **可见的规划步骤（0.8）**——本轮全量回归确认无退步，暂无已知具体缺口，维持
   现状。
5. 其余 5 项（语音 1.0、多轮上下文 1.0、错误处理透明度 1.0、消息呈现 1.0、
   控制感 1.0、整体连贯性 1.0）暂无已知具体缺口，本轮全量回归确认。

由于第 4 项是结构性限制、不受本轮或未来纯前端/测试改动影响，实际"可推进的
上限"目前是 **9.7/10**（10.0 − 0.3）——如果人类希望突破这个上限，需要先决定是否
改变第 4 项的评测策略（接入真实模型推理而非确定性 loopback 剧本），这是本轮
新发现的一个值得向人类汇报的判断，不是继续按老思路"下一轮再修一项"就能解决的。

### 本轮清理
本轮唯一产生的 docker compose 栈由 `pnpm run verify:chat-read` 通过
`with-test-isolation.ts` 起停，命令退出（含失败退出码 1）后已随脚本自身
teardown 释放；`docker ps --filter name=wsx-` 复核未见本轮遗留的栈。隔离 worktree
（`scratchpad/wt-da19g-round5`）未对产品代码做任何改动，仅用于跑既有测试取证
与提交本状态文件。
