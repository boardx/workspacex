...
### F6：差集诊断给出终局答案——不是产品缺陷，是点击自始至终没生效
...
**服务端一条都没多。** 两种可能就此分开：不是"线程建出来了但第二个标签页没切过去"
（那会是新增 ≥1 条，属产品缺陷、该开 issue），而是这一页上那次点击自始至终没生效。
**因此不开 issue。**

处置：**前置条件改用权威写入**（`POST /chat/threads/mutate`，与产品新建走同一个端口），
不再点按钮。理由不是"绕过去"，是这五跑暴露的一件事——F6 测的是「两条线程同时跑时事件
不串线」，**不是**「第二个标签页能不能点出新线程」。把两件事绑在同一条用例的前置里，正是
它拖了五跑的原因；被测的并发行为一个字都没放宽。

⚠ 若要证「第二个标签页点新建会话真的可用」，那是**另一条路径、该有自己的用例**，
不该继续挂在 F6 的前置里。
