...
| 束 | 覆盖的 feature | 核心不变量 |
|---|---|---|
| `identity` 身份与权限 | F01 F02 F03 F15 F16 | 两层交集鉴权；RLS 强制；管理员不是超级用户；本地组织三条硬隔离 |
| `artifact` 原件与版本 | F04 F05 F07 F08 | 原件不可变；更新走新版本；SHA-256 可校验；引用必指向不可变快照 |
| `context-pack` 上下文装配 | F09 F10 F11 F12 F13 | 引用必可定位；丢弃清单可查带原因；同 run id 可重放 |
| `binding` 三模式绑定 | F06 | 固定快照绑定后上游变化不改写它 |
| `web-kernel` 前端内核 | F14 | 设计 token / 字号档位单源；七态固定保留名 |
