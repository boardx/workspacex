...
6. **响应体也要被契约校验，不只是请求体**（修订 B-8，2026-07-29 升为通用规则）。
   ...
   **并特别覆盖拒绝路径**（那正是 B-7 里契约写错的地方），
   ...
   构建期失败才是发现它的正确位置。
8. **验收标准里的每个动词，必须能指到一个契约操作**（2026-08-04 新增，三次同型失败换来）。

   写 issue 的验收时，把每个动词（「新增」「发布」「停用」「应用」…）**逐个对着
   `packages/contracts/src/<bundle>.ts` 核一遍**：有没有一个操作真的能做这件事？
   `in` 里有没有它需要的字段？做不到就**不要写进验收**——写了也是永远绿不了的一条。

   ⚠ 三次先例，全部不是实现者偷懒，是**验收照着「应该能做什么」写的，没对着契约核**：

   | issue | 验收写的动作 | 契约的实际情况 |
   |---|---|---|
   | #459 | 「发布 v1 → 停用」 | `已启用` 只能由人工评审的 approve 分支产生；`SKILLS_FORBIDDEN_ROUTES` 逐字禁止直达启用路由。**要凑出这个流程只能去碰那个后门** |
   | #464 | 「新增模板 → 保存 → 刷新仍在」 | 五个模板操作全部作用于已存在的 `:key`，`publishTemplate.in` 无内容字段。**没有任何操作能造出一行** |
   | #460 | 「（空项目里）新建会话」 | `listThreads.out` 不含 capabilities，零会话时前端拿不到写权判据 |

   三次的共同代价是一样的：**实现者做到一半才发现验收不可达**，然后要么停下来等裁决
   （#459 停了一轮），要么被迫在"发明一个后端接口"和"交付不了"之间选（#464 选了
   如实报告，是对的）。

   **可操作的做法**：验收里每条断言后面标出它依赖的契约操作名。标不出来的，
   要么改验收，要么**先开一条补契约的 issue**——但那属新契约面，走 ADR-023。

   由 coord-chat-e2e 在连续撞上三次后提出，coord-main 采纳并升为全队规则。
